...
> ERP 신규 발견 버그는 본 §8 하단에 추가 기록 (#16+).
...
`/erp/vehicles/{id}/documents` 라우트에 별도 화면. 차량 데이터를 서식 템플릿에 머지해 PDF/Excel 생성:

| 서류 | 생성 소스 | 출력 |
|---|---|---|
| 차량말소신청서 | 차량 + 소유자 | PDF |
| 매입계약서 | 차량 + 소유자 + 매입가/계약금 | PDF |
| 등록증신청서 | 차량 + NICE API 데이터 | PDF |
| 양도증명서 (有/無) | 차량 + 바이어 | PDF |
| Proforma Invoice | 차량 + 바이어 + 통관 정보 | PDF |
| RO_CIPL / con_CIPL | 차량 + 선적 정보 (RORO/Container 분기) | Excel |

**라이브러리 후보**: `barryvdh/laravel-dompdf` (PDF) / `maatwebsite/excel` (Excel) — 도입 시점에 결정.

## 13. 핵심 비즈니스 로직 공식 (참조용)

### 판매 미입금액
```
총판매액 = sale_price + transport_fee + sale_other_costs - tax_dc + commission + auto_loading
총입금액 = deposit_down_payment + interim_payment + advance_payment1 + advance_payment2
        + Σ(final_payments.amount) + savings_used
미입금액 = 총판매액 - 총입금액
```
