...
- ✅ **2026-09-04 고침**(master `aaeea91`) — 화면 업로드 5곳 전부 `exists()` 로 실제 저장을 확인한다.
  가드도 그 검사를 요구하게 강화(`FileWriteResultCheckedTest`).
...
### 71. 🎨 양식에 박힌 **샘플 리터럴**은 칸 색깔에 따라 「비거나 · 거짓으로 인쇄된다」 (2026-08-31)

통관 SET 「용도/Usage」가 `heyman` 세트에만 cascade 수식이었고 `system`(ssancarerp)·`karaba` 는
**샘플 리터럴**(`자가용` / `Private car`)이 그대로 박혀 있었다. 같은 리터럴인데 **증상이 정반대**였다:

| 칸 | 색 | `DocumentFiller` 동작 | 증상 |
|---|---|---|---|
| 영문등록증 P4 | **노란칸** | 「매핑 없는 샘플값」으로 보고 **비운다**(§12) | **공란 인쇄** — jin 이 본 것 |
| 한글등록증 P4 | 흰칸 | 안 건드린다 | **모든 차량에 「자가용」 인쇄** — 거짓 |

🚨 **더 위험한 건 흰칸 쪽이다.** 비어 있으면 사람이 알아채지만 「자가용」은 그럴듯해 보인다
(실측 heymanerp 에 **영업용 3 대** 실재 — 그 차들의 통관 서류가 계속 「자가용」으로 나가고 있었다).

**🧭 규칙**
- **한 세트에서 고친 양식은 세 세트 전부 확인**할 것. `heyman` 만 수식이었다 — 나머지는 샘플 그대로였다.
  파생 스크립트(`generate-karaba-templates.php`)는 **회사정보 셀만** 치환하므로 이런 차이는 그대로 복제된다.
- **양식을 고칠 땐 「무엇을 넣나」보다 「안 넣는 칸에 무엇이 남아 있나」를 볼 것**(#68 과 같은 뿌리).
- 🚫 **별도 매핑으로 때우지 말 것** — 통관 SET 은 「구매리스트 마스터 → 6시트 cascade」 설계다.
  같은 값의 출처가 둘이 되면 갈린다. **뿌리 셀(`구매리스트!B8`)이 이미 매핑돼 있는지부터 확인.**
- 🔒 가드 = `ClearanceUsageAndContainerTest` — ①3 세트 양식을 **정적 대조**(수식 문자열 일치)
  ②**생성물의 셀을 실제로 읽어** `영업용 → Business` 를 단언(#37 「검증은 배열이 아니라 생성물로」).
  ⚠️ 기능 테스트로는 원리상 못 잡는다 — **서류는 정상 생성되고 칸만 비거나 틀린다**.
- 도구 = `scripts/fix-clearance-usage-cascade.php`(dry-run 기본 · `--verify` 로 3 세트 실측 대조).
  저장은 `stripHyperlinks` + **`setPreCalculateFormulas(false)`** — preCalc 켜면 크로스시트 수식이
  값으로 굳어 cascade 가 통째로 죽는다.

**곁다리 — 고쳐도 안 채워질 수 있다.** cascade 를 살려도 `nice_reg_use_type` 이 비면 빈칸이다
(실측 ssancarerp 4,687/4,695 가 빈값 — 적재 원본에 용도 열이 없었다). **「칸이 빈 이유」는
①양식 ②원본 데이터 둘 다일 수 있으니 양쪽을 실측**할 것. 거짓값을 채워 넣어 덮지 말 것.

---

### 71-B. 🫥 **잘려서 안 보이는 것은 「없는 것」이 아니다** (같은 날, 같은 양식)

인보이스·팩킹리스트 `E2`·`E3` 에 **양식 제작 당시 차량의 컨테이너 번호**가 라벨과 함께 박혀 있었다:
`Reference No.      DFSU6646075`. 3 사 전부, **흰칸이라 안 지워진다**(위 표의 그 분기).

**아무도 못 본 이유** = `E2:F2` 병합 폭이 9.9 뿐이고 옆 `G2` 에 값이 있어 **글자가 잘려 있었다.**
jin: *"이거 잘려서 내가 못봤네 클릭하기 전까지는."*

- 🧭 **폭·병합을 건드리는 변경 전엔 그 근처 셀을 「값」으로 훑을 것.** 화면에 안 보인다고 없는 게 아니다.
- 🔎 **전수 스캔이 싸다** — 9 종 × 3 세트를 정규식(`[A-Z]{4}\d{7}`)으로 훑어 **7 곳**을 찾았다.
  그중 4 곳만 진짜 문제였고 3 곳(`container_contract` F4 등)은 **매핑 있음 + 노란칸**이라 생성 시 덮인다.
  ⇒ **찾은 것을 다 고치지 말고 「매핑 여부 × 칸 색」으로 먼저 분류**할 것.

### 71-C. 📏 양식 폭이 **「규격」 기준**이면 실제 값의 다수가 잘린다

컨테이너 NO 칸(`G`)이 **ISO 규격 11 자**(너비 12.25) 기준이었는데 운영 실측은 이랬다:

| 값 | 자수 | 필요 너비 | 건수 |
|---|---|---|---|
| `ABCD1234567` (ISO) | 11 | 10.4 | ssancarerp 783 · heymanerp 75 |
| `6.06_G RORO 11-27_10` | 20 | **19.3** | **ssancarerp 3,571** |
| `EITU9093137 /  EMCKPM6885` | 25 | **24.0** | heymanerp 31 |

두 회사 다 **자체 관리코드·2 개 병기**를 적고 있었다 — 규격만 보면 다수가 잘린다.
- 🧭 **「이 칸에 실제로 뭐가 들어가나」를 길이 분포로 실측**할 것(`char_length` GROUP BY).
- **열 넓히기 + 자동축소(`shrinkToFit`) 조합**이 답이었다. 축소만 쓰면 25 자가 7pt 아래로 내려가
  못 읽고, 줄바꿈은 행 높이를 키워야 하는데 그 행이 다른 병합과 얽혀 있었다.
  ⚠️ **엑셀은 자동축소와 줄바꿈을 동시에 못 켠다** — wrap 이 켜져 있으면 축소가 **무효**다.
- 🅿️ 폭을 넓혀도 인쇄는 안 깨진다 — 이 시트들이 `fitToPage`(1×1)라 엑셀이 배율을 다시 잡는다.
  **바꾸기 전에 `getPageSetup()->getFitToPage()` 를 확인**할 것(고정 배율이면 얘기가 다르다).
- 🚨 **미러 시트는 값만 따라오고 서식은 안 따라온다** — 차량팩킹이 `=차량인보이스!G2` 로 값을 미러하는데
  **열 너비는 자기 것**이라, 인보이스만 넓혔더니 팩킹리스트는 그대로 잘렸다(#69 의 그 형태).
  **한 시트를 고쳤으면 그 값을 미러하는 시트를 `grep` 할 것.**

### 71-D. ⚠️ **「이미 정상이라 제외」한 대상은 기준이 바뀌면 다시 포함해야 한다**

용도 스크립트가 `heyman` 을 「이미 수식이 있다」고 세트 목록에서 제외했다. 그런데 나중에 문구가
`Private` → **`Private Car`** 로 바뀌자 **제외 이유가 사라졌는데 제외는 남아** heymanerp 만 옛 표기로
남을 뻔했다(`--verify` 가 잡았다).
⇒ **제외는 「대상 목록」이 아니라 「값 비교」로 할 것.** 목록에서 빼면 기준이 바뀌어도 안 돌아온다.
...
`vehicle.stat.ems_fee` 를 lang 파일의 `stat` 이 아니라 **`col` 그룹**에 넣었다. 결과:
화면에 `vehicle.stat.ems_fee` 라는 **키 문자열이 그대로** 찍혔다(jin 제보). 예외 0 · 로그 0.

🔑 **`LocaleKeyParityTest` 가 통과한 이유** — 그건 ko 와 en 을 **서로 대조**하는데, 두 파일에
같은 스크립트로 넣어서 **양쪽이 똑같이 틀렸다**. 대조는 통과하고 화면만 틀린다.
⚠️ 곁다리로 `col` 그룹엔 **같은 키가 두 번** 생겨 나중 값이 앞 값을 조용히 덮었다.

🧭 **fallback 때문에 한쪽만 빠지면 안 드러난다** — `fallback_locale='en'` 이라 ko 만 없으면
**영어로 렌더**된다. 키 문자열이 보이려면 **양쪽 다** 없어야 한다. ⇒ 재현 실험을 할 때
ko 만 지우면 «안 잡히네» 로 잘못 결론 내린다(실제로 한 번 그랬다).

**가드 = 렌더 결과를 본다** — `VehicleShipmentPanelTest::test_no_untranslated_key_leaks_into_the_vehicle_screen`.
화면을 렌더해 `vehicle.` · `common.` · `domain.` · `settlement.` 로 시작하는 **점 두 개짜리 토큰**이
남아 있으면 실패한다. 조건부 렌더(합계 블록·모달)는 값을 만들어 두고 전부 열어서 찍는다.

🚨 **가드를 넣었으면 「일부러 깨뜨려」 실패하는지 볼 것.** 이번에 처음 만든 정규식은
역슬래시+b(\\b) 가 **실제 백스페이스 문자(0x08)로 박혀** 아무것도 매칭하지 않았고 — 그래도 초록이었다.
키를 지우고 돌려 보고서야 알았다. 정규식을 스크립트로 파일에 써 넣을 땐 `cat -A` 로 실물을 확인한다.

### 74. 🗜️ **「켜져 있다」와 「내 응답에 적용된다」는 다르다** — nginx 압축 (2026-09-01)

3사 nginx 가 `gzip on;` 인데 **`gzip_types` 가 주석**이라 기본값 `text/html` 만 압축하고 있었다.
**Livewire 응답은 `application/json`** 이라(`vendor/livewire/.../HandleRequests.php:234`)
**화면 조작 응답이 전부 비압축**으로 나갔다. 우분투 nginx 가 배포하는 기본 상태다.

**실측** — ssancarerp 하루 `livewire/update` **2,473건 · 1,052MB · 평균 446KB**.
적용 후 같은 로그에서 **평균 96,968B (−78%)**. 정적 자산도 676KB → 165KB.

🔍 **왜 아무도 못 봤나 — 이게 이 부류의 특징이다:**
- **`gzip on` 을 보면 「압축 켜놨네」 하고 넘어간다.** 실제 동작은 그 아래 목록이 정한다.
- **확인해도 통과한다.** 로그인 페이지를 열어 보면 HTML 이라 **gzip 이 뜬다.**
  정작 트래픽의 86% 인 JSON 은 로그인해서 버튼을 눌러 봐야 보인다.
- **에러도 로그도 없다.** 조금 느릴 뿐 아무것도 안 깨진다.
⇒ **확인은 「설정에 뭐라고 적혀 있나」가 아니라 「그 응답 헤더에 뭐가 오나」로** 할 것.
  (SKILLS #57 의 `coverage: xdebug` 와 같은 뿌리 — **스캐폴딩·배포판 기본값은 검토된 것이 아니다.**)

🗺️ **곁다리로 밟은 것 — vhost 마다 `access_log` 가 다르다.** ssancarerp 는
`/ssancar-erp/logs/nginx-access.log` 로 따로 쓴다. 기본 경로(`/var/log/nginx/access.log`)에는
board 와 봇 스캔만 있어서 **「트래픽이 제일 적은 회사」로 오판**했다(실제로는 가장 많다 —
요청 5배·바이트 19배). ⇒ `sudo nginx -T | grep access_log` 로 **먼저** 확인할 것.
그리고 **한 회사 숫자로 일반화하지 말 것** — 3사는 데이터 규모가 18배 차이난다.

📏 **성능은 「DB 시간」만이 축이 아니다.** 목록 20행 렌더는 264대 38ms · 4,706대 33ms 로
**데이터량과 무관**했다(페이지네이션 + eager load 9종). 그런데 **응답 크기는 124KB → 446KB** 로
3.6배였다 — 데이터가 늘면 **그려내는 HTML 이 커진다.** 「DB 는 멀쩡한데 느리다」면 이 축을 볼 것.

🛠️ 적용 = `gzip_types` 주석 해제 · `location /build/` 에 `immutable` · `listen 443 ssl http2`
(**nginx 1.24 는 `http2 on;` 문법이 없다**). **서버 설정이라 git 배포로 안 나간다 — 3사 각각.**
상세·재확인 순서 = `docs/operations/perf-review-2026-09-01.md` **§10**(§0~§6 은 정정 전 초판).
...
karaba 양식 **4곳**(위임장 C25 · 통관SET 말소증 K21 · 한글/영문등록증 M8)에 **싼카 법인등록번호
`120111-0922270`** 이 그대로 인쇄되고 있었다. `generate-karaba-templates.php` 의 치환맵이 상호·주소·
사업자번호는 바꿨는데 **법인등록번호 칸은 아예 없었다.** 그중 위임장은 **관공서 제출 서류**다.

🔑 **heyman 은 같은 부류를 이미 정리한 이력이 있다**(`docs/operations/heyman-company-info-cleanup.md`,
실측 4곳 전부 정상). **heyman 때 잡은 걸 karaba 때 놓친 것** — 사람 기억으로는 못 막는다.
⚠️ jin 은 「분명 줬다」고 기억했고 실제로 줬다 — 준 건 **사업자등록번호**(`801-81-01696`)였다.
**자릿수·성격이 다른 별개 번호**라 그 칸엔 못 쓴다. 「줬다/안 줬다」가 아니라 **어느 번호냐**가 쟁점이었다.

**🧭 새 회사를 붙일 땐 「무엇을 바꿨나」가 아니라 「원본 회사 값이 어디 남았나」를 전수 검색**할 것 —
13자리 번호·상호·주소·계좌를 정규식으로 훑으면 나온다. 치환맵 리뷰로는 **빠진 칸을 원리상 못 본다**.
도구 = `scripts/fix-karaba-corp-number.php`(dry-run 기본 · `--verify` 는 **3사 전부** 훑어 남의 번호를 잡는다).
🚫 제외는 「이 파일은 고쳤다」는 목록이 아니라 **값 비교**로(#71-D).

---

#### 75-B. 📄 **「시트만 뜯어온 초기문서」를 양식으로 채택할 때** (같은 날, 말소증 한·영)

jin 이 준 `말소증_한글_영문.xlsx` 는 원본 워크북에서 시트만 분리한 것이라 세 겹으로 망가져 있었다:

| 잔재 | 규모 | 안 걷어내면 |
|---|---|---|
| `=#REF!` 수식 | 한글 10칸 · 영문 9칸 | 🚨 **매핑이 통째로 조용히 무시**된다 |
| `definedName` | **120개**(`매입가`·`주민번호`·`ㅏㅓㅏ` …) | Excel 이 "복구할 수 없는 콘텐츠" 로 거부(#19) |
| 외부링크 | 3개(`D:\성신무역\…`) | 열 때마다 링크 갱신 프롬프트 |

🚨 **`#REF!` 가 제일 위험하다** — `DocumentFiller::writeCell` 은 `=` 로 시작하는 셀을 **절대 안 덮어쓴다**
(cascade 보존 규칙). 매핑을 붙여도 예외·로그 없이 건너뛰고 서류에 `#REF!` 가 인쇄된다.
**PhpSpreadsheet 로 load→save 하면 외부링크는 자동으로 빠지지만 `#REF!` 와 definedName 은 안 빠진다.**

그리고 **흰칸 샘플 리터럴**(차종 `승용`/`Passenger` · 용도 `자가용`/`passenger`)이 또 있었다 — #71 의 그 자리.
노란칸 청소가 흰칸은 안 건드리고, 값이 없는 차는 `writeCell` 이 빈 값을 건너뛰어 **샘플이 그대로 인쇄**된다.
**테스트가 잡았다** — 「NICE 값 없는 차는 빈칸이어야 한다」를 **3사 전부** 돌린 덕이다.
⚠️ 처음엔 한 세트만 봐서 놓칠 뻔했고, 가드를 **일부러 깨뜨려 보다가** 엉뚱한 세트를 깼더니 초록이 나와
그제야 알았다(#73 의 그 확인 — **깨뜨릴 땐 그 테스트가 실제로 쓰는 세트를 깰 것**).

---

#### 75-C. 🔤 **변환 수식이 실제 데이터와 어긋나 있어도 아무도 모른다** (같은 날, 영문 차종)

통관 SET `구매리스트!I6` 의 중첩 SUBSTITUTE 가 승합·화물을 **「중형 승합」 순서**로 찾는데
실제 NICE 값은 **「승합 중형」**이라 안 잡혀 **영문등록증에 한글이 그대로** 인쇄됐다.
덤으로 승용 대형만 `HEAVY Passenger`(나머지는 Small/Medium), 승합은 `Ven`(Van 오타).

- **왜 안 드러났나** — 운영 분포가 승용에 쏠려 있다(heymanerp 265대 중 승합 **1대**·화물 0).
  이전 세션이 승용 순서만 고치고 나머지는 못 봤다. 서류는 정상 생성되고 그 칸만 한글이다.
- **🧭 변환표를 고칠 땐 운영 분포를 실측**할 것 — `GROUP BY` 한 줄이면 나온다.
  희소값은 **테스트에 명시적으로 넣어야** 지켜진다(그래서 `승합 중형`·`화물 대형`을 케이스로 박았다).
- **같은 변환이 4곳**이었다(3사 양식 수식 + 말소증 PHP) → `DocValue::vehicleFormEn()` 하나로 합쳤다(#45).
  🚨 **수식을 비우는 것과 매핑을 붙이는 것은 반드시 같은 커밋** — 순서가 어긋나면 한쪽은 조용한 무시,
  다른 쪽은 빈칸이다. 도구 = `scripts/fix-clearance-vehicle-type-en.php`(cascade 소비자 생존까지 확인 후 진행).
- 순서·띄어쓰기를 안 가리게 짰다(`승용 중형`·`중형 승용`·`중형승용` 동일) — 옛 적재분 표기가 섞여 있다.
  **모르는 값은 원본 그대로 통과**시킨다(실측 쓰레기값 `205 004` 1대) — 영문으로 위장하지 않는다.

**🔁 곧바로 같은 자리에서 하나 더 나왔다 — 「고친 것과 같은 부류가 옆 칸에」** (jin: *"기존 서류에서 고쳐야 할건 없었어?"*)

차종을 고친 뒤 **전수로 훑었더니** 같은 시트 아래쪽 **연료(I13)**가 더 컸다. 실측 heymanerp **39/254 = 15%**:
`휘발유(무연)`→`GASOLINE(무연)` · `하이브리드(경유+전기)`→`HYBRID(DIESEL+전기)` · `가솔린`·`엘피지`·`디젤`은 통째로 한글.
게다가 `DocValue::fuelEn()` 이 **이미 있었고 컨/RORO 인보이스가 쓰고 있었다** — 같은 값을 두 방식으로 계산 중이었다.

**전수 훑는 법이 핵심이다** — 매핑을 읽지 말고 **생성물의 영문 시트를 렌더해 한글(`/[가-힣]/u`)을 찾는다.**
그렇게 3건이 더 나왔다: 판매 인보이스 Code = **한글 번호판**(계약서는 07-29 부터 로마자였는데 안 따라옴) ·
판매 인보이스/RORO·컨테이너 **계약서** Maker = 한글 브랜드(**같은 선적 건의 Invoice&Packing 은 영문**이라 두 장이 갈렸다).
⚠️ 인쇄 영역(`getPrintArea()`)을 먼저 봐야 한다 — 한글 11건 중 상당수는 **드롭다운 소스 목록**이라 안 찍힌다.

🚫 **매핑에 없는 표기 변형을 놓치지 말 것** — `르노(삼성)`(괄호)이 `르노삼성` 매핑에 안 잡혀 그대로 나갔다.
**운영 값을 `GROUP BY` 로 뽑아 표를 채울 것**(3사 한글 브랜드 = 10대 5종이 전부였다).
🚨 그러다 **같은 키를 두 번 적어** 앞 정의가 조용히 무시됐다(PHP 배열은 뒤엣것이 이긴다, 예외 0) →
가드 `EnglishDocumentTermsTest::test_brand_map_has_no_duplicate_keys`.

🔒 **재발은 정적 검사로** — 외국인 서류 매핑 6개가 `$v->brand`·`$v->vehicle_number` 를 **가공 없이** 쓰면 실패
(`romanizePlate(...)` 로 감싼 것만 통과). 값이 한글이어도 서류는 정상 생성되므로 기능 테스트로는 원리상 못 잡는다.


### 76. 🔗 **여러 이름으로 서비스되는 서버에서, 밖으로 나가는 링크를 요청 컨텍스트에서 만들지 말 것** (2026-09-04)

카카오 알림톡 월배치 승인 버튼이 `K108:NoMatchedTemplateButton`(「등록된 버튼과 다름」)으로 **간헐 실패**했다.
화면엔 제출 성공으로 보이고 **대표에게는 아무것도 안 갔다** — 9/2 제출분이 이틀 방치됐다.

**승인본과 전부 대조해도 다른 게 없었다** — 템플릿코드·템플릿명·헤더·하이라이트·아이템 3줄·요약·
버튼명·타입·링크 접두어가 전부 일치하고 검수상태도 승인·정상. 남은 차이는 **호스트 하나**였다.

```
nginx server_name 이 회사마다 여럿이다 — 도메인 + www + IP (ssancarerp 는 localhost 까지)
앱에 forceRootUrl 이 없으면 **HTTP 요청 안에서 만든 URL 은 「접속한 호스트」를 그대로 쓴다**
```
제출자가 `www.` 나 IP 로 들어와 있으면 버튼 링크가 승인본과 달라진다. 카카오는 그걸 대조해서 거부한다.

🔑 **간헐성의 정체를 가른 실측** — 같은 버튼 코드를 쓰는 두 템플릿의 성적이 갈렸다:
| 템플릿 | 버튼 URL 생성 위치 | 성적 | K108 |
|---|---|---|---|
| `erp_payout_request` | **HTTP 요청 안**(제출 화면) | sent 9 / failed 5 | **5건 전부** |
| `erp_capital_weekly` | **cron(CLI)** | sent 12 / failed 1 | **0건** |

cron 은 요청 호스트가 없어 `APP_URL` 을 쓴다 — 그래서 한 번도 안 틀렸다. tinker(CLI) 재발송이
즉시 성공한 것도 같은 이유다. **「됐다 안 됐다」는 사람이 어느 주소로 들어왔느냐였다.**

**해결** — 만들기 **전에** 도메인을 고정하고 끝나면 되돌린다:
```php
URL::forceRootUrl(config('app.url'));
try { return URL::temporarySignedRoute(...); } finally { URL::forceRootUrl(null); }
```
🚨 **서명은 호스트까지 포함해 계산된다** — 만든 뒤 도메인만 바꿔치면 **링크가 깨진다**(403). 순서가 핵심이다.
🚫 전역 `forceRootUrl` 을 부트에서 걸지 않았다 — 앱 전체 URL 이 바뀌어 영향 범위가 너무 넓다.

**🧭 새로 만들 때 물어볼 것** — 밖으로 나가는 링크(알림톡·문자·메일·외부 API 콜백·웹훅)를
**요청 안에서** 만들고 있나? 그렇다면 서버가 한 이름으로만 서비스되는지 확인하고, 아니면 고정할 것.
⚠️ 이 부류는 **에러가 밖에서 온다** — 우리 로그엔 「등록된 버튼과 다름」까지만 뜨고 **어디가 다른지는 안 나온다.**
   그래서 승인본을 한 항목씩 대조해야 좁혀진다.

### 77. 🧵 **pint 의 `single_quote` 가 `"…\n…"` 을 실제 개행으로 펼친다** — 알림톡 본문에 CR 이 박힌다 (2026-09-07)

**원인**: 새 템플릿 본문을 큰따옴표+`\n` 으로 썼는데 `vendor/bin/pint --dirty` 가 작은따옴표로 바꾸면서
**이스케이프를 실제 개행으로 펼쳤다.** 그 시점 파일이 CRLF 라 문자열 안에 **CR 이 섞였고**,
뒤이어 `line_ending` fixer 가 **코드 줄바꿈만** LF 로 정규화해서 **문자열 안 CR 만 살아남았다**.

**증상이 고약하다** — `php -l` 통과, 테스트 전부 초록, 화면·터미널 출력도 **글자 하나 다르지 않게 보인다**.
BizM 등록본 대조에서 «기대 / 실제» 두 줄이 **눈으로 완전히 동일**한데 ❌ 만 뜬다.
실제 차이는 `bin2hex` 로 봐야 나온다(`0d0a` ↔ `0a`). 3사 전부 실패했다.

```php
// ❌ pint 가 '…' + 실제 개행으로 펼친 결과 → CRLF 혼입
'body' => '[제목]
내용',

// ✅ 큰따옴표 + \n 이스케이프 — pint 가 안 건드린다(기존 템플릿 20여 종이 전부 이 형태)
'body' => "[제목]\n내용",
```

- 🔎 **진단** = `substr_count($body, "\r\n")`. 기존 템플릿과 **나란히 세어** 내 것만 다른지 본다.
- 🧭 **규칙** — 개행이 든 문자열 상수(알림톡 본문·메일 템플릿·서류 문구)는 **항상 큰따옴표+이스케이프**.
  스크립트로 파일을 편집할 때도 백슬래시가 진짜 들어갔는지 `od -c`·`cat -A` 로 확인할 것(§8 #73 의 그 확인).
  ⚠️ **이 문서를 쓰면서 같은 실수를 두 번 했다** — 편집 스크립트의 문자열 안에서 백슬래시가 삼켜져
  개행이 들어갔다. 백슬래시를 담아야 하면 `chr(92)` 처럼 **삼켜질 수 없는 형태**로 조립할 것.
- ⚠️ **pint 를 돌린 뒤 한 번 더 검증**할 것 — 커밋 직전 포매팅이 의미를 바꿔 놓는 경우가 이 부류다.
- 가드 = `scripts/alimtalk-verify-upload.php` / `-itemlist-`(등록본 ↔ 상수 글자단위 대조).
  **테스트로는 원리상 못 잡는다** — 발송도 렌더도 정상이고, 어긋나는 상대가 **BizM 승인본**이라 레포 밖에 있다.
### 78. 📤 **출력 파일명이 고정인 생성기는 남의 제출본을 지운다** (2026-09-07)

`alimtalk-build-*-upload.php` 는 회사별로 **항상 같은 이름**(`upload_erp_{회사}_아이템리스트_신규.xlsx`)에
쓴다. 다른 템플릿을 만들려고 한 번 돌리는 순간, **BizM 심사에 제출해 둔 직전 등록본이 사라진다.**
실제로 「채권현황+일일요약」 제출본을 「말소필요」 1종으로 3사 전부 덮어썼다(jin 이 손수 떠 둔
`.bak-20260820제출본` 이 있어 복구). 스크립트는 성공 메시지만 찍고 아무 경고도 없다.

- ✅ 고침 = **덮어쓰기 전 자동 백업**(`.bak-YYYYMMDDHHMM`) + 백업 실패 시 중단. 두 빌더 모두.
- 🧭 **생성기를 돌리기 전에 그 경로에 뭐가 있는지 볼 것** — 특히 파일명이 코드·날짜를 안 담는 경우.
  같은 폴더의 `.bak-*` 이름들이 「사람이 손으로 지켜 온 것」이라는 신호다.
- 🧭 새 생성기를 만들 땐 **출력명에 무엇이 들어 있나**를 먼저 정할 것. 이름이 고정이면 백업은 필수다.
- ⚠️ 제출·승인 상태의 산출물은 레포 밖(Desktop)에 있어 **git 이 지켜 주지 않는다.**
### 79. ⚡ **「목록이 느리다」 — DB 가 아니라 HTML 크기다** (2026-09-07 차량목록 100행)

jin: *「100개씩 보기로 하면 렉이 걸린다」*. **가설을 두 번 세웠고 두 번 다 실측이 기각했다.**

| 잰 것 | 결과 |
|---|---|
| 운영 `rt=` 크기 구간별 | `~30KB 0.15s` / `30-60KB 0.33s` / **`60KB+ 1.58s·최대 45.6s`**(443건) |
| 워커 경합(NICE) | **기각** — 45초 요청과 NICE 조회는 시간대가 안 겹쳤다 |
| DB (복제 DB 4,288대) | **기각** — 20행 73ms ↔ 100행 85ms, **쿼리 30개 동일** |
| HTML 크기 | 20행 167KB → **100행 908KB** ← 여기 |

**원인** = 컬럼 토글이 localStorage 에만 있어 서버가 모른다 → 36칸을 다 그려 보내고 브라우저가
26칸을 CSS(`x-show`)로 가린다. 100행이면 **안 보이는 칸 2,600개**를 만들어 전송한다.

🚨 **고치다가 오히려 커졌다 — `@if` 를 쓰지 말 것.** 조건을 `@if` 로 감쌌더니 908 → **1,084KB**.
Livewire 가 조건마다 `<!--[if BLOCK]--><!--[if ENDBLOCK]-->` 마커를 넣는다(morph 힌트).
31열 × 2 × ~27B = **행마다 1.6KB**. 행 하나(5,171B)를 뜯어보니 **절반이 그 주석**이었다.
⇒ `@php if (조건): @endphp … @php endif; @endphp` 로 감싸면 마커가 안 생긴다.

```
결과  100행 908 → 584KB(−36%) · 행 1개 8.5 → 3.5KB(−59%) · td 36 → 10
```

- 🔑 **모르면 「그린다」로 기울일 것** — `visibleColumns === [] → 전부 렌더`. 반대로 두면 화면이
  알려 주기 전 첫 렌더에서 **컬럼이 사라진다**. `x-show` 는 그대로 둬서 서버가 그린 것은 클라이언트가 가린다.
- 🚫 **저장 키·기본값을 건드리지 말 것** — 바꾸면 전 사용자의 컬럼 설정이 초기화된다.
- 🧭 **재는 법** — 운영 nginx `rt=` 를 **응답 크기 구간별로** 집계한다(시간만 보면 뭐가 느린지 안 나온다).
  로컬은 데이터가 적어 재현이 안 되므로 **별도 DB 에 복제·증식**해서 잰다(기존 로컬 DB 는 읽기만).
  자식행도 같은 비율로 늘려야 관계 로딩 비용이 현실적이다.
- ⚠️ **성능 변경은 「전·후를 같은 방법으로」** 재고 커밋에 숫자를 남긴다. 안 그러면 이번처럼
  **더 느려진 것을 개선으로 착각**한다.
- 가드 = `VehicleListColumnRenderTest`(서버 조건 누락 · th↔td 불일치 · `@if` 재도입 · 폴백 규칙).
  **정적 검사여야 한다** — 되돌아가도 화면은 정상이고 응답만 커진다.
- 🅿️ 남은 축 = 목록 밖 **고정분 234KB**(편집 패널·모달)가 행 수와 무관하게 매번 나간다.

### 80. 🕳️ **DB 기본값은 모델 훅에 안 보인다** — 게이트가 통째로 꺼져 있었다 (2026-09-07 현금 원장)

바이어 현금 원장을 heymanerp 에 켜고 실제로 써 봤더니 **차감도 배분 기록도 없었다**(jin 제보).

**원인** = `final_payments.type` 이 `NOT NULL DEFAULT 'balance'` 인데 **모델에 기본값이 없었다.**
DB 기본값은 **INSERT 때만** 적용되므로, `type` 을 안 넘기고 만든 행의 **메모리 모델은 `type=NULL`** 이다.
`BuyerCashService::gated()` 의 `type !== 'balance'` 가 참이 되어 **문지기와 차감이 함께 꺼졌다.**

```
[fp] type=NULL   gated=false  alloc=0     ← created 훅이 보는 값
  FP 1 type=balance                        ← DB 에 저장된 값 (화면·집계는 전부 정상)
```

🔑 **운영의 두 생성 경로가 정확히 `type` 을 안 넘긴다** — 판매탭 잔금 N행(`vehicles/index` :5209)과
채권관리 「입금」 미러(`ReceivableHistory::syncFinalPayment` :154). 그게 **jin 이 지정한 차단 지점 2곳
전부**였다. 즉 기능이 배포된 날부터 **아무것도 안 하고 있었다.**

🚨 **기능 테스트로는 원리상 못 잡는다** — 스위트 32건이 전부 `'type' => 'balance'` 를 **명시**해서
잔금을 만든다. DB 에는 올바른 값이 들어가고 화면·집계·엑셀도 전부 맞다. **훅만 다른 값을 본다.**

⚠️ **「막히기는 하니까 동작한다」고 읽으면 안 된다.** 현금이 모자랄 때 예외는 실제로 떴는데,
그건 뒤늦게 **채권 미러 경로**(`FinalPayment::find()` 로 DB 에서 다시 읽어 `type='balance'` 인 모델)
에서 난 것이고 그 경로는 검사만 하고 배분은 건너뛴다. ⇒ **예외 「종류」만 단언하는 테스트는
고치기 전에도 통과한다.** 행이 안 남는지(=제자리에서 막혔는지)까지 볼 것.

**고침** = `protected $attributes = ['type' => 'balance']`. 저장되는 값은 그대로라 기존 데이터 무영향.
가드 = `BuyerCashGateTest` 4건(메모리 기본값 · 판매탭 경로 소진 · 차단 시 행 미생성 · 미러 경로).

**🧭 규칙 — 모델 훅이 읽는 컬럼에 DB 기본값을 기대지 말 것.** 쓸 거면 `$attributes` 에 같이 적는다.
새 게이트를 만들 때는 **운영 경로가 실제로 넘기는 속성만으로** 판정되는지 확인할 것
(테스트가 채워 주는 값에 기대면 안 된다 — SKILLS §8 #36 과 같은 「테스트만 통과」 부류).

🚫 **소급 배분(backfill)을 자동으로 돌리지 말 것.** heymanerp 실측: 배분이 빈 확정 잔금 14건 중
**12건이 원장 도입(09-05) 전**(6~8월)이다. FIFO 라 새 입금이 **6월 잔금으로 빨려 들어간다.**
원장은 「켠 날부터」가 설계다(전환 마이그레이션 없음) ⇒ 복구는 **그 건만 지우고 다시 입력**.
회수이력을 지우면 연결된 판매잔금도 같이 지워진다(`ReceivableHistory::deleted`).

### 81. 🚪 **예외를 만들면, 나중에 생긴 「같은 규칙의 다른 게이트」가 그걸 안 물려받는다** (2026-09-08 선적대기 항로)

`Port::allow_shipping_wait`(2026-07-18)는 «돈을 다 받기 전에 항구에 세워두고 서류를 진행하는» 항로를
**C5 진입 게이트에서 빼주려고** 만든 플래그다. 그런데 **이틀 뒤(07-20) 만든 묶음 aggregate 착수 게이트**가
그 예외를 안 물려받았다. 결과 — 개별 반입지 저장은 되는데 **묶음 착수만 막히고**, 착수불가 분기가
`@else` 를 통째로 숨겨 **수출신고번호·B/L번호 기입 버튼이 아예 안 뜬다**. 플래그가 허용하려던
「선적대기 서류작업」이 정확히 그 버튼들이었다.

실사고 = heymanerp AUTO SCOUT 8대 + R.S.H 1대(전부 DURRESS RORO · 미수 100%). **jin 은 이미 배포한
줄 알고 있었다** — 09-08 커밋(`81f1a8c`)은 board **후보 목록**만 고쳤고 착수 게이트는 범위 밖이었다.

🔎 **찾는 법** — 예외 술어를 `grep` 해서 **호출부 수를 세어 본다.** `isShippingWaitRoute` 는 2곳뿐이었고
(개별 C5 · board 후보), 정작 같은 규칙을 판정하는 곳은 4곳이었다(+ 묶음 aggregate · 개별 빨간 칩).
🧭 **새 게이트를 만들 때** — 같은 이름의 락(`shipping_entry`)을 읽는 기존 게이트를 먼저 열어
**그쪽이 보는 예외 목록을 그대로 가져올 것**(`hasEntryUnpaidOverride` 옆이 그 자리다).
⚠️ **짝이 되는 표시도 같이** — 착수는 열리는데 차량 칩이 빨간 채로 남으면 사람은 여전히 «막혔다»고 읽는다.
🚫 **경계는 그대로** — B/L(G1 100% 완납)은 이 예외를 안 본다. 화물인도권이라 그것만이 물건이 넘어가는 단계다.
가드 = `ShippingBundleGateTest` 4건(전량 wait-route 착수 · 대조군 차단 · 섞인 묶음은 일반 항로만 집계 ·
B/L 은 여전히 차단). **일부러 예외를 빼고 돌려 2건이 빨갛게 되는지 확인**했다(§8 #73).

💬 **곁다리 — 화면 문구에 임계를 박아 쓰지 말 것.** 「50% 미만입니다」가 lang 에 리터럴로 있었는데
실제 임계는 전역 60%·**바이어별 가변**이다. `entryAggregate` 가 `required_pct` 를 함께 돌려주게 해
`:req` 로 바꿨다. 숫자를 문구에 박으면 **설정을 바꿔도 화면만 옛말을 한다**.

### 82. 🗣️ **LLM 기능은 「코드가 맞나」가 아니라 「실제로 뭐라고 답하나」로 검증해야 한다** (2026-09-09 챗봇)

하루에 같은 뿌리로 세 번 고쳤다. 세 번 다 **테스트는 초록이고 코드도 의도대로 돌았는데 답이 나빴다.**

**① 낱말 목록으로 의도를 분류하면 실사용에서 샌다.** 「미수」가 들어갔다고 DB 금액 조회로 보내면
「선적대기 항로인데 미수여도 착수돼?」가 채권 KPI 표를 받는다. 의문사를 덧붙여 고쳤더니 다음 날
「…경우가 어떤게 있어?」가 또 샜다. **결정적 사례는 `채권담당자`** — 역할 이름이 「채권」+「담당자」로
쪼개져 담당자별 미수 조회로 가고 **권한 거부**까지 났다(사용자는 용어 뜻을 물었을 뿐이다).
질문에 물음의 흔적이 아예 없으니 **의문사 목록으로는 원리상 못 잡는다.**
⇒ 🔑 **화면·기능·역할 이름을 신호로 쓴다** — 금액을 묻는 사람은 화면 이름을 부르지 않고 금액과 사람을 부른다.
⇒ 🔑 **띄어쓰기를 지우고 비교**한다(실사용에 「채권담당자」와 「채권 담당자」가 함께 나온다).
⇒ 🧭 **낱말을 더 넣기 전에 운영 로그를 읽을 것** — `AuditLog action='assistant_query'` 에 질문 원문이 남는다.
   내가 상상한 예문에는 오분류 4건의 형태가 **없었다**. 테스트 문장은 로그 실물로 쓴다.
   ⚠️ 단 **개발자 테스트 질문이 섞여 있다** — 13회 반복된 질문을 실사용 사고로 오판했다가 정정받았다.

**② 프롬프트 지시는 반대쪽으로 넘친다.** 필수 사실 누락을 고치려고 「참고자료에 있는 만큼 **빠뜨리지 말고**
적는다」를 넣었더니 한 질문에 **45줄**이 왔다(관련 없는 화면·규칙까지 줄줄이). 게다가 모델이 시키지도 않은
4번째 섹션을 스스로 만들어 붙였다.
⇒ **「필요한 것만 쓴다」를 첫 규칙으로**, 부분별 줄 수 상한, 섹션 개수 고정(제목 신설 금지).
⇒ 🧭 **짧아진 것만으로 개선이라 하지 말 것** — 필수 사실 커버를 **함께** 재야 한다. 한쪽만 보면 반대쪽이 무너진다.

**③ 프롬프트가 마크다운을 쓰게 하면 화면에 글자로 보인다.** 위젯이 `{{ $m['text'] }}` +
`whitespace-pre-wrap` 이라 **평문을 그대로** 뿌린다(마크다운 렌더링 0). `**굵게**` 가 그대로 노출됐고
**사람이 눈으로 잡았다** — 테스트로는 안 잡힌다. ⇒ 렌더 경로를 먼저 확인하고 프롬프트에 서식을 금지한다.

**📏 그리고 근거를 「몇 장」으로 조절하면 안 된다.** 청크 길이가 10배 달라 `topk=4` 인데 문맥이
3,315자~**9,023자**로 갈린다. 모델 상주 컨텍스트(`/api/ps` 실측 4096)를 넘기면 **조용히 앞부터 잘려
점수 1위 근거가 사라진다** — 에러도 로그도 없다. ⇒ **문자 예산**으로 자르고, 예산 초과 청크는 건너뛰고
뒤의 작은 근거에 자리를 준다. 예산을 늘리려면 `num_ctx` 를 올려야 하고 그건 GPU VRAM 을 더 먹는다.

**🔒 검증 도구를 먼저 만든다** — `assistant:eval`(문항은 repo 의 채점표 JSON, 코드에 안 적는다).
검색 순위는 **LLM 없이 임베딩만**으로 재고(같은 입력에 같은 결과), 답변 품질은 **전후를 나란히 놓고 사람이
읽는다**(🚫 자동 채점 금지 — 8B 출력은 실행마다 흔들려 점수가 튄다).
⚠️ **후보 추림·채점·중복 제거·프롬프트를 평가 쪽에 복제하지 말 것**(§8 #44) — 갈리면 **평가가 거짓말하는
계기판**이 되고 그게 원래 버그보다 고치기 어렵다. 정적 가드로 막아 뒀다.

**🧭 마지막으로 — 배포했다고 반영된 게 아니다.** `config` 기본값을 올렸는데 3사 `.env` 가 옛 값을 못 박고
있어 무효였다(메모리엔 「오버라이드 없음」이라고 적혀 있었다). **배포 후 운영에서 `config(...)` 를 실측**하지
않았으면 「반영됐다」고 오보고했을 것이다.

### 83. 🚪 **제외를 `!==` 한 줄로 쓰면 「이유가 다른 것」이 조용히 휩쓸린다** (2026-09-09 송금수수료)

바이어 현금 게이트의 대상 판정이 `type !== 'balance'` **한 줄**이라 계약금·중도금·선수금1·수수료를
한꺼번에 제외했다. 기획서 제외 표에 적힌 근거는 **계약금 것뿐**이었는데(「수출 전 선수금이라 성격이 다르다」)
수수료도 그 근거로 걸러진 것처럼 보였다. 실제로는 **논의된 적이 없다.**

수수료(셀러부담)는 **정의상 바이어가 보낸 돈의 일부**다 — 입금은 「보낸 돈 10,000」으로 적으므로 은행이 먹은
6 은 이미 그 입금 안에 있다. 미수만 6 줄이고 현금을 안 빼니 원장에 차액이 남았고, 그 잔돈은 **다음 잔금에
조용히 쓰여** 바이어에게 그만큼 더 크레딧이 됐다. **게이트가 정확히 수수료 금액만큼 뚫린다.**

🚨 **사람은 그걸 「불편」으로 겪는다.** jin 은 원장 수수료에 같은 금액을 손으로 한 번 더 적어 우회했고,
**이틀에 4건 중 1건을 놓쳤다**(heymanerp 실측: 판매탭 수수료 24 EUR 중 원장 반영 18 → 원장이 남은 현금을
2,280 으로 표시하는데 실제 뒷받침은 2,274). ⇒ **우회가 습관이 되면 제보가 사고로 오지 않고 불만으로 온다** —
「양쪽에 적어야 하네」 같은 말을 들으면 **두 숫자가 어긋난 게 아닌지** 먼저 계산해 볼 것.

**🧭 그래서 이렇게 한다.**
- **제외는 화이트리스트 + 유형별 이유 표**로 쓴다(`BuyerCashService::GATED_TYPES`). 그리고 **그 목록을
  못박는 테스트**를 둔다 — 늘리려면 테스트를 고쳐야 하므로 「휩쓸려 들어가는」 일이 없다.
- ⚠️ **「지우고 다시 만드는」 sync 는 「정정은 통과」 예외를 무력화한다.** 판매탭 4항목은 매 저장마다
  delete+create 라 **감액도 항상 신규 행**이 되고, `$payment->exists` 를 보는 예외가 발동하지 않는다 →
  옛 값을 **줄이는데도** 전액을 요구해 죽는다(§8 #66 의 그 형태, 다른 문). 판정은 **삭제 전에** 배분 유무로.
- 🚫 **소급 배분(backfill)은 하지 않는다** — FIFO 라 새 입금이 옛 행으로 빨려 들어가고, 사람이 손으로 넣은
  **우회 기록과 이중으로 겹친다**(#80 과 같은 이유). 「지금까지 해놓은 건 그대로」가 기본값이다.
- 🔁 **우회 통로를 남길 땐 이중 입력을 화면이 막아야 한다.** 원장 수수료 폼에 「차량에 붙는 건 판매탭에서」,
  원장 배분 줄에 「송금수수료」 뱃지. 뱃지가 없으면 6 EUR 가 **「아주 작은 잔금」으로 보여 또 턴다**.
- 🚨 **그 뱃지는 부분 select 로 죽는다** — `with('allocations.finalPayment:id,payment_date')` 에 `type` 이
  없으면 판정이 **늘 false** 다(예외 0 · 화면은 정상 렌더). 가드는 **화면을 렌더해 뱃지 문자열을 세는** 것이다.

가드 = `BuyerCashGateTest`(대상 목록 못박기 · 원장 0 되는 실사례 · **운영 경로**(판매탭) · 「그대로」 3갈래)
· `BuyerAccountScreenTest::test_a_wire_fee_allocation_is_marked_on_the_screen`.
**둘 다 일부러 깨뜨려 빨개지는지 확인했다**(§8 #73) — 대상 목록 되돌리면 8건, 「원장 밖」 판정 없애면 2건,
eager load 에서 `type` 빼면 1건.

### 84. ⚖️ **자산만 빼면 부채가 남는다** — 그리고 같은 개념이 **두 함수**로 존재한다 (2026-09-09 매입취소)

매입취소 차량을 「우리 것이 아니니」 재고에서 빼려다 두 번 걸렸다.

**① 「재고」가 두 함수다.** 화면의 재고(`Vehicle::scopeInStock` — **출고일** 기준, 「어디 있나」)와
자금현황의 재고(`CapitalStatusService::inventoryKrw` — **선적일** 기준, 「아직 우리 것인가」)는
**일부러 다르게** 만들어져 있다(그 파일 주석에 그렇게 적혀 있다). 한쪽만 고치면 화면과 대표 지표가 갈린다.
⇒ 🧭 「재고에서 빼 달라」는 요청을 받으면 **재고를 계산하는 함수를 전부 grep** 할 것. 이름이 같아도 축이 다르다.

**② 자산만 빼면 청산가치가 크게 틀어진다.** `inventoryKrw` 에서만 빼면 `payableKrw` 가 **그 차의
매입 미지급을 계속 부채로** 세서 「갚지 않아도 되는 부채」만 남는다. 실측(heymanerp): 매입취소 4대의
재고 6,588만 ↔ 미지급 6,470만 이 **거의 상쇄**해 순영향이 **+25만**뿐이었는데, 자산만 뺐다면
청산가치가 **−6,495만** 움직였다. 그 파일에 이미 경고가 있었다 — 「완납 여부 무관 … 미지급을 별도로
빼므로 완납만 세면 자산-부채 비대칭」.
⇒ 🧭 **자산·부채는 한 커밋에서 함께** 넣고 빼고, 가드는 **둘을 같은 테스트에서** 단언한다
   (`CapitalStatusTest::test_cancelled_purchase_leaves_both_the_inventory_and_the_payable`).
   미지급 제외만 되돌려 보면 그 테스트 1건이 빨개진다 — 실제로 확인했다.

**③ 「할일 큐」와 「평가」는 축이 다르다.** jin 은 매입취소 차의 **매입 미지급을 재무 큐에는 남기라**고
했고(확인해야 할 돈), **자금 계산에서는 빼는 게** 맞다(갚을 돈이 아니다). 같은 숫자가 화면에 따라
다르게 취급되는 게 정상인 경우다 — 「한쪽에 있으니 다른 쪽에도」로 통일하지 말 것.

**곁다리 — 큐 제외는 `scopeAction` 한 곳에서.** 대시보드 카운트·목록·알림톡이 같은 출처를 봐서
세 숫자가 함께 맞는다(§8 #44). 알람은 `ScanTaskAlarms` 가 같은 scope 로 **자동해소**하므로
기존 알람을 손댈 필요가 없다. 목록에서 하나만 빼 보면 3건이 빨개진다(확인했다).

### 85. 🕰️ **「기록」과 「확정」이 갈리는 화면은 그 차이를 보여줘야 한다** (2026-09-09 채권관리 입금)

jin: *「과입금 만들려고 입금으로 넣었는데 안 잡히고 현금만 잡히네?」* — 데이터 버그가 아니었다.

`입금`만 **판매잔금(FinalPayment)을 미확정으로 만들고**, 미수는 **확정된 잔금만** 센다 ⇒ 재무 확정
전까지 미수가 안 움직인다. 나머지(현금·상계·기타·손실)는 회수이력 행이 **곧바로** 미수를 깎는다.
그런데 회수이력 목록이 `confirmed_at` 을 **한 번도 안 읽어서**(실측 0곳) 두 행이 똑같이 보였다.

- 🧭 **한 화면에 「즉시 반영」과 「승인 후 반영」이 섞이면 반드시 표시**할 것(§8 #60 의 그 부류).
  고친 것 = 미확정 입금 행에 「재무 확정 대기」 뱃지 + **방식을 고르는 순간** 안내(저장 뒤에 알면 늦다).
- 🔎 **운영에서 안 드러난 이유** = 미러 잔금 **229건 전부 확정 · 미확정 0건**. 재무가 늘 곧바로 확정해서
  Draft 구간이 남은 적이 없었다. ⇒ **「아무도 안 겪었다」가 「문제가 없다」는 아니다.**
- 🚫 회계 흐름은 안 건드렸다(jin: 표시만) — 즉시 확정으로 바꾸면 「기록↔확정」 이중 확인이 사라진다.

**🚨 같이 발견한 이중 크레딧 — 감액은 현금을 되돌린다.**
과입금을 적립금으로 돌리려면 확정 잔금을 **감액**하는데, 그러면 `FinalPayment::updated` →
`reallocateIfTracked` 가 배분을 줄여 **현금이 바이어 지갑으로 되돌아온다.** 거기에 적립금까지 주면
과입금 30 에 크레딧이 **60** 이 된다(재현 실측). 운영 피해 0 — 전환 16건이 **전부 원장 밖 잔금**이라
아직 안 터졌을 뿐이었다.
- 고침 = 두 경로 공통으로 되돌아온 몫을 원장에서 뺀다(`BuyerCashFee` kind=`overpay`).
- 🧭 되돌아온 금액을 **배분 계산을 옮겨 적어** 구하지 않고 **잔액 차이**로 구한다 — 배분 규칙이 바뀌어도
  따라온다(§8 #45 의 「공식을 복제하지 말 것」의 적용형).
- ⚠️ **미수 계산 제외 목록**(`MIRRORED_RECEIVABLE_METHODS`)에 새 항목(`misc_loss`)을 안 넣으면
  0 으로 만든 미수가 **다시 음수로 돌아간다** — 그 행이 또 빼기 때문이다.
- 🔎 진단은 **로그가 가장 빨랐다** — 클로저 `use` 에 변수를 안 넘겨 조용히 롤백되던 것을
  `laravel.log` 의 `Undefined variable $isForeign` 한 줄이 즉시 알려줬다(§8 #36 의 그 습관).

### 86. 🕰️ **재배분이 과거 기록을 다시 쓴다 — 날짜를 보여주면 「시간이 거꾸로」 간다** (2026-09-10 현금 배분)

jin: *「기록되는 날짜나 수정사항이 반영되는 게 좀 이상하게 보인다. +,- 되는 건 맞는 것 같고.」*
**금액은 전부 맞았다**(운영 검산 일치 — 입금별 Σ배분이 한 건도 안 어긋났다). 틀린 건 **언제 것인지**였다.

`BuyerCashService::allocate()` 는 잔금이 바뀔 때마다 그 잔금의 배분을 **지우고 FIFO 로 다시 깐다**.
다른 잔금들은 자리를 그대로 지키므로, 먼저 받은 돈을 다른 차가 차지하면 그 잔금은 **나중 입금으로 밀린다**.

```
09-10 입금(3,700) → 368머4746 ... 그 잔금의 수금일은 09-09   ← 09-10 돈이 09-09 에 쓰인 것으로 보인다
```
실측 heymanerp: 배분 id 1~80 중 **생존 23개**(57개가 지워지고 다시 깔렸다). 합계는 늘 맞고 **이력만 바뀐다.**

- 🚫 **`created_at` 을 배분 줄의 날짜로 쓰지 말 것** — 그 행은 재배분마다 새로 생기므로 그 값은
  「FIFO 가 마지막으로 돌아간 시각」이다(실측: 09-09 잔금의 행이 09-10 에 생성). 날짜는 **잔금의
  `payment_date`**(원장 수수료는 `charged_date`)다. 단일 출처 = `BuyerCashAllocation::usedDate()`.
- 🔁 **역행 줄을 표시한다**(`isBackfillFor`) — 「나중 입금」 뱃지 + 호버 설명. 표시가 없으면 사람이
  **금액 오류로 의심**해 멀쩡한 기록을 또 손본다. 판정은 모델 단일 출처로 두고 **3 화면이 같이 쓴다**
  (바이어탭 현금 · 바이어 정산현황 · 차량 판매탭). 둘만 고치면 나머지 한 곳에서 같은 혼란이 재현된다.
- 🏷️ **차량이 없는 배분 줄엔 이름을 줄 것** — 원장 수수료는 차량이 null 이라 `- 6.00` 으로만 찍혀
  「아주 작은 잔금」처럼 보인다(실측 R.S.H 3줄). 사람이 그걸 **또 턴다**.
- 🗓️ **「어느 입금에서」를 날짜로만 쓰지 말 것** — 같은 날 입금이 여럿이면 전부 똑같이 보인다
  (실측 R.S.H 09-09 에 입금 3건, 수수료 3건이 모두 한 건에서 나갔는데 `09/09 6.00` 세 줄). 금액을 함께 적는다.
- 🧭 **제보가 「금액이 틀리다」가 아니면 먼저 검산으로 금액을 털어낼 것.** 이번에 검산이 전부 맞아서
  수색 범위가 «표시·추적성»으로 좁혀졌다. 안 했으면 소수점·스냅·FIFO 를 전부 고치려 들었을 것이다.
- 🔒 가드 = `BuyerCashAllocationDateTest`(날짜 출처 · 3 화면 렌더 · 역행 뱃지 1줄 · 라벨 · 출처 구분 ·
  **`created_at` 금지 정적 검사**). ⚠️ **기능 테스트로는 원리상 못 잡는다** — 배분도 금액도 정상이고
  화면도 정상 렌더된다. 그래서 **렌더 결과에서 날짜 문자열과 뱃지를 센다**.
  ⚠️ 호버 전문(title 속성)에도 같은 낱말이 들어가므로 문자열만 세면 2 가 나온다 — **태그 안**(`>…<`)만 셀 것.

### 87. 🚢 **소급 적재는 「그 화면이 무엇을 세는가」를 통째로 바꾼다** (2026-09-10 운항 pill · 미수 0)

ssancarerp 소급 적재(3,839대, 2026-08-28)가 하루아침에 두 화면의 뜻을 바꿔 놨다. 둘 다 **코드는 그대로**고
**예외도 로그도 없다** — 숫자만 조용히 의미를 잃는다.

| 화면 | 무슨 일 | 실측 |
|---|---|---|
| 차량관리 운항 pill | 적재분이 선적일·ETA 를 다 들고 와 「도착예정」이 됨 | **3,883 / 4,793 = 81%** |
| 채권관리 | 미납을 회수이력 「기타」로 적어 미수를 0 으로 눕힘 | **317건** (45대는 금액이 운임비와 일치) |

**🔑 원인은 「일부러 안 보기로 한 조건」이다.** `scopeSailing` 주석에 «진행상태로 대상을 좁히지 않는다»
(2026-08-09)라고 적혀 있었고, 그때 heymanerp 300대 기준으로는 옳았다. 데이터가 4,700대로 바뀌자 전제가 무너졌다.
⇒ **설계 결정에 「그때 몇 대였나」를 같이 적어 둘 것.** 그래야 뒤집을 때 근거를 놓고 비교할 수 있다.

**🚫 좁힐 때 공유 축을 건드리지 말 것** — `scopeSailing`·`sailing_status` 는 ssancar.com 포털과 board 가
그대로 받아 화면을 그리고, `SailingStatusTest::test_scope_and_accessor_agree` 가 scope↔accessor 일치를 박아둔다.
⇒ **차량관리 목록·pill 카운트·그 목록의 엑셀만** 덧붙인다(`Vehicle::scopeExcludeClosedDeals`, 「매입중」과 같은 형태).
엑셀을 빼먹으면 **화면 580 · 엑셀 3,883** 이 되는데 눈으로는 못 잡는다.

**⏳ 제외 기준은 「끝났나」로 잡으면 저절로 줄어든다** — 「거래완료 + 2차 정산 마감」. 2차가 닫히는 순간 빠지므로
고정 목록이 아니라 **「아직 안 끝난 배」만 남는다**(적용 후 580, 그중 337 이 2차 미마감).
⚠️ 조건을 「2차 마감」 하나로 줄이지 말 것 — 지금은 결과가 같지만(실측 3,303 동일), 거래완료가 아닌 차에
마감된 정산이 생기면 그건 **아직 배 위에 있는 차**다.

**🧾 「미수를 0 으로 눕힌 기록」은 되살리지 말고 표시할 것** (jin 2026-09-10 결정).
되살리면 v5 규칙(「출고일 + 완납」)이 깨져 실측 **77대가 거래완료 → 판매중으로 추락**하고, 2차 마감된 228건은
`FinalPayment::creating` 가드에 막혀 **받은 돈을 넣을 수조차 없다**. ⇒ 미수는 0 그대로 두고 채권관리
「임포트 정리분」 탭 + 회수이력 줄 뱃지로 본다. 받으면 그때 그 행을 지우고 진짜 입금을 넣는다.
- 🔑 **`method='other'` 만으로 고르지 말 것** — 기타는 **사람이 실제 회수에도 쓰는** 방법이라
  진짜 받은 돈까지 「안 받은 돈」으로 뜬다. **오탐이 나는 목록은 곧 무시당한다**(§8 #39 의 그 교훈).
  단일 출처 = `ReceivableHistory::scopeImportCleared`(표식 = 적재기가 남긴 note 접두사).
- 🚨 **표식을 지우는 명령이 이미 있었다** — `ssancarerp:convert-import-receivables` 는 그 행을
  **확정 판매 잔금**으로 바꾼다(= 받은 것으로 기록). 돌리면 무엇을 못 받았는지 **다시 찾을 방법이 없다.**
  미실행 상태였고 docblock 에 경고를 박았다. ⇒ **일회용 적재 명령을 만들 땐 「이걸 나중에 돌리면 무엇이
  사라지나」를 같이 적을 것.**
- 탭은 **대상 건수로 노출**한다(§8 #67) — 다른 두 회사엔 안 뜨고, 다 정리하면 저절로 사라진다.
- ⚠️ 미수 컬럼이 전부 0 인 목록이라 **왜 여기 있는지 화면이 말해야 한다**(§8 #60·#85).
  안 적으면 「왜 여기 있지」가 되어 사람이 멀쩡한 기록을 손본다.
- 가드 = `SailingClosedDealTest`(7) · `ReceivableImportClearedTabTest`(7). 둘 다 **일부러 깨뜨려** 확인했다 —
  제외를 빼면 3건, 표식 조건을 빼면 1건이 빨개진다.

### 88. 🔇 **HTTP 200 인데 내용이 사라졌다** — 스크래핑은 「죽었다」고 말해 주지 않는다 (2026-09-11 환율)

jin: *「업무대시보드에서 환율 갱신이 안돼」*. 네이버가 marketindex 를 **Next.js 로 개편**해
상세페이지 HTML 에 **환율 값 자체가 없어졌다**(클라이언트 렌더). 셀렉터(`th_ex5`·`tbl_exchange`)가
통째로 사라졌는데 **응답은 계속 200 · 125KB** 라 `fetchHtml()` 이 정상 통과하고,
파서만 매칭 실패로 `null` 을 돌려줬다. 예외 0 · 로그 0 · 화면은 「—」.

**🔑 한 줄이 죽었는데 네 곳이 같이 죽었다** — 이게 진짜 피해다:

| 소비자 | 증상 |
|---|---|
| 업무 대시보드 환율 위젯 | 갱신 눌러도 무반응 ← **사람이 본 유일한 증상** |
| 판매탭 **잔금N+ 환율 자동기입** | 조용히 수기 입력으로 떨어짐 |
| **마감환율 일일 스냅샷** | `daily_exchange_rates` 가 **09-09 이후 안 쌓임** |
| board `/rates` | 죽음 |

🚨 **테스트는 내내 초록이었다.** 옛 HTML 을 fake 해서 **없어진 코드 경로를 검증**하고 있었다 —
fake 는 원리상 **포맷 변경을 못 잡는다**(§8 #36 의 「테스트만 통과, 운영만 죽음」과 같은 뿌리,
이쪽은 DB 가 아니라 **남의 HTML** 이 계약이다).

**🧭 그래서 이렇게 한다.**
- **확인은 「설정에 뭐라고 적혀 있나」가 아니라 「그 응답에 뭐가 오나」로**(§8 #74 의 nginx `gzip on` 과 같은 형태).
  진단 순서 = ①HTTP 상태·본문 길이 ②**셀렉터가 본문에 있기는 한가** ③파싱 결과. ②가 이 함정 자리다.
- **남의 HTML 을 긁고 있으면 JSON API 를 먼저 찾을 것.** 네이버도 있었다 —
  `m.stock.naver.com/front-api/marketIndex/prices?category=exchange&reutersCode=FX_{CUR}KRW&page=1`.
  🚫 **옆 필드를 잡지 말 것** — 한 행에 `receiveValue`(송금받을때) · `sendValue`(보낼때) ·
  `cashBuyValue` · `closePrice`(매매기준율)가 **전부 그럴듯한 환율**로 함께 온다
  (실측 USD recv 1,335.3 vs send 1,361.7). fake 픽스처의 옆 필드에 **딴 값**을 박아 오선택을 드러나게 했다.
- **「실패하면 조용히 null」 설계는 그 자체로는 옳다**(수기 fallback 이 있어야 하니까). 문제는
  **아무도 그 null 을 세지 않는 것**이다. ⇒ 이런 자리엔 **「마지막 성공이 언제였나」** 축이 필요하다.
  에러 알림으로는 원리상 안 잡힌다 — **에러가 안 났기 때문**이다.
  이 건은 `daily_exchange_rates` 가 이미 날짜별로 쌓이고 있어서 **「N일째 갱신 없음」 감시가 공짜**였는데
  아무도 안 보고 있었다.
- ⚠️ **소비자를 먼저 세어 볼 것** — 「대시보드 위젯이 안 된다」로 접수됐지만 실제론 4곳이었다.
  단일 출처를 만들면 고치기는 한 곳이지만 **죽을 때도 한 번에 넷이 죽는다**(§8 #38 의 뒷면).

고침 = master `ad62a69`(3사). 실측 복구 USD 1334.6 / JPY 864.09 / EUR 1549.09 / GBP 1802 / CNY 198.79.


### 89. ⏳ **승인 전에 `tmplId` 를 미리 넣어두면 그 기간 발송이 전부 K104 로 실패한다** (2026-09-11)

BizM **심사 중**인데 기능설정에 `tmplId` 를 먼저 채우면 `canSend()` 가 켜져 cron·버튼이 실제로 쏘고,
BizM 은 아직 그 템플릿이 없으니 `K104:TemplateNotFound` 로 전부 거부한다.
실측 — 말소 재촉(`erp_deregistration_reminder`)을 09-07 에 업로드하고 tmplId 를 미리 넣어 두어
**09-08 에 10건 실패**(heymanerp 3 · ssancarerp 7). 승인 뒤에는 정상 발송 중이다.

- 🧭 **심사 중에는 그 칸을 비워 둘 것.** 비면 버튼이 안 뜨고 cron 이 skip 한다 —
  「승인과 배포를 분리」(§8 #54-B)가 그 목적이다. 미리 채우면 그 장치가 무력화된다.
- ⚠️ **실패 기록은 승인 뒤에도 남는다** — `AlimtalkLog::needsAttention()`(미확인 failed/undelivered)에
  계속 잡혀 아침 점검(§8 #88 후속)이 매일 빨개진다. **확인 처리**해야 사라진다.

🚫 **오진 주의 — 「tmplId 가 템플릿 코드와 같다」는 정상이다.**
   BizM 템플릿코드는 **우리가 정해서 등록**하는 값이고(`alimtalk-build-upload.php` 가 B6 에 우리 코드를
   그대로 쓴다), 운영 실측 **21개 전부** `tmplId == code` 이며 대부분 정상 발송된다.
   2026-09-11 에 이걸 「잘못 입력」으로 읽고 jin 에게 틀린 지적을 했다 — jin 이 «발송되고 있는데?» 로 잡았다.
   ⇒ **K104 를 보면 입력값을 의심하기 전에 「그 시점에 승인이 나 있었나」를 먼저 볼 것.**

### 90. ⏱️ **`ScheduledTaskFinished` 는 exit code 검사 「전」에 발행된다** (2026-09-11)

벤더 실측(`ScheduleRunCommand::runEvent`):
```php
$event->run($this->laravel);
$this->dispatcher->dispatch(new ScheduledTaskFinished($event, …));   // ← 여기서 이미 발행
if ($event->exitCode != 0 && ! $event->runInBackground) { throw … }   // 그 뒤에 판정
} catch (Throwable $e) { $this->dispatcher->dispatch(new ScheduledTaskFailed($event, $e)); }
```
⇒ **실패한 잡도 Finished 를 먼저 낸다.** 이걸 「성공」으로 읽으면 실패할 때마다
🟢복구 + 🔴실패 **두 통**이 나가고 **매번 거짓 복구 알림**이 된다.
반드시 `$event->task->exitCode === 0` 을 직접 확인할 것.

**🧭 곁다리 — 감시·알림을 만들 때 함께 지킬 것 3가지** (2026-09-11 텔레그램 3단계에서 확립):
1. **「터졌나」와 「며칠째 안 도나」는 축이 다르다.** 예외·로그 기반 감시로는 조용한 정체를 원리상 못 잡는다
   (#88 이 그 사고). 후자는 **「마지막 성공이 언제였나」**를 따로 봐야 한다.
2. **상태 전이에만 보낸다.** 이상→이상은 침묵. 2주 멈춘 게 2주 내내 울리면 사흘째부터 아무도 안 읽는다.
   계속 걸려 있는 것은 정기 요약이 대신 말한다.
3. **「기록 없음」은 이상이 아니다.** 갓 붙인 회사·아직 한 번도 안 돈 항목을 X 로 찍으면 매일 빨개지고,
   **오탐이 나는 알림은 곧 무시당해 없는 것보다 나빠진다**(있다고 믿게 되므로 — #39 의 그 교훈).
   임계값·항목 on/off 는 **배포 없이 고칠 수 있는 자리**(설정)에 둔다.

### 91. 💱 **표시용으로 깎은 숫자가 「편집칸」에 들어가면 다음 저장이 원본을 덮는다** (2026-09-11 수수료 6.46)

jin: *「판매탭에서 수수료 6.46 기입했는데 6.00 으로 절삭돼서 들어간것 같음」*.
**저장 경로는 멀쩡했다**(`(float)`). DB 도 `decimal(15,2)` 다. 같은 차의 판매 잔금은
`10434.54` 가 소수째 살아 있었다. 잘린 곳은 **불러오는 한 줄**이었다:

```php
return $sum > 0 ? (string) (int) $sum : '';     // 6.46 → '6'
```

🔑 **이게 「표시 반올림」이었다면 무해했다.** 문제는 그 문자열이 **`wire:model` 입력칸의 값**이라는 것 —
다음 저장에서 화면 값이 정답 취급을 받아 **DB 를 덮는다**.

```
1회차 저장  '6.46' → DB 6.46          ← 정상
재로드      (int) 6.46 = '6'          ← 칸이 '6' 으로 바뀐다
2회차 저장  |6 − 6.46| ≥ 0.01 → delete + create 6.00
```

**🚨 파급이 셋이었다 — 전부 조용하다(예외 0 · 로그 0 · 테스트 초록).**

1. **수수료를 안 건드려도 깎인다.** `save()` 는 매 저장마다 4항목 sync 를 **전부** 재평가한다 →
   통관탭만 고쳐도 덮인다(§8 #65 의 그 형태). 덤으로 `payment_date` 가 `today()` 로 리셋된다.
2. **읽기 전용 항목까지 재작성됐다.** 계약금·중도금·선수금1 은 2026-07-06 에 신규입력이 중단돼
   *"props 불변이라 저장 시 no-op"* 이라고 주석에 적혀 있었는데, **그 전제를 이 캐스팅이 깨고 있었다.**
   실측 ssancarerp **계약금 4,271건**(전부 외화) 중 **125건**이 아직 소수를 들고 있었다 = 아직 안 깎인 것.
3. **회계 락이 두 겹으로 뚫린다.** 이 sync 의 삭제는 **bulk delete** 라 `FinalPayment::deleting` 의
   2차 마감 가드가 **발화조차 안 하고**, 거기에 `$allowConfirmedMutation` 로 확정 잠금까지 연다.
   ⇒ 마감된 차량의 계약금이 **감사기록도 없이** 다시 쓰인다. 08-28 소급 적재분이 정확히 그 상태였다.
4. **1 미만은 행째 사라진다.** `0.50` → 칸 `'0'` → 삭제되고 `$newAmount > 0` 이 거짓이라 **재생성 안 됨.**

**🧭 그래서 이렇게 한다.**
- **입력칸에 들어갈 값은 저장값과 같은 정밀도로** 만든다. 잔금 N행(`(string) $p->amount`)이 이미 옳았다 —
  **같은 화면 안에서 한쪽만 달랐다**(정책이 아니라 불일치다. 실제로 「절삭하기로 했다」는 결정은
  전량 grep 해도 없었고, 가장 가까운 기록은 «채권관리 **표시 전용** 0자리 통일»이었다).
  뒤 0 은 떨어뜨릴 것 — `6.00` 으로 보이면 사람이 또 손댄다.
- **가드는 「이번 저장이 만든 변화인가」로**(§8 #65 ①). 불러온 값을 `#[Locked]` 스냅샷으로 들고 있다가
  폼 값이 같으면 sync 를 통째로 건너뛴다. 이게 없으면 **밖에서 지운 행이 폼의 옛 값으로 되살아난다**.
  ⚠️ **되살아나는 경로는 「저장하고 계속」뿐이다** — 일반 `save()` 는 끝에 `close()` 라 폼이 비워진다.
  (실사고 23로0319 의 정확한 순서는 재구성 불가다. id 공백은 「삭제가 있었고 재생성이 있었다」까지만
  말해 준다 — **추정을 확정처럼 적지 말 것.** 확정된 것은 절삭 메커니즘과 DB 실측값뿐이다.)
- **bulk delete 로 잠긴 행을 지우지 말 것.** 모델 이벤트가 안 떠서 가드가 통째로 없는 것과 같다.
- 🚫 **이미 깎인 건을 소급 보정하지 말 것** — FIFO 현금 배분이 다시 깔려 §8 #80·#83 이 재현된다. 원본은 적재 엑셀에 있다.

#### 91-D. 🔢 **`number_format($x)` 는 버리는 게 아니라 「반올림」한다** (같은 날, jin 재제보)

절삭을 고치고 배포했는데 jin 이 다시 제보했다 — *「판매탭에서 수수료 6.46 넣는건 되는데
잔금N행에 표시되는게 6으로만」*. **DB 도 입력칸도 6.46 이었다.** 틀린 건 **표시**였다.

잔금 행은 회수이력 미러가 붙는 순간 **잠기고**(= 사실상 전부), 그때 입력칸 대신 **텍스트**로
렌더된다 — `number_format($lockedAmt)` 였다. 사람이 실제로 보는 건 이쪽이라
**고친 게 안 고쳐진 것처럼 보인다.** 같은 자리가 셋 더 있었다:
판매탭 위쪽 요약(총판매가·입금·미수, 실측 12대) · 이체로 생성된 잔금 행.

🚨 **반올림이라 더 나쁘다** — 10,434.54 가 **10,435** 로 보였다. 잘린 거라면 「소수가 안 보이네」로
읽히지만, 반올림은 **없는 금액**을 보여준다. 바이어 송금액과 대조하면 어긋난다.

🧭 **저장 정밀도를 올렸으면 「그 값을 그리는 곳」을 전부 훑을 것.** 입력칸만 고치면 절반이다.
잠긴/읽기전용 렌더는 **다른 코드 경로**라 같이 안 따라온다. 소수가 없을 땐 정수로만 찍어
기존 화면을 안 흔든다(`fmod($x, 1) == 0.0 ? 0 : 2`).

---

#### 91-E. 🪞 **미러는 「만들 때」가 아니라 「지울 때」 어긋난다** (같은 날, jin 결정 = B안)

채권관리 회수이력 ↔ 판매잔금은 **양방향 미러**로 설계돼 있었다 —
채권관리에서 지우면 `ReceivableHistory::deleted` 가 판매잔금까지 데려가고,
판매탭에서는 **잠긴 잔금 행을 아예 못 지운다**(`array_diff($toDeleteIds, $lockedFinalIds)`).
즉 «채권관리가 원천» 규칙이 양쪽에 박혀 있었다.

**딱 한 곳, 4항목 sync 만 그 밖이었다.** 잠금을 우회해 **bulk delete** 를 하니
모델 이벤트가 안 뜨고, FK 가 `nullOnDelete` 라 **잔금만 사라지고 회수이력이 짝 없이 남았다.**
채권관리에 「지워도 아무 일이 안 일어나는」 입금 줄이 쌓였다(실측 heymanerp 4건, 한 달에 한 건 꼴).

🔑 **고칠 때 새 경로를 만들지 말 것** — 미러를 **먼저** 지우면 기존 «채권관리에서 지우면 같이»
통로를 그대로 탄다. 규칙이 한 벌로 유지되고, 나중에 그 규칙이 바뀌어도 여기가 저절로 따라온다.

🗣️ **설계 의도를 사용자가 먼저 말했다** — jin: *「서로 미러가 되어서 한쪽에서 삭제하면 같이
적용되어야 하지 않나? 내가 이해한게 맞는건가?」*. 맞았다. 나는 「4항목은 분류 메타데이터니
아예 기록을 안 남기는 안」도 나란히 올렸는데, 그건 **채권관리에서 계약금·수수료 이력이
사라지는** 안이었다. ⇒ **선택지를 낼 땐 각 안이 「무엇을 잃는지」를 같이 적을 것.**
사람은 그걸 보고 고른다(jin: *「채권관리에는 기록이 남아야 되지않나..?」*).

---

**🔒 가드 = `PaymentBreakdownPrecisionTest`** — 핵심은 **왕복**이다(저장 → 재로드 → **아무것도 안 바꾸고** 재저장).
⚠️ **저장 한 번만 보는 테스트로는 원리상 못 잡는다** — 1회차는 6.46 이 정상으로 들어간다.
기존 스위트 32건이 전부 `'6'`·`'10'` 같은 **정수**만 넣어서 5개월간 아무도 못 봤다.
정적 검사(`(string) (int) $sum` 재등장 금지)도 같이 둔다 — 되돌아가도 화면은 정상 렌더되고 저장도 성공한다.

🚨 **그 12건 중 하나가 처음엔 헛통과했다.** 「밖에서 지운 행이 안 되살아난다」를 일반 `save()` 로 썼는데,
`save()` 가 끝에 `close()` 로 폼을 비워서 **가드를 지나지도 않고** 초록이 떴다. 고친 코드를 **되돌려
보고서야** 알았다(§8 #73) — 브레이크 3종 중 그것만 빨개지지 않았다. ⇒ **가드를 넣었으면 반드시
일부러 깨뜨려 볼 것.** 초록은 「지켜진다」가 아니라 「그 경로를 안 밟았다」일 수 있다.
그리고 **가드 C(마감 차량)를 지워도 안 빨개졌다** — `FinalPayment::creating` 이 이미 막고 있었기 때문이다.
그래서 진짜 구멍을 따로 찾아야 했다: **금액을 「비우면」 생성이 없어 그 가드가 안 걸리고,
삭제는 bulk 라 `deleting` 도 안 뜬다** ⇒ 마감된 확정 행이 조용히 사라진다. 그게 가드 C 의 존재 이유다.

---

### 91-B. 🖱️ **Livewire 왕복 한 번마다 패널 스크롤이 맨 위로 튄다** (같은 날, jin 제보)

jin: *「스크롤 내리고 input 쪽을 클릭하면 갑자기 화면이 맨위로」* → 결정적 단서는 그다음 말이었다:
*「잔금N행을 추가하는순간 맨위로 올라가」*. **버튼 클릭 = 확실한 서버 왕복**이라 원인이 한 번에 좁혀졌다.

목록과 패널이 **하나의 11,444줄 컴포넌트**라, 패널 안 어떤 조작이든 왕복이 나면 스크롤 컨테이너가
morph 되고 `scrollTop` 이 0 이 된다. 왕복 지점이 생각보다 많았다 —
`wire:model.live` 체크박스 2개(클릭 즉시) · `wire:model.blur` 7칸(**칸을 빠져나오기만 해도**) ·
`wire:poll.30s` 2개 · 그리고 **날짜칸**.

🚨 **날짜칸은 그 자체로 버그였다** — 2026-07-20 에 1970 버그를 막으려고 `focusout` 에서 `input` 을
**무조건** 재발행했는데, 그 사이 그 칸이 `wire:model.live` 가 되면서 **값을 안 바꿔도 왕복**이 나갔다.
⇒ **값이 실제로 바뀐 경우만** 알리면 방어는 그대로 살고 헛왕복만 사라진다.
🧭 **「무조건 dispatch」는 나중에 바인딩이 바뀌면 비용이 된다** — 그때 다시 보지 않으면 아무도 모른다.

**고침 = 위치를 기억했다 되돌린다.** `commit` 훅에서 왕복 직전 `scrollTop` 을 저장하고 `succeed` 뒤
rAF 에서 복원. ⚠️ **표식에 편집 중인 차량 id 를 담아 다른 차를 열면 복원하지 않는다** — 그땐 맨 위가 맞다.

---

### 91-C. 🔁 **「보냈다」 도장을 보내기 전에 찍으면, 실패가 영구히 래치된다** (같은 날, 컬럼 2회 새로고침)

jin: *「새로고침 2번해야 컬럼이 반영돼서 제대로 나올때가 있음」*(로컬도 재현).

차량목록 컬럼은 **진실이 둘**이다 — **존재 여부는 서버**(세션을 보고 `<th>/<td>` 를 아예 안 그린다, §8 #79)
**표시 여부는 localStorage**(`x-show`). 둘을 잇는 다리는 Alpine `init()` 의 **단 1회** 호출이다.

```js
if (now === this._pushed) { return; }
this._pushed = now;                       // ← 도장을 먼저 찍고
if (this.$wire) { this.$wire.syncVisibleColumns(on); }   // ← 그다음 확인
```

Alpine 이 Livewire 보다 먼저 살아나면 그 한 번이 **조용히 유실**되는데 도장은 이미 찍혀 있어
**그 페이지 수명 동안 재시도가 없다.** 그래서 1회차는 낡은 세션으로 그려지고 2회차부터 맞는다.

🧭 **재시도 억제 플래그는 「성공한 뒤에」 세울 것.** 그리고 실패했을 때 **깨어날 신호**를 하나 붙인다
(`livewire:initialized` 1회). 곁다리로 `x-data` 의 자동 `init()` 과 `x-init="init()"` 이 **둘 다 불려**
초기화가 2회 돌고 있었다 — 무해했지만 의도된 코드가 아니었다.

---

### 92. 🔀 **유일하지 않은 컬럼으로 정렬하면 페이지네이션이 흔들린다** (2026-09-11 · 일괄 적재의 뒷면)

`ORDER BY created_at DESC` + `LIMIT/OFFSET` 인데 **같은 시각이 여럿이면** 동점 행의 순서를 DB 가
보장하지 않는다. 페이지마다 정렬이 새로 계산되므로 **같은 행이 두 페이지에 나오거나 어떤 행은
어느 페이지에도 안 나온다.** 예외도 로그도 없고, 사람은 「분명 있는 차인데 목록에 없다」로 겪는다.

**평소엔 안 터진다 — 일괄 적재가 만든다.** 실측:
```
ssancarerp  차량 4,794대 중 4,693대(98%)가 같은 「초」에 등록
            2026-08-28 13:48:12 한 초에만 79대
```
사람이 하나씩 등록하면 시각이 다 달라 동점이 거의 없다. **적재 한 번이 조건을 통째로 바꾼다**
(§8 #87 의 「소급 적재가 화면의 뜻을 바꾼다」와 같은 뿌리 — 이번엔 *정렬*이 바뀌었다).

🔎 **발견 경로가 중요하다** — 코드가 멀쩡한데 **CI 가 한 번 빨개져 3사 배포가 막혔다**
(`VehicleBuyerFilterComboboxTest`, 차 25대를 한 루프에서 만들어 시각이 전부 같았다).
「재실행하면 통과하는 테스트」를 flaky 로 넘기지 말 것 — **그게 운영 버그의 신호일 수 있다.**

**고침 = 정렬 끝에 유일키를 붙인다.** 방향은 주 정렬과 맞춘다(안 맞추면 동점 구간만 거꾸로 선다).
시각이 다른 행의 순서는 **하나도 안 바뀐다** — 순수 추가다. 적용 **17곳**(차량·재고·채권·정산·이체3·
승인·바이어·컨사이니·영업담당·항구·사용자·로그4). 예외 둘:
- **GROUP BY 목록**(선적요청)은 `id` 를 못 쓴다 → 그룹키(`batch_id`) 자체가 유일하므로 그걸 쓴다.
- 차량 엑셀 내보내기는 원래 `orderBy('id')` 라 이미 안전했다(**페이지네이션이 아니어도 확인할 것**).

**🚨 가드를 만들다 세 번 틀렸다 — 전부 「초록인데 안 지켜지는」 부류다(§8 #73).**

1. **SQLite 로는 재현이 안 된다.** 동점이어도 rowid 순으로 **안정적으로** 돌려준다. 운영은 MySQL 이다
   (§8 #36 의 그 드라이버 차이). tie-break 를 빼도 기능 테스트가 초록이었다.
   ⇒ **결과가 아니라 「실제로 나간 SQL」을 검사**한다(`DB::listen` → `order by` 절이 유일키로 끝나는가).
   이건 드라이버와 무관하게 유효하다. ⚠️ 집계·드롭다운 쿼리가 섞이므로 **`offset` 이 있는 쿼리만** 본다.
2. **정적 스캔이 변수 컬럼을 못 봤다.** `orderBy($this->sortColumn, …)` 는 따옴표 컬럼이 아니라
   정규식에 안 잡혔고, 그 앞의 엉뚱한 `latest('id')` 를 「마지막 정렬」로 읽어 **통과시켰다.**
   ⇒ 정렬 호출을 **종류로** 전부 찾아 **위치가 가장 뒤인 것**을 보고, 거기서 유일키를 확인한다.
3. 🧵 **역슬래시+b(\\b) 가 진짜 백스페이스 문자(0x08)로 박혔다** — 스크립트로 정규식을 파일에 쓸 때. 그 결과
   `limit` 절이 안 잘려 멀쩡한 코드가 빨갛게 나왔다. **§8 #73 에 이미 적힌 함정을 그대로 반복했다.**
   ⇒ 정규식을 파일에 써 넣었으면 **`cat -A` 로 실물을 확인**할 것. 눈으로는 구분이 안 된다.

**🧭 새 목록을 만들 때** — `paginate()` 를 쓰면서 정렬키가 `name`·`created_at`·금액처럼 **중복될 수 있으면
반드시 유일키를 마지막에** 붙인다. 「지금은 중복이 없다」는 근거가 못 된다 — 적재 한 번이면 생긴다.
가드 = `PaginationStableOrderTest`(SQL 검사 + 전 화면 정적 스캔).

### 93. 🧨 **단언에 거대한 소스를 넘기면, 실패가 「멈춤」으로 위장된다** (2026-09-12)

정적 가드를 쓰면서 11,500줄 blade 를 그대로 넘겼다:
```php
$this->assertStringContainsString('data-panel-scroll=', $blade);   // $blade = 600KB
```
통과할 땐 멀쩡하다. **실패하는 순간** PHPUnit 이 haystack 을 **통째로 실패 메시지에 찍느라**
`php artisan test` 가 120초 타임아웃에 걸렸다. 「일부러 깨뜨려 확인」(#73)을 돌렸더니 6건 중
**2건만 결과가 안 나왔고**, 나는 그걸 「가드가 안 걸린다」로 읽을 뻔했다. 실제로는 **잘 걸렸는데
출력이 안 끝난 것**이다. 나머지 4건이 빨리 끝난 이유도 그제야 설명된다 — 그것들은 haystack 이
작은 파일(30KB)이거나 **숫자 비교**였다.

⇒ **판정은 `str_contains`/`preg_match` 로 미리 하고, 단언에는 `assertTrue($ok, '사람이 읽을 메시지')`.**
   메시지에 필요한 조각만 담는다(발견된 토큰 목록 등).
🧭 곁다리 — **MEMORY 의 「실패가 있으면 프로세스가 안 끝난다」가 이것과 겹친다.** 파이프(`| grep`)로
   받으면 더 확실히 멈춘다. 검증 스크립트는 **로그 파일로 받고** 끝난 뒤 읽을 것.
가드 = `PanelScrollAndColumnSyncGuardTest`(그 파일 상단 주석에 이유를 박아뒀다).

### 94. 🪤 **「읽기는 하는데 저장 배열에 안 실린」 필드는 조용히 죽는다** (2026-09-12)

`AlimtalkRecipients::ruleMatches` 는 규칙 행의 `active` 를 존중하는데, 화면의 `saveTimeRules` 는
`to/days/from/till` **4개만** 기록한다 ⇒ **`active` 는 화면에서 켜지고 저장하면 사라지는 죽은 필드**다.
예외도 로그도 없다. 아무도 몰랐던 이유는 **그 필드를 쓰는 UI 가 없었기 때문**이다.

신호별 수신자(`types`)를 같은 구조에 얹으면서 **정확히 같은 함정 자리**를 지났다.
⇒ 🧭 **규칙·설정 배열에 새 키를 추가할 땐 「읽는 곳」과 「쓰는 곳」을 같이 grep** 하고,
   **저장 왕복 테스트**(화면에서 set → save → 다시 읽어 단언)를 반드시 둘 것.
   단위 테스트로 값을 직접 넣으면 저장 경로를 안 타서 **원리상 못 잡는다**(§8 #80 과 같은 뿌리).
⚠️ 하위호환은 **「키가 없으면 전부 적용」**으로 잡는다. 「빈 배열 = 아무것도 아님」으로 두면
   배포 순간 3사의 기존 설정이 통째로 죽는다. 그리고 **전부 선택이면 키를 아예 안 남긴다** —
   저장물이 이전과 글자 단위로 같아지고, 나중에 항목이 늘어도 그 규칙이 자동으로 따라온다.
가드 = `AlimtalkSignalRoutingTest`(왕복 · 하위호환 · 전부선택 · 미지의 값 폐기).



### 95. 📐 **양식을 「백지에서」 만들 때 — 병합셀은 행 높이를 자동으로 못 맞춘다** (2026-09-12 양도증명서)

jin 이 준 원본이 **텍스트 0자 스캔 PDF** 라 읽어올 격자가 없었다. 기존 빌더는 전부
「남의 xlsx 를 청소」하는 것이고 **백지 작성은 이 레포 최초**였다. 밟은 것들:

- 🚨 **병합셀 + `wrapText` 는 엑셀이 행 높이를 자동으로 안 늘린다.** 원본 양식의 줄바꿈을 그대로
  옮겼더니 우리 표가 더 좁아 한 줄이 두 줄로 다시 감겼고, 높이가 모자라 **아랫줄이 통째로 잘린 채
  인쇄**됐다. 예외 0 · 화면 정상 · 테스트 초록.
  ⇒ **폭 예산에 맞춰 코드가 직접 감고**(`reflow()`) 줄 수로 높이를 정한다. 그래야 구조적으로 맞는다.
- 📏 **폭 예산의 이론값은 15% 낙관적이다.** 「483pt ÷ 6pt = 80자」로 계산했는데 실제는 66자였다
  (자간·기호 폭). ⇒ **LibreOffice 로 PDF 를 뽑아 눈으로 보고** 정할 것 —
  `soffice.com --headless --convert-to pdf` + `pymupdf` 로 30초면 된다. 이 프로젝트는
  `PdfConverter` 가 이미 그 경로를 쓰므로 **인쇄 결과를 실제로 볼 수 있다.**
- 🟡 **노란칸 좌표를 양식과 매핑에 따로 적지 말 것.** 빌더가 `Mapping::CELLS` 를 **읽어서** 칠하면
  둘이 어긋날 수 없다(§8 #44 의 적용형). 거꾸로 칠하면 조용히 틀린다 —
  매핑 있는 칸이 흰칸이면 **노란 배경이 인쇄물에 남고**, 고정 리터럴이 노란칸이면 기입 전에
  비워져 **공란**이 된다(§8 #71).
- 🖨️ **`fitToPage` 를 레이아웃 수단으로 쓰지 말 것.** 안전망이다 — 그것에 기대면 값 글자까지
  같이 줄어 읽기 어려워진다. 한 장에 들어가게 짜고 나서 켠다.
  🚨 **그래서 「몇 장인가」로 확인하면 안 된다 — 페이지 수는 채움을 보장하지 않는다.**
  2026-09-16 에 이걸 밟았다: 양도증명서가 세로만 맞고 **가로를 34% 남긴 채** 배포됐는데,
  fitToPage 가 한 장으로 만들어 주니 「3사 전부 A4 1장」으로 초록이었다. 인쇄물만 왼쪽으로
  몰렸다(jin: *「왼쪽으로 치우쳐서 오른쪽 여백이 많이 남고」*).
  ⇒ **fitToPage 를 끈 사본으로 100% 렌더해 ①페이지 수 ②내용 x-extent 를 둘 다** 잰다.
  ⚠️ **엔진이 열 너비를 다르게 계산한다** — 같은 파일이 엑셀 90% · LibreOffice 99%(실측 9% 차).
  사용자가 보는 건 엑셀이다: `New-Object -ComObject Excel.Application` → `ExportAsFixedFormat(0, path)`.
  🔢 **숫자 서식도 엔진마다 다르게 그린다 (2026-09-20)** — `#,##0.##` 을 정수에 적용하면
  **엑셀은 `1,234,567.` 로 마침표를 매달고 LibreOffice 는 안 붙인다.** LibreOffice 로만 확인하고
  배포했다가 jin 이 *「콤마는 맞는데 마침표는 왜 들어갔어..ㅋㅋ」* 로 잡았다.
  ⇒ **사람이 보는 것은 엑셀이다** — 서식은 엑셀 COM 으로 확인할 것:
  `$xl=New-Object -ComObject Excel.Application; $ws.Cells.Item($r,$c).Text`(`.Text` 가 «보이는 글자»다).
  ⇒ 실측표(2026-09-20): `#,##0.##` → `1,234,567.` / `#,##0` → `9`(8.6409 이 뭉개진다) / `General` → `8.6409`.
  **쉼표를 붙이면서 마침표를 안 남기는 서식은 엑셀에 없다** — 금액은 `#,##0`, 소수가 뜻을 갖는 칸(환율)은 `General`.
  ⚠️ 폭을 키우면 **글자 폭 예산도 같이** 키운다 — 폭만 키우면 줄이 짧게 끝나 오른쪽이 또 비고,
  예산만 키우면 상자 밖으로 **잘린 채 인쇄**된다. 배율 하나(`WIDTH_SCALE`)에 묶으면 어긋날 수 없다.
  ⚠️ **세로 여백 0.01 인치가 한 장을 가른다** — 구 양식은 **0.45pt** 초과로 45행에서 갈라졌다.
  엑셀은 행 높이를 화면 픽셀(0.75pt) 배수로 **올려** 잡는다 — 원값 합만 재면 못 잡는다.
  🅿️ 단 **프린터 드라이버의 하드마진까지는 파일로 못 잡는다** — 「실제 크기」로 찍으면 프린터마다 다르다.
- 🔖 **도장 위치는 렌더해서 볼 것.** 셀 높이가 촘촘하면(17pt ≈ 23px) 상자를 크게 잡은 도장이
  **아래 상자를 파고든다.** 실제로 하단 서명란 도장이 「(유의사항)」 안으로 들어갔다 —
  숫자만 보고는 몰랐고 PDF 를 보고 알았다(앵커를 두 행 위로 올려 해소).
- ⬅️ **값칸은 한 칸 들여쓸 것**(`setIndent(1)`). 라벨이 가운데 정렬이라 상자 오른쪽 끝에 닿아서
  들여쓰기가 없으면 「자동차등록번호99테0001」처럼 붙어 보인다.

---

#### 95-B. 🧵 **§8 #77 이 세 번째로 재발했다 — 이번엔 pint 가 아니라 `perl -0pi` 였다**

`"압류 및 \n 저당권"`(백슬래시+n) 을 perl 치환으로 넣었더니 **실제 개행**이 박혔고(CRLF 파일이라 CR 포함),
뒤이어 pint 의 `line_ending` 이 **코드 줄바꿈만** LF 로 정규화해 **문자열 안 CR 만 살아남았다.**
`lang/*/vehicle.php` 의 기존 여러 줄 문자열에서도 같은 일이 났다(내가 안 건드린 줄인데 CR 이 생겼다).

⇒ **치환 도구(perl·python·sed)로 이스케이프 개행(백슬래시+n)을 넣은 뒤에는 반드시 실물을 확인**할 것:
`php -r 'echo substr_count(file_get_contents($f), chr(13));'` — 눈으로는 구분이 안 된다.
⇒ **pint 를 돌린 뒤에도 한 번 더** 세어 볼 것. 변경한 파일 전부를 훑는 게 싸다.
