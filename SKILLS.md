...
**🔒 가드 = `NotionGuideAudienceTest`** — audience 무결성·발행 함수의 마커 생성·`--verify` 존재를 정적 검사.
⚠️ 단 이 테스트는 **"낡은 설명이 남았는지"까지는 못 잡는다**(자연어라 불가). 그건 삭제 PR 체크리스트로 막는다:
지우는 기능의 **이름·메뉴 라벨을 `scripts/notion-*` 에 grep** 해서 걸리면 같이 고칠 것.
