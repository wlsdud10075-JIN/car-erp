...
### 96. 🪟 **스크롤이 「안 먹는」 것과 「튀는」 것은 다른 병이다** — flex 자식의 `min-height:auto` (2026-09-12)

정산 「예외 대상」 모달을 만들며 jin 이 *「스크롤 내리고 올려도 반응이 없네?」* 라고 했다.
§8 #91-B 의 **스크롤 튐**(왕복마다 맨 위로)으로 읽고 그쪽을 뒤질 뻔했는데 **다른 병**이었다 —
막대가 아예 안 생겼다.

```html
<div class="flex max-h-[85vh] flex-col">   <!-- 부모는 85vh 로 묶임 -->
    <div class="flex-1 overflow-y-auto">   <!-- ❌ 스크롤이 영영 안 생긴다 -->
```
**flex 자식은 기본 `min-height: auto` 라 내용보다 작아지지 않는다.** 그래서 이 칸이 내용만큼
커져 버리고, 자기 안에 넘칠 게 없으니 `overflow-y-auto` 가 **발동할 일이 없다.** 목록은
부모 밖으로 삐져나가 잘린다. 에러 0 · 렌더 정상.

⇒ **`min-h-0` 을 스크롤 칸에 준다**(세로 flex면 `min-h-0`, 가로면 `min-w-0`).
🧭 **진단 한 줄** — 스크롤 막대가 **보이긴 하나?** 안 보이면 레이아웃(이 건), 보이는데 위치가
돌아가면 §8 #91-B(왕복 복원)다. 두 병을 먼저 갈라야 엉뚱한 데를 판다.
