...
### 89. 🎫 **알림톡 `tmplId` 에 템플릿 코드를 그대로 넣으면 「켜짐」으로 보이고 BizM 만 거부한다** (2026-09-11)

`AlimtalkConfig::canSend()` 는 **`tmplId` 가 비어 있지 않은지**만 본다 — 그 값이 **BizM 이 발급한 것인지**는
확인할 방법이 없다. 그래서 템플릿 코드(`erp_deregistration_reminder`)를 그 칸에 그대로 넣으면
**화면·버튼·게이트가 전부 「정상」** 이고 BizM 만 `K104:TemplateNotFound` 로 돌려보낸다.

**실측 2건** — 운영 heymanerp·ssancarerp 의 말소 재촉이 그 상태로 09-08 에 **10건 실패**했고,
jin 이 로컬에서 세금계산서 템플릿을 시험할 때도 같은 값을 넣었다. **두 번 다 같은 형태다.**

- 🔎 **증상이 「발송 0건」이 아니라 「전부 실패」**다 — #62(수신자 표 누락 = 조용히 skip)와 갈린다.
  ⇒ 「승인했는데 안 온다」 진단 순서(#62)에 **①-b 「tmplId 가 진짜 발급값인가」** 를 끼워 넣을 것.
  BizM 발급값은 대개 코드와 다른 문자열이다(우리 코드는 `erp_` 로 시작한다 — 그게 그대로 들어 있으면 의심).
- 🧭 **심사 중에는 그 칸을 비워 둘 것.** 비어 있으면 버튼이 안 뜨고 cron 은 skip 한다 — 조용하지만
  **실패 로그가 안 쌓인다**. 값을 넣어 두면 대상이 생길 때마다 실패가 쌓여 아침 점검이 매일 빨개진다.
- ⚠️ 이 부류는 **알림톡 로그에만** 남는다(`Log` 를 안 탄다 — #62 의 그 구조). 로그 화면을 열어야 보인다.
