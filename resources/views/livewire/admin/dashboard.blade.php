...
<div class="flex h-full w-full flex-1 flex-col gap-4 p-3 md:p-6">
    ...
    <div class="flex items-end justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">매출 / 채량 진행 KPI · 기간별 비즈니스 지표</p>
        </div>
    ...
    <div class="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <div class="card">
            <div class="text-xs text-gray-500">기간 차량 수</div>
        ...
        <div class="card">
            <div class="text-xs text-gray-500">기간 매입가 합계</div>
        ...
        <div class="card">
            <div class="text-xs text-gray-500">기간 판매가 합계 (KRW 환산)</div>
        ...
        <div class="card">
            <div class="text-xs text-gray-500">채권 관리</div>
            <a href="{{ route('erp.receivables.index') }}" wire:navigate
               class="mt-1 inline-block text-sm font-medium text-violet-600 hover:underline">
                미수금 / 회수 이력 보기 →
            </a>
            <div class="mt-1 text-xs text-gray-400">채권은 별도 화면에서 관리합니다.</div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="section-header">
            ...
            <span class="section-title">채널별 차량 수</span>
        </div>
        <div class="mt-3 grid grid-cols-3 gap-3">
            @foreach (['export' => '수출', 'heyman' => '헤이맨', 'carpul' => '카풀'] as $code => $label)
            <div class="card-sm flex items-center justify-between">
                <span class="text-sm text-gray-700">{{ $label }}</span>
                <span class="text-lg font-bold text-gray-800">{{ number_format($this->kpis['by_channel'][$code]) }}</span>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="card">
        <div class="section-header">
            ...
            <span class="section-title">진행 단계별 차량 수</span>
        </div>
        <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-6">
            ...
            <div class="card-sm flex items-center justify-between {{ $cnt > 0 ? '' : 'opacity-40' }}">
