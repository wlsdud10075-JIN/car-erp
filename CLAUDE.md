...
**Claude 작업 규칙 (1인 개발 컨텍스트 — jin이 직접 앉은 인터랙티브 세션 기준)**:
- **별도 feature 브랜치 만들지 않음** — 모든 변경은 `dev`에 직접 커밋·푸시
- PR 만들지 않음 (사용자가 명시적으로 "PR 만들어줘"라고 한 경우만 예외)
- 커밋 단위로 변경을 정리해서 추적성 확보 (한 커밋 = 한 논리적 변경)
- ⚠️ **위 3줄은 인터랙티브 세션 한정.** FleetView에서 병렬로 던진 **백그라운드 잡은 예외** — 아래 절차를 따름.

### 병렬 백그라운드 잡 통합 절차 (FleetView 다중 잡)

> 병렬 백그라운드 잡들은 **서로를 모른다**(대화·메모리 비공유). 같은 레포를 만지면 §30식 "조용한 드롭"(충돌 표시 없이 상대 잡 변경 누락) 위험.
> 하니스가 **워크트리 격리를 강제**(공유 체크아웃 편집 거부)하므로 각 잡은 자기 브랜치에 커밋 → push → **draft PR** 로 끝낸다 (dev/master 직접 push·force·머지 **금지**). ⇒ "작업 중" 오염은 자동으로 막힘. **위험은 오직 "dev로 합치는 순간".**

**충돌·조용한 드롭 예방 3층** ("매번 순차"도 "매번 눈으로 체리픽"도 아님 — **작업은 병렬, 통합만 순차**):
1. **쪼개서 던지기** — 병렬 잡은 겹치지 않는 파일/모듈로. 겹침이 유일한 적. (작업 자체는 병렬 OK)
2. **통합만 순차 (rebase)** — PR1 dev 머지 → 다음 브랜치 `git rebase origin/dev`(충돌을 그 브랜치 기준으로 명시 표면화) → 해결 → 머지 → 다음. **병렬 브랜치를 dev로 넣을 때 `cherry-pick` 금지** — cherry-pick은 dev→master 배포 트릭 전용(커밋 원부모 기준 병합이라 상대 잡 변경을 조용히 드롭함, §30 원인). stale 브랜치 눈감고 머지 금지.
3. **매 머지 후 `php artisan test`** — §30식 조용한 드롭을 자동 검출(테스트 커버된 코드 한정). dev→master 배포 땐 추가로 `git diff HEAD..dev -- <파일>` 재검증(§30).

**자동화 요청 패턴**: jin "PR들 통합해줘" → 위 2번(rebase→충돌 해결→머지→테스트)을 순서대로 대행, 겹친 지점 보고.
