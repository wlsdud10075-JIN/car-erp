...
> - 🎯 **정본 = `C:\Users\User\Desktop\연구소\ERP_SSANCAR_WEB\ERP_연동_통합정리_v1.N.md` — 폴더에서 가장 높은 번호가 최신.** 양측이 **번갈아 버전을 올리며** 이어 쓴다(직전 버전 복사 → 번호 올림 → 그 위에 씀, **기존 파일·남의 판단 삭제 금지**, 근거는 `파일:줄`). 규칙 전문 = 그 문서 §0. 현재 = **v1.2(ERP측)**.
