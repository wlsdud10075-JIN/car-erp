...
## 14. 최종 작업 큐 (v5.1 — 기존 큐 진행 상태 반영)

### ⚠️ v5 → v5.1 정정

회의록 v5 초안 작성 시 기존 큐 진행 상태(메모리 `project_queue_status`)를 반영 안 했음. 정정:

| 큐 | v5 (잘못) | v5.1 (실제) | 출처 |
|---|---|---|---|
| 큐 3 | "워밍업 ~1h" | ✅ **완료** | 커밋 06a6dea·a2790ff |
| 큐 4 | 후속 추천 | ✅ **완료** | 7커밋 (e9785ee·e5fc0c1·02fd855·8d47ce0·0cc4d15·2095fe8·c9e20fc) |
| 큐 6 잔여 | 누락 | ✅ **완료** | a0bcd76 |
| 큐 7 확장 | 후속 | ✅ **완료** | a0bf401·8ef4a3a (C7-a·H9·H10·H11) |
| 큐 9 | 누락 | ✅ **완료** | b11c700·ce0a2cd (H1·H2·H7) |
| 큐 10 | 누락 | ✅ **완료** | 90d8724 (H3·H4·H5·H6) |
| 큐 11 | "G7 감사 로그 확장 6~8h" | ⏳ **기존 미완 + G7 감사 로그 통합** | 메모리 |
| 큐 5 | 누락 | ⏳ **기존 미완** (메모리 "다음 큐") | 메모리 |

### 최종 작업 큐 (v5.1)

```
[Phase 1 — 기존 미완 클리어]
1. 큐 5 (업무 대시보드 [담당자별]↔[역할별] 토글)                3~4h
2. 큐 11 (H8 N+1 통일 + H12 자동 DB 백업 + forceDelete 백업
   + G7 감사 로그 audit_logs 확장 통합)                          8~12h

[Phase 2 — 회의 결과 신규, 즉시 가능]
3. 큐 18 (G7 차량 편집 close confirm 모달)                       1~2h
4. 큐 16 (G6 채널 단순화 — enum + 5컬럼 drop + 시드 재생성)      4~8h

[Phase 3 — 핵심 정책 (role 재설계 → 안건 4)]
5. 큐 14 (role 재설계 + approval_requests + in-app 알림)          12~18h
6. 큐 19 (안건 4 inter_vehicle_transfers, 큐 14 의존)             8~12h

[Phase 4 — 도메인 룰 (분모 정합 확정으로 즉시 가능)]
7. SKILLS.md §13 분모 단일 출처 박스                              0.5h
8. 큐 9 확장 (G1 50% 룰 + B/L 잠금, rule_version v3)              10~14h
9. 큐 10 확장 (G3 선적전/선적후/디파짓 미수 분류)                 12~15h
...
11. 큐 8 NICE API
12. 큐 12 포워딩 SMTP
13. 별건 1 G4 알림톡 (워크플로우 완성 후)
14. 별건 2 G7 동시 편집 락 (인프라 결정 후)                       16~32h
15. 별건 3 사이드바 재구성 (모든 화면 완료 후)                    2h
16. 큐 13 AWS Lightsail 배포 (최종)
```

**총 예상 (Phase 1~5)**: 60~95h ≈ 5~6주 (1인 25~30h/주 기준)
