...
## 📝 사용자 결정 (2026-05-19 후속) — 옵션 C 단계 배포 채택

회의록 정독 후 사용자가 **2주 내 100% 반영 가능성**을 재검토:

> "이거 배포 전에 어느정도 완성도가 있어야 해서 지금 내용은 다 반영이 되어야 할 것 같은데. 그리고 이거 1인 개발자가 할 분량이니?"

회의 결과는 8 부서가 각자 안건만 평가했고 **전체 합산은 누구도 안 함**. 정직한 재집계:

| 단계 | 작업 | 공수 |
|---|---|---|
| 본 회의 결정 (2주 내) | P0 fix + I 명칭 + E + C + 3-C-light + 큐 22-A·B + G + F | 50h |
| 사용자가 추가로 원하는 명세 잔여 | 큐 22-C + 안건 J 본격 + NICE API + I manager tier + 알림톡 | 30~50h |
| **총합 80~100h** | | |

1인 개발자 28일(2주 + 2주) × 5h/일 = 140h 가용 → 80~100h는 안전 마진 35% 확보 가능. **단 2주 단일 일정은 불가능**.

### 옵션 C 채택 (사용자 결정)

**1차 배포 (Day 1~14, 약 40~45h)**:
- P0 결함 fix 3건 + D·B·H 문서/링크
- I 명칭만 (정산→재무 / 통관→수출통관)
- E 판매 required + makeVehicle 헬퍼
- C 말소 [everyone] + canHandleDeregistration() + 3-C-light
- 큐 22-A 판매 입금 4컬럼 통합
- 큐 22-B savings_used 통합
- AWS Lightsail 1차 배포

**2차 배포 (Day 15~28, 약 35~45h)**:
- 큐 22-C 매입 측 통합
- 안건 G 50% 룰 H1 게이트 완화 (큐 22-A 완료 후)
- 안건 F 입금 분리 (TYPE_DEPOSIT_ADD_REQUEST 신설)
- 안건 J 본격 (v3 rule_version + 5곳 SQL — 사용자 의도 100% 반영)
- NICE API 연동 (1차 배포 후 1~2일 안정화 후)
- 안건 I manager tier 회의 + 구현 (Gemini "배포 후" 권고 충족)
- 알림톡 G4 (별건 1, 외부 카카오 API) — 별도 회의 필요
- 2차 배포

**3차 이후 (배포 안정화 후, 별도)**:
- 새회의 2·4·5번 (사용성 보강)
- 별건 2 G7 동시 편집 락
- 별건 3 사이드바 재구성 + 로그 화면 노출
- 3-A 그룹셋 정식

### 단계 배포 채택 사유

1. **1인 개발자 안전 페이스**: 1차 안정화 + 사용자 피드백 받고 2차 진입 (위험 분산). 글로벌 ERP 표준 패턴
2. **운영 안전 차단 요건 우선**: P0 결함 3건 + SoD/회계 안전이 1차에 모두 포함
3. **사용자 명세 거의 100% 반영**: 2차에 잔여 안건 완료. NO-GO된 안건 I/J도 2차에 manager tier/v3 rule_version으로 사용자 의도 충족
4. **3차 이후 묶음**: 사용성 보강은 운영 안정화 후 단계 적용 — 회의 결정 결과와 자연 일치

### 다음 세션 Day 1 즉시 시작 가이드

메모리 `project_queue_status.md` §🚀 다음 세션 즉시 진입 명령 + §🔧 Day 1 작업 가이드 참조. 코드 위치까지 명시되어 있어 즉시 진입 가능.
