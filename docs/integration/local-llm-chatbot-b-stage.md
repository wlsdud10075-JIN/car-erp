...
## 9. ✅ Jin 결정 (2026-07-24) + v1 착수 범위
- **배치 = ERP 전용 내장** (하이브리드 아님). 챗봇 = car-erp Livewire 위젯.
- **B 기본 = 대시보드에서 보는 것들 DB 조회·내역**: 인원별 미수현황·바이어별 미수현황·채권관리 내역.
- **super/최고관리자(admin) 심화**: 자금현황·회사 이익 (2단계 권한 — `canViewCapital`).
- **A(Notion 사내 업무 가이드) 기본 제공** — RAG 그대로.
- **v1 라우팅 = 결정적 키워드 분류기** (LLM tool-calling 아님). 이유: qwen3:8b tool-calling 신뢰도 미검증 리스크를 slice에서 제거 + 완전 테스트 가능. LLM은 A(RAG 답변 생성)에 사용. tool-calling은 검증 후 업그레이드 옵션.
  - B 숫자는 §1 불변식대로 **서버에서 쿼리 실행 → 문장 템플릿에 숫자 삽입**(LLM 미경유).
- **권한 2단계**: `canUseAssistant`(super/admin/관리) = 미수·채권 조회 / `canViewCapital`(super/admin) = 자금·이익 심화.
- **인프라 = 로컬 우선**: car-erp(dev, GPU 데스크톱) → localhost Ollama. **프로덕션 미배포**(3사 배포는 WireGuard 터널 + 상시 GPU 확보 후). .md·미배포 코드로 dev에만.
- **⭐ 이식성 필수 (Jin 2026-07-24)**: 지금은 Jin PC, **회사 GPU PC 준비되면 그대로 이식**. 따라서 **전부 config/env 기반** — Ollama URL·모델명·index.json 경로 전부 `.env`(하드코딩·머신 특정값 0). 이식 = `.env` 값만 바꾸고 Ollama+모델 설치 + index.json 복사/재생성. (PoC config.php 철학 계승.)
