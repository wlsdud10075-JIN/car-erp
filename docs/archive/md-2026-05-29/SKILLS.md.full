...
`progress_status_rule_version` 분기 — v1=단일 트리거(grandfather), v2=이중 트리거, v3=거래완료 단순화, **v4=워크플로우 순서 변경** (기본값, 신규 row).
...
public function getProgressStatusAttribute(): string
{
    ...
    // 안건 J 본격 (2026-05-20) — v3 거래완료 trigger 단순화.
    // 회의확장씬 안건 1 (2026-05-21) — v4 워크플로우 순서 변경: 반입(선적) → 통관 → B/L → 거래완료.
    //   v4 = 현재 default. 사용자 의도: 선적 먼저 → 통관 → B/L → 거래완료 (v3 통관→선적→B/L 정반대).
...
- 10단계(매입중/매입완료/말소완료/판매중/판매완료/선적중/선적완료/통관중/통관완료/거래완료) 카운트 가로 스트립 (v4 — 회의확장씬 안건 1)
...
**도메인 고정 매핑 (car-erp)**:
- **차량 진행상태 5단계 그룹** (v4 — 회의확장씬 안건 1, 2026-05-21):
  - `매입중`/`매입완료`/`말소완료` = **`badge-blue`** (매입 단계)
  - `판매중`/`판매완료` = **`badge-purple`** (판매 단계)
  - `선적중`/`선적완료` = **`badge-amber`** (선적/반입 단계 — v3 통관 색 swap)
  - `통관중`/`통관완료` = **`badge-green`** (통관 단계 — v3 선적 색 swap)
  - `거래완료` = **`badge-gray`** (완료)
  - **v3 grandfather 호환**: `수출통관중`/`수출통관완료` = `badge-amber` 그대로 유지 (운영 데이터 0이지만 안전망)
...
### NICE 연동 대비 (GAP 컬럼, Phase 3-1 마이그)
...
- `deregistration_date`(말소일)·`nice_spec_cylinders`(기통). NICE 연동 전엔 해당 칸 공란, 연동 시 자동. 매매업등록번호(통관 D3·G3)는 NICE 비제공→공란(수기).
...
- **방식 = 오프라인 pre-extend + 런타임 removal-only** (advisor 채택안). 양식은 `scripts/extend_shipping_templates.php` 로 30슬롯 확장·커밋(1회 사람 검증). 런타임(`DocumentFiller::fillMulti`)은 N대 채우고 미사용 슬롯 `removeRow`+`garbageCollect` 로 트림 → 정확한 크기.
- **⚠️ removeRow 는 footer SUM range 자동축소 안 함**(실측 — 경계 케이스). 그래서 footer 집계수식을 채운영역(`=SUM(I21:I<lastfilled>)`)으로 **removeRow 전에 명시 재기록** → 참조가 전부 제거구간 위라 안전. container_invoice 는 stride-3 명시리스트→range 변환(fuel/spacer 행 I/J/K/L 공란이라 동치). per-row 수식(`=H`·`=SUM(I,L)`·`=RIGHT(E,6)`·`=F+G`)은 슬롯에 박혀 자동 보존.
...
### 남은 작업
- **NICE 연동(큐 8)**: ingest 시 nice_raw 저장 + engineSpec→nice_spec_cylinders 파싱 + resValidPeriod 분할.
...
| **G1 100% B/L 게이트** (2026-05-26 회의 — B/L 발급) | `unpaid_ratio > 0`(미완납) 차단. **B/L 발급은 잔금 100% 완납 필수**. grandfather + `unpaid_export_overrides`(stage='shipping') 우회 — 관리/관리자 승인(`canApproveUnpaidExport`). ⚠️ 2026-05-14 도입 시 50%였으나 사용자 워크플로우(통관·선적 50% / B/L 100%)에 맞춰 상향 |
