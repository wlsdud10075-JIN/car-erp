...
| **[판매대금확인]** | 바이어 1 + 차량 N대 | "이 바이어 차 N대 대금 넣었으니 확인해주세요" | **수동** — 차량별 체크 (3/5 부분확인) |

> 👤 **확인 주체 = `canConfirmFinance()` = super · admin · 업무관리자 · role∈{재무, 관리}**.
> 화면 진입(`transfers::mount`)도 같은 권한이라 **재무 전용이 아니다**(jin 2026-08-09 지적 — 문서가 좁게 적혀 있었다).
> 핵심은 **영업이 스스로 확인할 수 없다**는 것뿐이다.
