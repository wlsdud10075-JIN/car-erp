...
> 상태: **구현 완료** (2026-08-09 배포). 이후 2026-08-11 에 **입금요청이 계약금/매입잔금 2종 + 금액**으로 개정됨.
> 재개 트리거: "board 요청확인 이어서"
>
> 🔀 **2026-08-11 개정 — 아래 §1·§0 의 "금액은 오가지 않는다" 는 폐기됐다.**
> 신호에 금액이 없으니 받는 사람이 **얼마를 보내야 하는지 몰라** 결국 카톡으로 되물었다.
> 이제 매입 신호는 **계약금(`purchase_deposit`) / 매입잔금(`purchase_balance`)** 둘로 갈리고
> 각각 `amount_krw` 를 필수로 싣는다. **다만 표시 전용** — 회계 컬럼엔 여전히 안 쓴다(§11-5 유지).
> ⚠️ 계약금은 **미지급 0 으로 자동소멸시키지 않는다**(수동 확인만) — 미지급 0 은 잔금까지 다 준
> 상태라, 그때 꺼지면 "계약금 아직 안 보냈다"는 거짓 신호가 차 인수 시점까지 남는다.
> **현행 권위 = `docs/integration/board-portal-api.md §11`.** 아래는 최초 설계 기록이다.

## 0. 한 줄

board 영업이 **"해주세요"** 를 누르면 erp 관리 이상에게 뱃지가 뜨고, 처리하면 꺼진다.
**금액은 오가지 않는다** — 돈 기입은 지금까지처럼 erp 관리 이상이 한다.
*(↑ 2026-08-11 개정 — 위 박스 참조. 금액은 오되 회계엔 안 쓴다.)*
