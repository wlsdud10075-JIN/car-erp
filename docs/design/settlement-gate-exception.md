...
수동 「신규 정산」에는 차량 상태 검증이 **하나도 없고**, 그 아래 「미수 지급보류」가 돈을 막아 주고는
있으나 **그 보류에 예외가 전혀 없어** 운임비 후입력 건과 소액 미수 건이 **영구히 지급 불가**다.
⇒ 조건을 단일 출처 함수로 뽑아 생성 게이트와 지급 보류가 **같은 술어**를 보게 하고,
...
⇒ 실사례 2종이 **지금도 영구 보류**:
1. 컨테이너 운임비 — 도착까지 길게 5개월. 운임비는 정산 기준액 **밖**인데 미수를 만들어 지급을 막는다.
2. 6 USD / 10 EUR 소액 — 바이어가 다음 건과 묶어 보내겠다고 한 경우.
...
| **예외 거는 곳** | **지급보류 걸린 정산 행에서 바로** (주 통로) + 신규 정산 생성 시 (부 통로) |
