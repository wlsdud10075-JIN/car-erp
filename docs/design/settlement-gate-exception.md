...
> 상태 = ✅ **구현 완료 · 배포 대기** (dev, 2026-09-12). 🚫 **다시 조사하지 말 것.**
> 커밋 = `7215f49` 조건 단일화(동작 불변) · `fb95fef` 마이그 5컬럼 · `e47ffcc` 술어 4곳+잠금유예
> · `2bc278e` 화면·테스트·가이드 · `1846058` Notion 소스(dev only) · 뒤이어 미리보기 뱃지·중복계산 정리.
> 테스트 = `SettlementGateOverrideTest` **18건**, **8곳을 하나씩 되돌려 전부 빨개지는 것을 확인**(§8 #73).
> 전체 스위트 2811 pass / 7 fail(알려진 GD 差).
>
> 🅿️ **배포 전 남은 것** — ①마이그 3-DB 검증(CLAUDE.md ⑤) ②Notion 발행(Codex 몫).
> ⚠️ **배포 직후 예상 화면**: heymanerp 「예외 대상」 **6대**(전부 「운임비와 동일」 뱃지) /
>    ssancarerp **458대**(전부 진짜 미수 — 뱃지가 없으면 누르지 않는다) / karabaerp 미실측.
>
> 이 문서가 정본이다. 아래 「조사로 확인된 사실」이 2026-09-12 실측 결과다.
