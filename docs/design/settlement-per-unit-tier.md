...
> jin 2026-08-04 확정. 구현 완료(dev) · CI green(`mysql-check` 포함).
> 🛑 **jin 지시로 배포 보류 중(2026-08-04) — dev 에 두고 대기.** 배포 시 §6 체크리스트 필수.
...
| 두 체크박스 수정 권한 | **「[관리] 이상」만** — 재무는 제외(2026-08-04 재확인). role `관리` · 업무관리자 · 최고관리자 · 시스템관리자 |

> ⚠️ 재무는 체크박스를 못 쓰지만 `per_unit_amount` **직접 입력(우선순위 ①)** 은 여전히 가능하다.
> 지금까지 승계 5만원을 그렇게 수동 입력해 왔다 — 체크박스가 안 쓰이고 수동 입력만 계속될 여지가 있으니,
> 운영에서 그런 조짐이 보이면 권한 범위를 다시 논의할 것.
