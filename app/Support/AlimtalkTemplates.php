...
class AlimtalkTemplates
{
    ...
    /**
     * `erp_purchase_paid_v2` 의 `#{특이사항}` 에 들어갈 두 값 (jin 2026-08-21).
     *
     * 🚨 **빈 문자열을 넣지 말 것.** 저당이 없어도 이 줄은 뭔가로 채워야 한다 — 변수 자리를 비우면
     *    반려 위험이 있다(`AccountMask::NONE` 이 계좌에 `-` 를 넣는 것과 같은 이유).
     * ⚠️ 이 문구는 **BizM 승인본과 무관하다** — 등록본에 올라가는 건 `#{특이사항}` 이라는 자리뿐이라,
     *    문장을 고쳐도 재승인이 필요 없다. 반대로 본문(`TEMPLATES`)을 고치면 재승인이다.
     */
    public const MORTGAGE_NOTICE = '※ 저당이 설정된 차량입니다. 저당 해지를 부탁드립니다.';

    public const MORTGAGE_NONE = '※ 특이사항 없음';
    ...
    public const TEMPLATES = [
        ...
        'erp_daily_summary' => [
            'name' => '일일요약',
            'recipient' => 'admin',
            // 🔀 2026-08-20 (jin) — **돈 이야기를 erp_receivable_status 로 분리**하고 진행 현황만 남겼다.
            //   구: 이번 달 매출 + 선적전/선적후 미수. 채권과 성격이 섞여 "오늘 뭘 밀어야 하나" 가 안 보였다.
            'vars' => ['날짜', '판매건수', '매출액', '선적대기', '판매중건수', '판매중금액', '통관대기', 'BL대기'],
            'title' => '',
            // ⚠️ 「판매중」의 기준(60%)은 **본문**에 적는다 — 카드 description 은 변수를 포함한
            //    **원문**이 20자를 넘으면 등록이 거부된다(BizM "메시지 길이제한"). 본문은 1000자라 여유가 있다.
            'body' => "[일일 현황] #{날짜}\n\n이번 달 매출과 차량 진행 현황입니다.\n※ 판매중 = 선적 진입 입금률(60%)에 미달한 차량\n차량관리에서 자세히 확인하실 수 있습니다.",
        ],
        // 채권 현황 (jin 2026-08-20) — 일일요약에서 **돈 이야기를 떼어낸** 신규 템플릿.
        //   설계 핵심 = **분모가 하나다**. 미수로 잡힌 차(잔금 남은 차 전부)의 판매금액을 100 으로 놓고
        //   유예·선적전·선적후·입금액을 쪼갠다 ⇒ 네 줄을 그냥 더하면 100% 가 된다.
        //   모수가 섞이는 문제(2026-08-20 세션 내내 추적한 그것)가 원천적으로 안 생긴다.
        //   ⚠️ 총 미수금 = 선적전 + 선적후 (유예 제외 — 2026-07-06 "결제대기는 아직 채권 아님" 유지).
        //   유예는 같은 분모를 쓰되 미수 합계에는 안 들어간다 — "아직 기다리는 돈" 으로 내용만 알린다(jin).
        'erp_receivable_status' => [
            'name' => '채권현황',
            'recipient' => 'admin',
        ...
        // ── 국내 딜러(dealer) 수동 발송 3종째 — 세금계산서 발행 요청 (jin 2026-09-11) ──
        // 매입 탭에서 재무·수출통관·관리 이상이 버튼으로 발송. 수신 번호는 말소등록증·입금완료와
        // **같은 칸**(vehicles.deregistration_notice_phone)을 쓴다 — 세 기능의 세 번째 소비자다.
        //
        // 🚫 사업자등록증 PDF 를 첨부할 수 없다 — 알림톡은 파일 첨부가 안 된다(위 말소증 주석과 같은 이유).
        //    그래서 만료 서명 링크(#{링크})로 내려받게 한다. 만료 7일 = 딜러가 세무 처리할 시간
        //    (말소증 3일보다 길다). ⚠️ **본문에 「7일」이 박혀 있으므로 기간을 바꾸면 재승인**이다.
        //
        // 🏢 회사명·사업자번호를 **변수로** 싣는다 — 3사가 같은 등록본을 쓰려면 리터럴로 박으면 안 된다.
        //    안 실으면 딜러가 어느 회사의 요청인지 본문만 보고 알 수 없다.
        'erp_tax_invoice_request' => [
            'name' => '세금계산서발행요청',
            'recipient' => 'dealer',
            'vars' => ['차량번호', '회사명', '사업자번호', '링크'],
            'title' => '',
            'body' => "[세금계산서 발행 요청] #{차량번호}\n\n차량 매입 대금에 대한 세금계산서 발행을 요청드립니다.\n\n▶ 차량번호: #{차량번호}\n▶ 공급받는 자: #{회사명}\n▶ 사업자등록번호: #{사업자번호}\n\n아래 링크에서 사업자등록증을 확인·다운로드하실 수 있습니다.\n#{링크}\n\n(보안을 위해 링크는 7일 후 만료됩니다.)",
        ],
        ...
        // ── 위 템플릿의 후속본 (jin 2026-08-21) — ①저당 안내 ②담당영업 동시 수신 ──
        // 🚨 **왜 같은 코드를 안 고치고 새로 등록하나** — `erp_purchase_paid` 는 2026-08-18 승인·가동 중이다.
        //    본문만 갈아엎으면 «코드는 새 변수를 보내는데 tmplId 는 아직 옛 것» 인 구간이 생겨 그동안
        //    딜러 알림톡이 통째로 반려된다. 새 코드면 승인 전까지 옛 버튼이 그대로 살고, tmplId 가
        //    채워지는 순간 `canSend()` 가 켜지며 자동 전환된다(SKILLS §8 #54-B 의 승인·배포 분리).
        //    ⇒ v2 가 3사 모두 승인·가동된 뒤에야 `erp_purchase_paid` 를 은퇴시킨다. 그 전엔 손대지 말 것.
        //
        // 🔀 **2행(수신 대상)을 고쳤다** — 담당영업도 같은 통지를 받으므로 «딜러로 등록된 분께» 는
        //    거짓이 된다. 카카오는 본문만 읽고 수신자를 판별하며, 2026-07-30 자금보고가 정확히
        //    「수신 대상 불명확」으로 반려됐다. 수신자를 넓히면 이 문장도 같이 고쳐야 한다(=재승인).
        //
        // ※ **`#{특이사항}` 은 한 줄을 통째로 담는 변수다.** `erp_board_request` 의 `#{요청내역}` 이
        //    같은 형태로 승인을 통과했다. ⚠️ 빈 값을 보내지 말 것 — 저당이 없으면 `※ 특이사항 없음`
        //    을 채운다(`AlimtalkTemplates::MORTGAGE_NONE`). 빈 문자열은 반려 위험이 있다.
        'erp_purchase_paid_v2' => [
            'name' => '매입대금입금완료(저당표기)',
            'recipient' => 'dealer',
            'vars' => ['차량번호', '구분', '금액', '계좌', '입금자명', '특이사항'],
            'title' => '',
    ...
    public const ITEMLIST = [
        // 진행 현황 4항목 (jin 2026-08-20). 돈은 erp_receivable_status 가 맡는다.
        //   ⚠️ 「선적대기」는 **완납 + 아직 안 떠남**이다. "입금 60% 이상" 이 아니다 —
        //      실측에서 60%↑ 15대 중 11대가 이미 B/L 나온 거래완료였다(출고일만 미입력).
        //   ⚠️ 「판매중」은 선적 진입 입금률(기본 60%)에 못 미쳐 못 나가는 차. 진행상태 '판매중' 과 뜻이
        //      겹치지만 같지 않다(우회 승인으로 선적된 차가 섞인다) → description 에 기준을 적는다.
        'erp_daily_summary' => [
            'header' => '일일 업무 현황',
        ...
        //   요약칸은 **금액 표기만** 허용이라 % 를 넣지 않는다 — K140 반려(SKILLS §8 #40).
        'erp_receivable_status' => [
            'header' => '채권 현황',
            'highlight' => ['title' => '#{총판매금액}', 'description' => '미수 차량 #{대상대수}대 판매금액'],
            'items' => [
                ['title' => '유예기간', 'description' => '#{유예건수}건 · #{유예금액}'],
                ['title' => '선적전', 'description' => '#{선적전건수}건 · #{선적전금액}'],
                ['title' => '선적후', 'description' => '#{선적후건수}건 · #{선적후금액}'],
                ['title' => '입금액', 'description' => '#{입금액}'],
            ],
            'summary' => ['title' => '총 미수금', 'description' => '#{미수합계}'],
        ],
        'erp_weekly_summary' => [
            'header' => '주간 업무 현황',
