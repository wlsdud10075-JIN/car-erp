...
class CarmodooService
{
    ...
    public function parseHtml(string $html): array
    {
        ...
        //
        // 🚨 **「0건」과 「못 읽음」을 구분한다** (jin 2026-09-11). 예전엔 0 으로 초기화하고
        //    정규식이 안 맞으면 0 을 그대로 돌려주며 `success=true` 를 냈다. carmodoo 가
        //    페이지 구조를 바꾸는 날부터 **모든 차가 「압류 0 / 저당 0」= 깨끗**으로 보이고,
        //    화면은 그걸 회색 뱃지로 그려 진짜 깨끗한 차와 구분이 안 된다. 예외도 로그도 없다.
        //    환율 사고(SKILLS §8 #88)는 값이 「안 보이는」 실패였지만 이건 **틀린 답을 자신 있게
        //    보여주는** 실패다 — 매입 판단에 직접 쓰이므로 더 나쁘다.
        //
        //    ⇒ 라벨(「압류」 …)이 본문에 **있는데 숫자를 못 읽은 경우**와 **라벨 자체가 없는 경우**를
        //      둘 다 `null`(판정 불가)로 돌린다. 화면이 「확인 불가」로 그린다.
        //    ⚠️ `null` 은 「없다」가 아니다 — 소비자는 `?? 0` 으로 뭉개지 말 것.
        $text = $this->clean($dom->textContent);
        $summary = [];
        foreach (['압류', '저당', '구조'] as $k) {
            $summary[$k] = preg_match('/'.$k.'\s*(\d+)\s*건/u', $text, $m) ? (int) $m[1] : null;
        }
