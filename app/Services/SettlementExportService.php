...
class SettlementExportService
{
     ...
     *
     * 🚨 **`.##` 을 붙이지 말 것 — 엑셀은 정수에 마침표를 매달아 둔다.**
     *    jin: 「콤마는 맞는데 마침표는 왜 들어갔어..ㅋㅋ」 — 처음에 `#,##0.##` 로 넣었다가 받은 지적이다.
     *    **엑셀 COM 실측 (2026-09-20)** — 같은 파일을 두 엔진이 다르게 그린다:
     *    ```
     *              1234567      10434.54    8.6409
     *    #,##0.##  1,234,567.   10,434.54   8.64      ← 엑셀만 마침표가 매달린다(LibreOffice 는 안 붙인다)
     *    #,##0     1,234,567    10,435      9
     *    General   1234567      10434.54    8.6409
     *    ```
     *    ⇒ **쉼표를 붙이면서 마침표를 안 남기는 서식은 엑셀에 없다.** 금액은 `#,##0` 을 쓴다.
     *
     * ℹ️ 외화 판매금액의 소수는 **표시만** 반올림된다(셀 값은 그대로라 SUM·정본 대조는 정확하다).
     *    §8 #91-D 와 다른 상황이다 — 거긴 깎인 값이 **입력칸에 되돌아가 원본을 덮었고**,
     *    여기는 되읽지 않는 산출물이다.
     */
    private const NUM_FORMAT = '#,##0';

    /**
     * 환율 전용 — **`General`**. `#,##0` 이면 JPY 8.6409 가 **「9」** 가 되어 뜻이 사라지고,
     * `.####` 를 붙이면 정수 환율에 마침표가 매달린다(위 표). 환율은 네 자리라 쉼표가 필요 없다.
     */
    private const NUM4_FORMAT = 'General';
