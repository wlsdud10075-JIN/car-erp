...
class BuyerCashService
{
    ...
    /**
     * 현금 원장이 소진 대상으로 보는 판매입금 유형 — **제외 이유를 유형별로 남긴다.**
     *
     * 🚨 **2026-09-09 `fee` 합류 (jin 제보).** 종전엔 `type !== 'balance'` **한 줄**이라 수수료가
     *    「제외 이유가 적히지 않은 채」 휩쓸려 있었다(기획서 제외 표의 근거는 계약금 것뿐이다).
     *    실사고 = 판매탭 송금수수료가 **현금을 한 푼도 안 쓰고 미수만 줄여**, 원장이 남은 현금을
     *    부풀려 표시했다(heymanerp 실측: 수수료 4건 24 EUR 중 원장 반영 18 → 원장 2,280 ↔ 실제 2,274).
     *    그 6 은 **다음 잔금에 조용히 쓰여** 바이어에게 그만큼 더 크레딧이 된다 — 게이트가 막으려던
     *    것이 정확히 수수료 금액만큼 뚫린다. 예외도 로그도 없다.
     *
     * | 유형 | 대상 | 이유 |
     * |---|---|---|
     * | `balance` 잔금 | ✅ | 이 기능의 본체 |
     * | `fee` 송금수수료(셀러부담) | ✅ | **정의상 바이어가 보낸 돈의 일부**다. 입금은 「보낸 돈」으로 적으므로 그 수수료는 이미 그 입금 안에 있다 — 미수만 줄이면 차액이 원장에 영영 남는다 |
     * | `deposit_down` 계약금 | ❌ | **돈의 출처를 모른다** — 회사가 대신 낸 계약금이 실재해서 무담보 한도가 따로 있다(SKILLS §8 #49). 바이어 현금으로 단정하면 거짓 기록이 된다 |
     * | `interim` 중도금 · `advance_1` 선수금1 | ❌ | jin 명시 범위 밖(2026-09-09). 원장 바이어에게 실사용 **0 건**이라 지금 새는 것이 없다 — 쓰기 시작하면 이 표를 다시 볼 것 |
     *
     * 🚫 여기에 유형을 더할 땐 **판매탭 4항목 sync 의 「원장 밖」 판정**도 함께 볼 것
     *    (그 sync 는 지우고 다시 만들어서 정정도 신규 행이 된다 — `vehicles/index` 참조).
     */
    public const GATED_TYPES = ['balance', 'fee'];
    ...
    public function gated(FinalPayment $payment): bool
    {
        ...
        if (! in_array($payment->type, self::GATED_TYPES, true)) {
            return false;                       // 유형별 제외 이유 = 위 GATED_TYPES 표
        }
