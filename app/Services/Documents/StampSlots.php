...
class StampSlots
{
     ...
     *
     * 위치 = 신청인 서명란(K26). 그 자리는 「(서명 또는 인)」 라벨만 있는 빈칸이고
     * baked drawing 이 없어서 `clearAnchors` 가 필요 없다(3세트 실측 drawing 0건).
     *
     * 📏 **크기 218×107px — 「원래 크기로, 줄이지 말고」(jin 2026-09-04).**
     *    처음엔 셀 상자 세로 그대로 85 였는데, karaba 직인은 정사각 도장이 아니라
     *    **사업자 고무인 블록**(사업자번호·상호·대표·주소·업태 7줄 + 빨간 인감, 실측 1902×930 = 2.045:1)
     *    이라 세로가 병목이 되어 **174×85(4.6cm)로 축소**됐다 — 7줄 글자가 뭉갰다.
     *    세로를 107 로 올리면 비율맞춤의 병목이 **가로**로 바뀌어 K~M 폭을 다 쓴다(174×85 → 218×107).
     *    ⚠️ **cm 로 말할 땐 인쇄배율을 곱할 것** — 이 시트는 `fitToPage`(1쪽 폭, 실측 69%)라
     *       218×107px(5.8×2.8cm)은 **종이에선 약 4.0×2.0cm** 다. 더 키우려면 도장이 아니라
     *       페이지 설정 얘기가 된다. 세로는 29행 구분선까지의 가용 117px 안이라
     *    **양식을 안 넘는다**(29행 「특별시장…귀하」 글자는 왼쪽에서 끝나 K~M 구간은 비어 있다).
     *    ⚠️ 가로 218 = **양식의 K~M 실측 열폭**이다(맑은고딕 11 기준 70+70+78).
     *       구 주석의 233 은 Calibri 기준 환산이라 M 열 오른쪽으로 살짝 넘쳤다.
     * 🚫 더 키울 수 없다 — 인쇄영역이 `A1:M39` 라 M 열 오른쪽은 잘린다. 다른 서류의 직인 상자
     *    (인보이스 323 · 판매계약서 361)를 여기 그대로 가져오면 종이 밖으로 나간다.
    ...
    private static function karabaSlots(): array
    {
        ...
        $slots['deregistration_set'][] = [
            ...
            'anchor' => 'K26', 'width' => 218, 'height' => 107,
        ];
