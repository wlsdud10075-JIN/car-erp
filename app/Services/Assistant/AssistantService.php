...
class AssistantService
{
    ...
    /**
     * 화면·기능 이름이나 규칙을 묻는 질문인가 — DB 금액 조회로 보내지 않을지 판정한다 (jin 제보).
     *
     * 🚨 **실사고 2건** — ①「선적대기 허용 항로인데 미수여도 묶음 착수돼?」 가 채권 KPI 표를 받았고
     *    ②karabaerp 실사용 로그에서 **「채권담당자」(역할 이름)가 「채권」+「담당자」로 쪼개져**
     *    담당자별 미수 조회로 가 **권한 거부까지** 났다(사용자는 용어 뜻을 물었을 뿐이다).
     *
     * 🔬 **설계 근거 = 운영 로그 실측**(2026-09-09, heymanerp 49건 + karabaerp 31건).
     *    오분류 7건이 전부 「미수·채권」 낱말 하나 때문이었고, 그중 절반은 **질문에 규칙 신호가 아예 없다**
     *    (「채권담당자」 두 낱말뿐). ⇒ 의문사만 늘리는 방식으로는 원리상 못 잡는다.
     *
     * 판정 순서 — 앞이 이기면 뒤를 보지 않는다.
     *   ① **금액을 요구하는 신호**가 있으면 종전 DB 라우팅을 그대로 둔다. 실측 정상건이 전부 여기 걸린다
     *      (「미수금 얼마야」·「바이어별 미수현황은?」·「무사백 미수금 리스트 알려줘」·「채권리스트」).
     *      ⚠️ 이 순서를 뒤집으면 잘 쓰던 조회가 통째로 가이드로 새어 **숫자를 못 받는다**.
     *   ② **화면·기능·역할 이름**이 있으면 규칙 질문이다. 이게 가장 강한 신호다 — 금액 질문은 화면
     *      이름을 부르지 않고 금액과 사람을 부른다.
     *   ③ 규칙·절차·정의·가능여부를 묻는 신호.
     *   ④ 셋 다 없으면 **종전대로 DB 조회**다(「미수금」 한 마디 = 지금도 금액표가 맞다).
     *      여기서 기본값을 가이드로 바꾸면 잘 쓰던 조회가 조용히 죽는다.
     *
     * 🔑 **띄어쓰기를 지우고 비교한다** — 실사용에 「채권담당자」와 「채권 담당자」가 함께 나온다.
     */
    private function looksLikeGuideQuestion(string $q): bool
    {
        $flat = (string) preg_replace('/\s+/u', '', $q);
        $has = fn (array $kw) => (bool) array_filter($kw, fn ($k) => mb_strpos($flat, $k) !== false);

        // ① 금액·목록을 요구하는 신호 (인원별·바이어별 같은 «쪼개 보여달라» 도 포함)
        if ($has([
            '얼마', '총액', '합계', '몇', '현황', '리스트', '목록', '순위',
            '인원별', '담당자별', '바이어별', '거래처별', '사람별', '별로',
        ])) {
            return false;
        }

        // ② 화면·기능·역할 이름 — 실측에서 가장 강한 규칙 질문 신호.
        //    🚫 여기서 값을 빼면 그 이름이 금액 낱말과 같이 나올 때 다시 DB 조회로 샌다.
        if ($has([
            '채권담당자', '채권관리', '선적요청', '묶음', '착수', '반입지', '수출통관', '통관', '면장',
            '선하증권', 'B/L', '재고관리', '재고', '정산', '승인큐', '승인', '재무처리', '적립금',
            '차량관리', '서류', '알림톡', '기능설정', '컨사이니', '포워딩', '출고', '말소', '역할',
        ])) {
            return true;
        }
        ...
        // ③ 규칙·절차·정의·가능여부.
        return $has([
            '되나', '되니', '되냐', '되는', '되면', '돼?', '되어도', '여도', '해도', '가능', '수있', '있나',
            '왜', '어떻게', '어디서', '어디에', '어디야', '어떤', '뭐뭐', '무엇', '뭐야', '무슨', '뭔지',
            '조건', '기준', '규칙', '방법', '절차', '뜻', '의미', '차이', '이유', '인가', '일까',
            '막히', '차단', '안되', '안돼', '못하', '허용', '예외',
        ]);
        // 🚫 '언제' 는 넣지 않는다 — 「손익분기 언제 넘어?」 처럼 **시점을 계산해 주는 DB 질문**이
        //    규칙 질문으로 새어 버린다. 시점을 묻는 규칙 질문은 그 자체로 금액 낱말이 없어 어차피 가이드로 간다.
    }
