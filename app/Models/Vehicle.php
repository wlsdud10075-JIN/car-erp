...
class Vehicle extends Model
{
     ...
     *
     * 🔀 **2026-09-02 (jin) — 면허비 11,000 · 탁송비 30,000 을 0 으로 내렸다.** 말소비만 남는다.
     *    둘 다 업체 월명세서로 실측 기입되는 항목이라(면허=선적요청 2차비용 탭 · 탁송=명세서 기입)
     *    추정치를 미리 깔아둘 이유가 없다. 대상 = heyman·ssancar(이 상수). karaba 는 자기 세트라 무관.
     *    ⚠️ 그동안 이 추정치가 **미기입 구간의 마진을 눌러주는 역할**을 했다 — 이제 명세서를 올리기
     *    전에 1차 정산을 확정하면 총마진이 그만큼 크게 잡힌다(2차 정산 이월로 정정된다).
     */
    ...
    /**
     * 2026-09-02 이전 면허비 기본값. 「면허비 미기입」 판정(선적요청 2차 비용 탭)이
     * **전환 전에 등록된 차량**(11,000 그대로인 미기입 건)을 놓치지 않도록 남긴다.
     * 🚫 지우지 말 것 — 지우면 그 차량들의 미기입 뱃지가 조용히 꺼진다(SKILLS §8 #38 의 그 형태).
     */
    public const LEGACY_DEFAULT_LICENSE_FEE = 11000;
    ...
    public const AUDITED_COLUMNS = [
        ...
        'container_number',
        // 2026-09-08 (jin) — 수출신고번호. 선적요청 묶음 화면에 이어 차량관리 「선택 N대」에서도 일괄로
        //   찍는 입구가 생겼다. 세관에 신고한 번호가 수십 대 단위로 덮이는데 추적이 없으면 안 된다
        //   (07-28 에 shipping_date·eta_date 를 등재한 것과 같은 이유).
