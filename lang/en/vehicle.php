<?php

// i18n — Vehicle management (erp/vehicles/index). List view. (slide panel keys added later)
return [
    'title' => 'Vehicles',
    'total' => ':count vehicles',
    'per_page' => ':count / page',
    'per_page_count_only' => 'Count only',
    'count_only_hint' => ':count vehicle(s) match — list rendering skipped. Use [Export] to get them all.',
    'bulk_doc' => [
        'btn' => 'Bulk upload documents',
        'title' => 'Bulk document upload',
        'type' => 'Document type (one per dialog)',
        'type_checkbill' => 'Check bill',
        'type_export_declaration' => 'Export declaration',
        'type_bl' => 'B/L document',
        'completes_deal' => '⚠️ This will mark :count vehicle(s) as completed and auto-fill their release date.',
        'blocked_count' => ':count vehicle(s) blocked by a gate will be skipped (hover the number for the reason).',
        'blocked_bl_no_loading_location' => 'No loading location — enter it on the shipping tab first',
        'blocked_bl_no_sale_price' => 'No sale price — unpaid ratio cannot be evaluated',
        'blocked_bl_unpaid' => 'Balance unpaid — needs full payment or a B/L unpaid-export override',
        'type_deregistration' => 'Deregistration (a different file per vehicle)',
        'over_max' => ':count vehicles — deregistration allows :max at a time. Narrow the selection.',
        'picked' => ':count file(s) attached',
        'row_has_file' => 'already has one',
        'target' => ':count vehicle(s) — one file is saved to each',
        'no_target' => 'No target vehicles. Select them with the checkboxes first.',
        'no_scope' => ':count vehicle(s) outside your scope are excluded.',
        'file' => 'File to upload',
        'reason' => 'Reason (audit trail)',
        'reason_ph' => 'e.g. Sept 1st shipment declaration',
        'reason_default' => 'Bulk document upload',
        'apply' => 'Upload',
        'replace' => 'Also replace the :count vehicle(s) that already have a file',
        'unit' => '',
        'no_number' => '(no number)',
        'mixed_title' => '⚠️ The selection contains more than one number',
        'mixed_ack' => 'I confirm the document may differ per number',
        'mixed_block' => 'Numbers are mixed — tick the confirmation box to proceed.',
        'outside' => 'Also include :count vehicle(s) with the same number left out of the selection',
        'done' => 'Uploaded :applied · skipped :skipped',
    ],
    // Bulk entry of export declaration no / container no (jin 2026-09-08) — blank field = leave as is.
    'bulk_num' => [
        'btn' => 'Bulk enter numbers',
        'title' => 'Bulk enter export declaration no / container no / B/L no',
        'target' => 'Will be written to the :count selected vehicle(s)',
        'keep_hint' => 'A blank field is left untouched — filling only one is fine. (This tool cannot clear a value.)',
        'decl' => 'Export declaration no',
        'decl_ph' => 'e.g. 12345-67-8901234',
        'container' => 'Container no',
        'container_ph' => 'e.g. TEMU1234567 or 6.09_A RORO 12-33_5',
        'bl' => 'B/L no',
        'bl_ph' => 'e.g. HDMUABCD1234567',
        'mixed_title' => '⚠️ :field currently has :kinds distinct values — writing will overwrite them all',
        'mixed_ack' => 'I have reviewed the above and want to overwrite',
        'mixed_block' => 'Values are mixed — tick the confirmation box to proceed.',
        'unit' => '',
        'no_number' => '(empty)',
        'no_target' => 'No target vehicles. Select them with the checkboxes first.',
        'no_scope' => ':count vehicle(s) outside your scope are excluded.',
        'empty' => 'Enter an export declaration no or a container no.',
        'reason' => 'Reason (audit trail)',
        'reason_ph' => 'e.g. Sept Durres RORO declaration',
        'reason_default' => 'Bulk number entry',
        'apply' => 'Apply',
        'done' => 'Written :applied · unchanged :unchanged · excluded :skipped',
    ],
    'bulk_date' => [
        'btn' => 'Bulk shipping/ETA/VSL',
        'title' => 'Bulk set shipping date / ETA / vessel',
        'target' => 'Applies to :count vehicle(s) matching the current filter.',
        'ship' => 'Shipping date',
        'eta' => 'ETA',
        'vsl' => 'Vessel (VSL)',
        'vsl_ph' => 'e.g. MV GMT ASTRO V.2602',
        'ct_prefix' => 'Replace container prefix',
        'ct_keep' => '-- keep as is --',
        'ct_none' => 'No prefix-style container numbers in this selection (ISO or legacy formats only).',
        'ct_to_ph' => '6.09_A',
        'ct_preview' => ':count vehicle(s): :from -> :to (the cargo part stays)',
        'ct_hint' => 'ISO container numbers and legacy formats have no prefix and are left alone. Filter the list first when loaded and unloaded cars share a prefix.',
        'reason' => 'Reason (vessel name, etc.)',
        'reason_ph' => 'e.g. GMT shipment',
        'reason_default' => 'Bulk shipping date / ETA / vessel',
        'hint' => 'Blank fields are left unchanged. Undo requires setting them again.',
        'apply' => 'Apply to :count',
        'done' => 'Applied :applied · unchanged :unchanged · skipped :skipped',
        'unit' => '',
        'vsl_conflict' => '⚠ The target vehicles carry different vessel names',
        'vsl_empty' => '(empty)',
        'vsl_overwrite' => 'All :count vehicle(s) will be overwritten with ":vsl". If vehicles from another vessel are mixed in, cancel and narrow the filter.',
        'vsl_force' => 'Overwrite anyway',
    ],
    'batch_filter' => 'Showing only :count vehicle(s) from shipping request batch',
    'batch_filter_clear' => 'Show all',
    'create_btn' => 'Add Vehicle',
    'import_template_btn' => 'Import Template',
    'export_btn' => 'Export Data',
    'export_scope' => 'Range',
    'export_scope_current' => 'Current view',
    'export_scope_all' => 'All (ignore filters)',
    'export_scope_hint' => '· Current view = applies screen date/search filters / All = ignores all filters, every period',
    'export_cols_expand' => 'Expand columns',
    'export_cols_collapse' => 'Collapse columns',
    'export_do' => 'Download Excel',
    'export_pick_col' => 'Select at least one column.',
    'export_col_pinned' => 'always',
    'export_pinned_hint' => '· Vehicle no. and VIN are always included, so settlement-only exports stay traceable.',

    'search_placeholder' => 'Vehicle no. · B/L · decl. no. · vessel · container',
    'search_title' => 'Vehicle no. · brand · model · owner · export decl. no. · B/L no. · vessel · container no. · supplier · EMS/DHL tracking no.',
    'search_btn' => 'Search',
    'reset_btn' => 'Reset',
    'reset_hint' => 'Restore search, dates, filters and accumulation to the initial state.',
    'all_salesmen' => 'All Salesmen',
    'all_buyers' => 'All Buyers',
    'filter_all' => 'All',
    'filter_ship_ems' => 'EMS',
    'filter_ship_dhl' => 'DHL',
    'filter_ship_none' => 'Not sent',
    'filter_ship_month' => 'Sent month',
    'filter_ship_month_ph' => '202607',
    'filter_ship_hint' => 'Document shipping (EMS/DHL) — independent of progress status',
    'filter_dereg' => 'Deregistration due',
    'filter_dereg_hint' => 'Purchase fully paid :days+ days ago but not yet deregistered — independent of progress status, so cars being sold or shipped also appear',

    'date_type' => [
        'purchase' => 'Purchase Date',
        'deregistration' => 'Deregistration Date',
        'sale' => 'Sale Date',
        'shipping' => 'Shipping Date',
        'bl' => 'B/L Date',
        'balance' => 'Balance Paid',
        'all' => 'All (any date)',
    ],

    'columns' => 'Columns',
    'show_columns' => 'Visible Columns',
    'reset_defaults' => 'Reset to defaults',

    'col' => [
        'number' => 'Vehicle No.',
        'brand_model' => 'Brand/Model',
        'vin' => 'Chassis No.',
        'status' => 'Status',
        'purchase_date' => 'Purchased',
        'sale_date' => 'Sold',
        'shipping_date' => 'Shipped',
        'eta_date' => 'ETA',
        'bl_issue_date' => 'B/L Date',
        'deregistration_date' => 'Deregistration',
        'export_declaration_number' => 'Export Decl. No.',
        'container_number' => 'Container',
        'vessel_name' => 'Vessel (VSL)',
        'ems_tracking' => 'EMS Reg. No.',
        'dhl_tracking' => 'DHL Waybill',
        'ems_fee' => 'Postage',
        'dhl_fee' => 'DHL billed',
        'shipping_fee' => 'Shipping total',
        'shipping_sent_date' => 'Sent date',
        'bl_number' => 'B/L No.',
        'salesman' => 'Salesman',
        'purchase_from' => 'Purchase Source',
        'buyer' => 'Buyer',
        'consignee' => 'Consignee',
        'channel' => 'Channel',
        'currency_rate' => 'Currency/Rate',
        'purchase_price' => 'Buy Price',
        'sale_price' => 'Sell Price',
        'sale_total' => 'Sell Total',
        'transport_fee' => 'Freight',
        'transport_fee_usd' => 'Freight (USD)',
        'unpaid_amount' => 'Outstanding',
        'unpaid_ratio' => 'Paid %',
        'unpaid_purchase' => 'Purchase unpaid',
        'unpaid_sale' => 'Sale unpaid',
        'settlement_stage' => 'Settlement',
    ],

    // 정산 단계 뱃지 (jin 2026-09-07) — 값 = Vehicle::SETTLEMENT_STAGE_*. 컬럼 라벨은 col.settlement_stage.
    'settlement_badge' => [
        'done' => 'Settled',
        'waiting' => 'Pending',
        'freight' => 'Freight',
    ],

    'fully_paid' => 'Paid',
    'balance_badge' => 'D-:d bal',
    'balance_badge_overdue' => 'bal overdue',
    'deposit_badge_waiting' => 'Deposit-buy',

    'buyer_undecided_badge' => 'No buyer',
    'purchase_unpaid_badge_title' => 'Purchase balance unpaid: :amount KRW - click to show only vehicles with an unpaid balance',
    'progress_pill' => [
        'maeipjung_hint' => 'Shows every vehicle that still owes a purchase balance - including those already sold or shipped. The dashboard stage strip counts fewer because it is a distribution.',
    ],
    'date_range_ignored_hint' => 'The date basis is set to All, so the range you typed is not applied. Pick a basis (purchase date, sale date, ...) and it takes effect - for both the list and the export.',
    'buyer_undecided_title' => 'Bought without a buyer (speculative) — clears once a buyer is set',
    // Per-tab memos (2026-08-11) — separate from the shared memo at the bottom.
    'tab_memo' => [
        'title' => 'Tab memo',
        'placeholder' => [
            'purchase' => 'Notes about this purchase (visible on this tab only)',
            'sale' => 'Notes about this sale (visible on this tab only)',
            'clearance' => 'Notes about customs clearance (visible on this tab only)',
            'shipping' => 'Notes about shipping / warehouse-in (visible on this tab only)',
            'bl' => 'Notes about the B/L (visible on this tab only)',
        ],
    ],

    'board_badge_purchase' => 'Pay req.',
    'board_badge_sale' => 'Confirm',
    // Split of the payment request into two signals (2026-08-11)
    'board_badge_deposit' => 'Deposit',
    'board_badge_deposit_ceo' => 'CEO deposit',
    'board_badge_balance' => 'Balance',
    'board_title_purchase' => 'Sales requested a purchase payment — clears once the balance is recorded',
    'board_title_sale' => 'Sales reported an incoming payment — confirming replies back to board',
    'board_title_deposit' => 'Sales requested a deposit transfer — clears once you record the deposit in the purchase tab (or press Confirm). The amount is for reference; record it in the purchase tab yourself',
    'board_title_deposit_ceo' => 'Sales asked the CEO directly for a deposit transfer — used when the owner is away, so it is sent to the CEO regardless of time or weekday. A regular deposit request may be open on the same vehicle; watch for a double transfer (the amount is for reference)',
    'board_title_balance' => 'Sales requested the purchase balance — clears once the outstanding purchase amount reaches zero',
    'board_amount_unit' => ' KRW',
    'board_confirm_btn' => 'Confirm payment',
    'board_no_permission' => 'You are not allowed to confirm sale payments.',
    'board_confirm_done' => 'Confirmed — board will be notified.',
    'board_confirm_gone' => 'That request is already handled or cannot be confirmed here.',
    'board_requested_by' => ':email · requested :date',
    'deposit_badge_paid' => 'Deposit-buy paid',
    'deposit_purchase_waiting' => 'Deposit purchase · awaiting buyer sale payment',
    'deposit_purchase_paid' => 'Deposit purchase · fully paid (0 receivable)',
    'deleted' => 'Vehicle deleted.',
    'delete' => 'Delete',
    'delete_confirm' => 'Delete vehicle :number?',
    'delete_confirm_simple' => 'Delete this vehicle? If it has accounting records you will be asked for a reason.',
    'empty' => 'No vehicles.',
    'freight_total' => 'Freight total ₩:amount',
    'sale_total_sum' => 'Sell total ₩:amount',
    'freight_ph' => 'Freight amount',
    'vin_ph' => 'VIN search',
    'stat' => [
        'count' => 'Vehicles',
        'unit' => '',
        'freight' => 'Freight total',
        'sale_total' => 'Sell total',
        'shipping_fee' => 'Shipping',
        'purchase_unpaid' => 'Unpaid total',
        'purchase_unpaid_title' => 'Total purchase balance still owed, under the current search and filters. Click to show only vehicles with an unpaid balance - including those already sold or shipped.',
        'purchase_unpaid_off' => '- click to clear',
        'purchase_unpaid_cancelled' => 'includes :count cancelled purchase(s) (:amount KRW)',
        'ems_fee' => 'Postage',
        'dhl_fee' => 'DHL',
    ],

    'shipdoc_select_title' => 'Multi-select for shipping docs (export vehicles)',
    'shipdoc_select_all' => 'Select/clear all export vehicles on this page',
    'selected' => ':count selected',
    'max30' => 'Up to 30 vehicles per document',
    'clear_selection' => 'Clear selection',
    'shipdoc_license_link' => 'License n/1',
    'shipdoc' => [
        'container_invoice_packing' => 'Container Invoice & Packing',
        'container_contract' => 'Container Contract',
        'roro_invoice_packing' => 'RORO Invoice & Packing',
        'roro_contract' => 'RORO Contract',
        'sales_contract' => 'Sales Contract',
        'invoice' => 'Proforma Invoice',
    ],
    'sales_contract_homogeneous_hint' => 'Only vehicles with the same buyer and currency can be issued together.',

    // ── Cumulative search + bundle to shipping request (jin 2026-07-08) ──
    'accum' => [
        'title' => 'Cumulative search (add one by plate no. / VIN)',
        'hint' => 'Search the list by plate number / VIN and check vehicles in the results. Selected vehicles pile up as tags above and persist even when you change the search. When done, bundle them into a shipping request or print documents.',
        'placeholder' => 'Search by plate no. / VIN',
        'add_btn' => 'Add',
        'search_btn' => 'Search',
        'non_export' => 'Not export',
        'none' => 'No vehicle (in your scope) matches ":term".',
        'added_one' => 'Added 1 vehicle.',
        'added_many' => 'Added :count vehicles.',
        'bundle_btn' => 'Bundle to shipping request',
        'bundle_confirm' => 'Create one shipping-request bundle from the accumulated vehicles and open it in Shipping Requests. Continue?',
        'bundle_empty' => 'Add some vehicles first.',
        'bundle_all_skipped' => 'No eligible vehicles (export channel and not already in progress only). Skipped: :skipped',
        'bundle_created' => 'Shipping-request bundle created — :count vehicle(s). Proceed as usual in Shipping Requests.',
        'bundle_created_skipped' => 'Shipping-request bundle created — :count vehicle(s). Skipped :skip (not export / already in progress).',
    ],

    // ── Slide panel ─────────────────────────────────────────
    'panel' => [
        'cash_drawn' => 'Drawn from cash',
        'cash_drawn_line' => ':amount ← :date receipt of :total',
        'cash_drawn_sum' => 'total :sum from :count receipts',
        'cash_receipt_word' => 'receipt of',
        'buyer_cash_available' => 'Cash on hand for :buyer: :amount — sale balances are limited to this.',
        'buyer_cash_empty' => 'Record the incoming payment on the buyer Cash tab first.',
        'created_badge' => 'Created',
        'edit_next' => 'Edit mode — continue next step',
        'edit_title' => 'Edit Vehicle',
        'create_title' => 'Add Vehicle',
        'readonly_note' => '(edit mode — locked)',
        'lookup' => 'Look up',
        'lookup_ing' => 'Looking up…',
        'lookup_nice' => 'NICE lookup',
        'lookup_edit_hint' => 'Check the owner name, look up, then click Save to apply.',
        'owner_name_ph' => 'Owner name (required for NICE lookup)',
        'select_placeholder' => '-- Select --',
        'show' => 'Show',
        'hide' => 'Hide',
        'uploading' => 'Uploading…',
        'new' => 'New',
        'photo_count' => ':count/10 · click Save to apply.',
        'photo_multi_hint' => 'Select several photos/documents at once, or add more in separate picks — they accumulate. Each pick is limited to :pick files (40MB total), so split larger batches.',
        'tracking_open' => 'Open :company cargo tracking',
        'tracking_blocked' => [
            'too_early' => 'Available from the day after departure',
            'not_departed' => 'Available after departure',
            'no_vin' => 'Enter the chassis number to enable tracking',
        ],
        'open_new_tab' => 'Open in new tab',
        'validation_title' => 'Cannot save — please check the items below',
        'tab' => [
            'basic' => 'Basic Info',
            'purchase' => 'Purchase',
            'sale' => 'Sale',
            'shipping' => 'Shipping',
            'clearance' => 'Export Clearance',
            'bl' => 'B/L',
            'dhl' => 'EMS / DHL',
            'docs' => 'Documents',
        ],
        'flow' => [
            'purchase' => 'Purchase',
            'deregistration' => 'Deregister',
            'sale' => 'Sale',
            'payment' => 'Payment',
            'shipping' => 'Shipping',
            'clearance' => 'Clearance',
            'bl' => 'B/L',
            'dhl' => 'DHL',
        ],
        'sec' => [
            'basic' => 'Basic Info',
            'nice_reg' => 'NICE Registration (12)',
            'nice_spec' => 'NICE Specs (12)',
            'photos' => 'Photos / Attachments (max :max)',
            'ship_photos' => 'Vessel Photos / Attachments (max :max)',
            'purchase_basic' => 'Purchase Basics',
            'purchase_account' => 'Seller Account (remittance target)',
            'fee_account' => 'Selling-Fee Account (separate target)',
            'costs' => '9 Cost Items',
            'purchase_payment' => 'Purchase Payment',
            'sale_basic' => 'Sale Basics',
            'payment_status' => 'Payment Status',
            'shipping' => 'Shipping (Arrival)',
            'clearance' => 'Export Clearance',
            'bl' => 'B/L Issuance',
            'ems' => 'Korea Post EMS',
            'dhl' => 'DHL',
            'dhl_recipient' => 'DHL Recipient',
            'dhl_sender' => 'DHL Sender',
        ],

        // Clearance / Shipping / DHL tab misc
        'discharge_auto' => '-- auto by buyer country --',
        'discharge_note' => 'If unset, uses buyer country name',
        'export_decl_amount_note' => 'Usually equals invoice amount. Enter separately only if CIF/FOB differs.',
        'upload_enables_loaded' => '(uploading can reach Loaded status)',
        'g1' => [
            'no_price' => '⚠ Sell price missing — enter sale info (sell price) before issuing B/L',
            'override' => '⚠ Unpaid :ratio% — management/admin unpaid-bypass approval (B/L issuance) applied → B/L can be issued',
            'locked' => '🔒 B/L issuance locked — unpaid :ratio% (balance not 100% paid). Issue after full payment, or needs management/admin unpaid-bypass approval (B/L issuance stage).',
            'ok' => '✓ B/L can be issued — balance 100% paid',
        ],
        // C5 entry gate (clearance/shipping entry 50%) — shipping & clearance tab banner
        'c5' => [
            'rate_missing' => '⚠ Exchange rate missing — enter the rate to allow clearance/shipping entry for foreign-currency vehicles',
            'blocked' => '🔒 Clearance/shipping entry locked — paid :paid% (under 50%). Pay 50%+, or needs management/admin unpaid-bypass approval (clearance/shipping entry).',
            'override' => '⚠ Paid :paid% — management/admin unpaid-bypass approval (entry) applied → clearance/shipping entry allowed',
            'ok' => '✓ Clearance/shipping entry allowed — paid :paid% (50%+)',
            'bl_note' => 'B/L issuance requires 100% full payment (issue on the B/L tab).',
        ],

        // Sale tab misc
        'add_new' => '+ New',
        'buyer_first' => 'Select a buyer first',
        'inherit_from_sale' => '(auto from sale buyer)',
        'inherit_from_bl' => '(auto from B/L consignee)',
        'no_match' => 'No match',
        'fee_note' => '(seller pays)',
        'savings_deposit_note' => '(accumulates on save, resets)',
        'savings_balance_note' => '(SavingsStatus by buyer×currency)',
        'after_save_note' => '(updates after save)',
        'fully_paid' => '✓ Paid',
        'overpaid' => 'Overpaid · check',
        'overpay_correct' => 'Correct overpay',
        'overpay_correct_hint' => 'Correct to fully-paid (if mis-entry)',
        'overpay_reason_ph' => 'Reason (required)',
        'sale_overpay_hint' => 'Convert to savings in Receivables',
        'fee_account_hint' => 'Enter only if different from the purchase-price account. Leave blank to use the purchase-price account.',
        'purchase_unpaid_summary' => 'Purchase Balance Due',
        'purchase_total' => 'Total Purchase',
        'purchase_total_sub' => '(price + selling fee)',
        'purchase_paid' => 'Paid',
        'purchase_paid_sub' => '(down + fee payment + confirmed balances)',
        'purchase_unpaid' => 'Unpaid',
        'item_purchase_price' => 'Buy Price',
        'item_selling_fee' => 'Dealer Fee',
        'item_paid' => 'paid',
        'item_unpaid' => 'Unpaid',
        'sale_unpaid_summary' => 'Sale Balance Due',
        'sale_total' => 'Total Sale',
        'sale_total_sub' => '(price + surcharges)',
        'sale_paid' => 'Received',
        'sale_paid_sub' => '(down + interim + advance + confirmed balances)',
        'sale_unpaid' => 'Unpaid',
        'breakdown_banner' => 'Payments are unified under [Balance] N+. Enter down payment, interim and advance-1 as balances (their input fields were removed). Fee and savings are finance/admin only.',
        'legacy_breakdown' => 'Legacy payments (down payment / interim / advance-1 — input discontinued, still counted in receivables)',
        'transfer_applied' => 'Received via deposit application (not included in the totals above)',
        'transfer_applied_note' => 'Recorded by an inter-vehicle fund transfer. It cannot be edited or deleted here — correct it by voiding the transfer in the balance list.',
        'transfer_locked_pbp' => 'Deposit prepayment',
        'edit_in_receivables' => 'Edit in Receivables',
        'rate_at_payment' => 'Rate at payment',
        'krw_converted' => 'KRW converted (saved)',
        'transfer_out' => '→ transferred to vehicle :number',
        'transfer_in' => '← transferred from vehicle :number',
        'transfer_voided' => '(voided)',
        'transfer_pending_void' => '(void pending approval)',
        'void_requesting' => 'void requested',
        'void_request_btn' => 'Request transfer void',

        // Purchase tab misc
        'encrypted_note' => '🔒 encrypted',
        'auto_hyphen' => '— auto hyphen on bank select',
        'add' => '+ Add',
        'shipment_add' => '+ Add shipment',
        'shipment_total' => 'Shipping fee total',
        'shipment_empty' => 'No shipments recorded.',
        'shipment_hint' => 'Deducted in full from the settlement payout. Leave the amount at 0 when the company absorbs it — the tracking number still stays visible to the buyer.',
        'shipment_locked' => 'Secondary settlement is closed — shipments can only be edited after [🔓 ledger re-adjustment].',
        'finance_only_add' => 'finance/admin only',
        'confirmed' => '✓ Confirmed',
        'pending' => '⏳ Pending',
        'confirmed_title' => 'Finance confirmed: :at (:by)',
        'pending_title' => 'Awaiting finance confirmation — not in ledger',
        'proof' => 'Wire slip',
        'proof_attach' => 'Attach slip',
        'proof_pending_save' => '(attaches on save)',
        'proof_cancel' => 'Cancel',
        'proof_remove' => 'Remove',
        'proof_will_remove' => '(removes on save)',
        'before_save' => '(before save)',
        'secondary_pending_title' => '⏳ 2nd settlement pending — actuals adjustable',
        'secondary_pending_desc' => 'After one month, update to the actual billed costs. On save, total margin / settlement / net payout on the settlement screen update automatically.',
        'finance_area_desc' => 'The purchase funds area is editable by finance/admin only. When sales enter the buy price/account, a purchase balance Draft is auto-created; finance confirms it under the purchase balance tab at /erp/transfers.',
        'ledger' => [
            'unlocked_title' => 'Unlocked — auto re-locks after one save',
            'unlocked_desc' => 'Buy/sell price, rate, license amount, costs, buyer, salesman editable. Locks immediately on save.',
            'locked_title' => '2nd settlement finalized — accounting columns locked',
            'locked_desc' => 'The 2nd settlement is finalized, so buy/sell price, rate, license amount, 9 costs, buyer, salesman are locked. To correct them, unlock via [🔓 Readjust] on the settlement screen.',
            'unlock_btn' => 'Unlock',
            'readjust_hint' => 'Settlement screen → 🔓 Readjust',
        ],
    ],

    'field' => [
        'vehicle_number' => 'Vehicle No.',
        'brand' => 'Brand',
        'model_type' => 'Model',
        'year' => 'Year',
        'cc' => 'Displacement (cc)',
        'weight_kg' => 'Weight (kg)',
        'mileage' => 'Mileage (km)',
        'color' => 'Color',
        'salesman' => 'Salesman',
        'vin' => 'VIN',
        'engine_no' => 'Engine Type',
        'fuel_type' => 'Fuel Type',
        'use_type' => 'Use Type',
        'vehicle_form' => 'Body Type',
        'first_date' => 'First Reg. Date',
        'reg_date' => 'Reg. Date',
        'owner_name' => 'Owner Name',
        'owner_rrn' => 'Owner Resident/Corp Reg. No.',
        'max_load' => 'Max Load (kg)',
        'passengers' => 'Seating Capacity',
        'reg_color' => 'Color',
        'owner_addr' => 'Owner Address',
        'spec_maker' => 'Maker',
        'spec_model' => 'Model Name',
        'spec_year' => 'Year',
        'spec_displacement' => 'Displacement (cc)',
        'spec_transmission' => 'Transmission',
        'spec_drive_type' => 'Drivetrain',
        'spec_length' => 'Length (mm)',
        'spec_width' => 'Width (mm)',
        'spec_height' => 'Height (mm)',
        'spec_wheelbase' => 'Wheelbase (mm)',
        'spec_curb_weight' => 'Curb Weight (kg)',
        'spec_fuel_efficiency' => 'Fuel Economy (km/L)',
        'purchase_date' => 'Purchase Date',
        'purchase_from' => 'Source',
        'purchase_evidence_type' => 'Purchase Evidence',
        'purchase_partner_type' => 'Partner Type',
        'purchase_registration_type' => 'Purchase Registration',
        'purchase_evidence_subtype' => 'Evidence Type',
        'is_dealer_purchase' => 'Dealer',
        'buyer_undecided' => 'Buyer undecided (speculative purchase)',
        'buyer_undecided_hint' => 'Only tick this when buying before a buyer is decided. The vehicle is saved without a buyer and appears under Inventory → General stock. It clears automatically once a buyer is set.',
        'is_deposit_purchase' => 'Deposit purchase (paid with buyer deposit)',
        'is_deposit_purchase_hint' => 'Enabling this starts the buyer payment reminder alerts (day 5 to sales/management, past day 10 to the executive). They stop automatically once the buyer meets the required payment ratio.',
        'has_mortgage' => 'Mortgage',
        'has_mortgage_on' => 'Mortgaged',
        'has_mortgage_off' => 'No mortgage',
        'has_mortgage_hint' => 'When on, the payment notice includes a mortgage release request. Turn it off once released.',
        'is_unsecured_down' => 'Deposit paid from unsecured credit',
        'is_unsecured_down_hint' => 'Tick only when the company paid the deposit on behalf of the buyer. Leave it unticked if the buyer sent the money. Only ticked vehicles draw on the unsecured credit, and it is released once shipping entry is met.',
        'is_unsecured_down_locked' => 'Cannot untick — the deposit was already drawn from unsecured credit. It is released automatically once shipping entry is met.',
        'unsecured_in_use' => 'KRW :amount of this buyer deposit is drawn from unsecured credit. It is released once shipping entry is met.',
        'unsecured_covered_by_deposit' => 'Deposits are covered by the buyer deposit, so unsecured credit is untouched.',
        'unsecured_idle' => 'Not used yet. Deposits draw from here when the buyer deposit runs out.',
        'purchase_price' => 'Buy Price (KRW)',
        'selling_fee' => 'Dealer Fee (KRW)',
        'bank_name' => 'Bank',
        'account_holder' => 'Account Holder',
        'account_no' => 'Account No.',
        'gate_hint' => 'Advances stage on input',
        'account_memo' => 'Account Memo',
        'cost_deregistration' => 'Deregistration',
        'cost_license' => 'License',
        'cost_towing' => 'Towing',
        'cost_carry' => 'Carry',
        'cost_shoring' => 'Shoring',
        'cost_insurance' => 'Insurance',
        'cost_transfer' => 'Transfer',
        'cost_parking' => 'Parking Fee',
        'cost_extra1' => 'Other 1',
        // karaba-only labels (jin 2026-08-26) — picked by Vehicle::costLabel().
        'cost_extra1_karaba' => 'Inspection Fee',
        'cost_extra2_karaba' => 'Other Cost',
        'cost_extra2' => 'Other 2',
        'cost_inspection' => 'Inspection',
        'cost_performance' => 'Performance Check',
        'cost_repair' => 'Maintenance',
        'cost_advertising' => 'Advertising',
        'parts_amount' => 'Parts',
        'purchase_vat_amount' => 'Purchase VAT',
        'down_payment' => 'Down Payment',
        'selling_fee_payment' => 'Dealer Fee Paid',
        'balance' => 'Balance',
        'deregistered' => 'Deregistered',
        'derg_doc' => 'Deregistration Certificate',
        'remittance_memo' => 'Remittance Memo',
        'registration_number' => 'Registration No.',
        'registration_number_ph' => 'e.g. 2024-12345',
        'registration_number_hint' => 'Fills Clearance SET 구매리스트 → Deregistration cert. "No. ○○"',
        'reg_cert_number' => 'Vehicle Registration Cert. No.',
        'reg_cert_number_ph' => 'Registration no. on the vehicle registration certificate',
        'reg_cert_number_hint' => 'Fills Clearance SET 구매리스트 → KR/EN registration cert "자동차등록번호" (separate from purchase Registration No.)',
        'deregistration_date' => 'Deregistration Date',
        'deregistration_date_hint' => 'Date the deregistration certificate was issued (used in Clearance SET 구매리스트)',
        'sale_date' => 'Sale Date',
        'currency' => 'Currency',
        'exchange_rate' => 'Exchange Rate',
        'buyer' => 'Buyer',
        'consignee' => 'Consignee',
        'sale_price' => 'Sell Price',
        'transport_fee' => 'Freight',
        'transport_fee_usd' => 'Freight (USD)',
        'transport_fee_usd_hint' => 'memo only · not calculated',
        'auto_loading' => 'Loading & Other',
        'sale_other_costs' => 'Other Sale Costs',
        'sale_total_display' => 'Total Sale',
        'deposit_down' => 'Down Payment',
        'interim' => 'Interim',
        'advance1' => 'Advance 1',
        'fee' => 'Remittance Fee',
        'savings_used' => 'Savings Used',
        'savings_deposit' => 'Savings Deposit',
        'savings_balance' => 'Savings Balance',
        'unpaid_ratio' => 'Unpaid %',
        'remaining_balance' => 'Remaining Balance',
        'export_buyer' => 'Clearance Buyer',
        'export_consignee' => 'Clearance Consignee',
        'forwarder' => 'Forwarder',
        'export_decl_amount' => 'Declaration Amount',
        'export_decl_number' => 'Export Decl. No.',
        'shipping_date' => 'Shipping Date',
        'shipping_method' => 'Shipping Method',
        'port_loading' => 'Port of Loading',
        'incoterms' => 'Incoterms',
        'discharge_port' => 'Discharge Port',
        'export_cleared' => 'Export Cleared',
        'export_decl_doc' => 'Export Declaration',
        'bl_buyer' => 'B/L Buyer',
        'bl_consignee' => 'B/L Consignee',
        'bl_number' => 'B/L No.',
        'container_number' => 'Container No.',
        'loading_location' => 'Loading Location',
        'vessel' => 'Vessel (VSL)',
        'bl_issue_date' => 'B/L Date',
        'document_deadline' => 'Doc Deadline',
        'document_deadline_hint' => 'Alerts [Management] from 5 days before',
        'bl_doc' => 'B/L Document',
        'checkbill_doc' => 'Check B/L',
        'checkbill_note' => 'Draft for review before final B/L issuance — separate from final B/L',
        'ems_tracking_no' => 'Registered No.',
        'dhl_tracking_no' => 'Waybill No.',
        'ems_fee' => 'Postage',
        'dhl_fee' => 'Billed amount',
        'shipment_sent_date' => 'Sent date',
        'dhl_recipient_name' => 'Recipient Name',
        'dhl_recipient_phone' => 'Recipient Phone',
        'dhl_recipient_address' => 'Recipient Address',
        'dhl_sender_name' => 'Sender Name',
        'dhl_sender_address' => 'Sender Address',
        'dhl_weight' => 'Weight (kg)',
        'dhl_dimensions' => 'Size (W×H×L cm)',
        'dhl_request_done' => 'DHL dispatch requested',
        'memo' => 'Memo',
        'memo_common' => 'Memo (shared across all tabs)',
    ],

    // Progress-flow chip hover reasons (panel header)
    'reason' => [
        'purchase_warn' => 'Purchase balance :amount KRW outstanding',
        'purchase_pending' => 'Buy price not entered',
        'dereg_warn' => 'Deregistration checked but certificate not uploaded',
        'dereg_pending' => 'Not deregistered — check Deregistered + upload certificate',
        'sale_pending' => 'Sell price not entered',
        'payment_warn' => 'Sale unpaid :amount KRW outstanding',
        'payment_pending' => 'Sell price not entered — payment cannot be tracked',
        'clearance_progress' => 'Export declaration upload needed (checkbox alone does not progress the step)',
        'clearance_pending' => 'Clearance info missing — enter clearance buyer/shipping date/forwarder, then upload export declaration',
        'shipping_pending' => 'Loading location not set — pick it on the Shipping tab',
        'bl_progress' => 'B/L document upload needed (arrival & clearance done)',
        'bl_pending' => 'B/L not issued — after arrival & clearance, upload the B/L document on the B/L tab',
        'dhl_pending' => 'DHL dispatch not checked',
    ],

    // Concurrent-edit lock (2026-06-30)
    'lock' => [
        'unsecured_check_required' => ':buyer is exempt from the purchase lock via an unsecured credit limit (current unpaid ratio :ratio%). To record a down payment, turn on "Paid unsecured" on the Purchase tab — only then is the limit drawn down, and it is released automatically once sales payments meet the shipping-entry threshold. Leaving it off means the limit never decreases and this buyer is never locked.',
        'someone' => 'Another user',
        'locked_by' => ':name is currently editing — you cannot make changes.',
        'banner' => ':name is editing this vehicle — opened read-only (auto-releases shortly after they leave).',
        'now_editable' => 'The edit lock was released — you can edit now.',
    ],

    // PHP toasts / flash / validation messages
    'overpay' => [
        'reason_required' => 'Please enter a correction reason.',
        'not_overpaid' => 'Not an overpayment.',
        'exceeds_confirmed' => 'Overpayment exceeds total confirmed purchase balance. Please check individually in Finance.',
        'failed' => 'Overpayment correction failed.',
        'done' => 'Corrected overpayment of :amount (marked fully paid).',
    ],

    'cancel' => [
        'section_title' => 'Purchase Cancellation',
        'hint' => 'Enter the penalty as the sale price on the Sale tab. Fully paid = cancellation complete; unpaid = close as receivable loss.',
        'mark_btn' => 'Mark as cancelled',
        'unmark_btn' => 'Undo cancellation',
        'badge_active' => 'Cancelled',
        'badge_done' => 'Cancel complete',
        'badge_closed' => 'Closed unpaid',
        'marked' => 'Marked as purchase cancellation. Enter the penalty as sale price on the Sale tab.',
        'unmarked' => 'Purchase cancellation undone.',
        'closed_no_unmark' => 'A closed (unpaid) cancellation cannot be undone.',
        'close_btn' => 'Close unpaid',
        'close_confirm' => 'Give up collecting the penalty and close. The shortfall becomes a confirmed loss and the freelancer bears half. Proceed?',
        'close_only_active' => 'Only an active cancellation can be closed.',
        'close_no_unpaid' => 'No unpaid amount. A fully-paid cancellation cannot be closed.',
        'closed' => 'Closed unpaid. Loss of :amount confirmed (freelancer bears half).',
        'closed_loss' => 'Closed unpaid · loss :amount confirmed (freelancer half deducted in monthly batch loss summary)',
    ],

    'cancel_filter' => [
        'all' => 'All',
        'active' => 'Cancelled (unpaid)',
        'done' => 'Cancel done',
        'closed' => 'Closed unpaid',
        'normal' => 'Normal only',
    ],

    'toast' => [
        'db_constraint' => 'A data constraint error occurred while saving. Please check your inputs (sale date, buyer, exchange rate, amounts).',
        'file_store_failed' => 'File upload failed, so the save was cancelled. Existing files are untouched — please try again.',
        'unsecured_lock_kept' => 'The unsecured flag was kept — the deposit is already drawn. It is released once shipping entry is met.',
        'buyer_first' => 'Select a buyer first',
        'buyer_created' => 'New buyer created',
        'consignee_created' => 'New consignee created',
        'fields_restored' => ':count accounting-sensitive field(s) restored to original (no permission; admin/sales only)',
        'override_no_perm' => 'No permission for unpaid-bypass approval.',
        'save_first_approve' => 'Save the vehicle first before approving.',
        'override_done' => 'Unpaid bypass approved — this step can now proceed',
        'edit_own_only' => 'You can only edit your own vehicles.',
        'edit_team_only' => 'You can only edit your team\'s vehicles.',
        'unlock_no_perm' => 'No unlock permission (super/admin or owning-team manager only)',
        'unlock_failed' => 'Unlock failed: :error',
        'unlock_done' => 'Unlocked. Auto re-locks after one save.',
        'create_perm' => 'New vehicle registration is allowed only for sales/management role or admin/super.',
        'created_next' => 'Vehicle created. Next step: :label — :reason',
        'created' => 'Vehicle created.',
        'updated' => 'Vehicle updated.',
        'enter_vnum_first_lock' => 'Enter the vehicle number first. (Approval is locked per vehicle number)',
        'select_buyer_first' => 'Select a buyer first.',
        'enter_vnum_first' => 'Enter the vehicle number first.',
        'pending_request_exists' => 'A pending request already exists.',
        'overlap_sent' => 'New-deal approval request sent.',
        'save_first_transfer' => 'Save the vehicle first before requesting a transfer.',
        'pending_transfer_exists' => 'A pending fund-transfer request already exists. Retry after management approves/rejects.',
        'transfer_vehicle_not_found' => 'Transfer source or target vehicle not found.',
        'transfer_sent' => 'Inter-vehicle fund-transfer approval request sent.',
        'void_only_executed' => 'Only executed transfers can be voided.',
        'transfer_not_found' => 'Transfer not found.',
        'void_sent' => 'Transfer void approval request sent.',
        'nice_need_owner' => 'Enter the owner name first. (NICE lookup needs vehicle number + owner name)',
        'nice_not_configured' => 'NICE lookup is not configured — enter manually.',
        'nice_failed' => 'NICE lookup failed.',
        'nice_success' => 'NICE lookup done — :count registration/spec fields auto-filled. (Save to apply)',
        'paid_locked' => "Accounting columns of a 'paid' settlement vehicle cannot be changed (attempted: :fields). For a closed settlement, unlock it first with [🔓 Readjust] on the settlement screen.",
        'max_photos' => 'Up to :max attachments can be added.',
        'rrn_required' => 'Deregistration requires the owner resident/corp registration number — Basic Info tab.',
        'dup_vehicle' => 'Vehicle number (:value) is already registered as vehicle #:id.:hint',
        'dup_hint' => ' (this may be the vehicle you just saved — check the vehicle list)',
        'rrn_format' => 'Resident/corp registration number must be in 000000-0000000 format.',
    ],

    // validation :attribute display names
    'attr' => [
        'vehicle_number' => 'Vehicle No.',
        'sales_channel' => 'Sales Channel',
        'currency' => 'Currency',
        'shipping_method' => 'Shipping Method',
        'purchase_date' => 'Purchase Date',
        'sale_date' => 'Sale Date',
        'shipping_date' => 'Shipping Date',
        'eta_date' => 'ETA',
        'bl_issue_date' => 'B/L Date',
        'nice_reg_first_date' => 'First Reg. Date',
        'nice_reg_date' => 'Reg. Date',
        'salesman' => 'Salesman',
        'buyer' => 'Sale Buyer',
        'consignee' => 'Sale Consignee',
        'export_buyer' => 'Export Buyer',
        'export_consignee' => 'Export Consignee',
        'forwarder' => 'Forwarder',
        'bl_buyer' => 'B/L Buyer',
        'bl_consignee' => 'B/L Consignee',
        'derg_doc' => 'Deregistration Doc',
        'export_decl_doc' => 'Export Declaration',
        'bl_doc' => 'B/L Document',
        'checkbill_doc' => 'Check B/L',
        'year' => 'Year',
        'cc' => 'Displacement',
        'weight' => 'Weight',
        'mileage' => 'Mileage',
        'purchase_price' => 'Buy Price',
        'selling_fee' => 'Dealer Fee',
        'sale_price' => 'Sell Price',
        'exchange_rate' => 'Exchange Rate',
        'export_decl_amount' => 'Declaration Amount',
        'dhl_weight' => 'DHL Weight',
        'owner_rrn' => 'Owner Resident/Corp Reg. No.',
        'purchase_balance_amount' => 'Purchase balance amount',
        'purchase_balance_date' => 'Purchase balance payment date',
        'final_payment_amount' => 'Sale balance amount',
        'final_payment_date' => 'Sale balance payment date',
    ],

    // Panel footer / save bar
    'footer' => [
        'cancel' => 'Cancel',
        'close' => 'Close',
        'save_edit' => 'Save Changes',
        'save_continue' => 'Save & Keep Open',
        'save_create' => 'Create',
        'saving' => 'Saving...',
        'processing' => 'Processing...',
    ],

    // Unpaid-bypass approval footer
    'override' => [
        'title' => 'Management/Admin — Unpaid Bypass Approval',
        'unpaid_left' => 'Outstanding:',
        'none' => 'none',
        'won' => 'KRW',
        'existing' => ':count existing approval(s):',
        'existing_badge' => ':count approved',
        'stage_label' => 'Stage',
        'stage_select' => 'Select',
        'stage_entry' => 'Clearance·Shipping entry (50% paid)',
        'stage_bl' => 'B/L issuance (100% paid)',
        'reason_label' => 'Reason (20+ chars)',
        'reason_ph' => 'e.g. Forced due to container departure schedule. Balance payment confirmed for 5/20.',
        'approve_btn' => 'Record Approval',
    ],

    // Receivable purchase gate (2026-07-08) — block new registration when buyer's unpaid ratio is too high
    'purchase_gate' => [
        'title' => 'Outstanding too high — registration blocked',
        'subtitle' => 'This buyer still owes a large amount, so new vehicle registration is blocked.',
        'unpaid' => 'Outstanding ₩:amount · :count vehicles',
        'ratio' => 'Unpaid :pct%',
        'deposit' => 'Deposit limit ₩:limit (:pct% of pre-ship total ₩:total) exceeded — unpaid over limit, registration blocked',
        'unsecured_exhausted' => 'Unsecured credit exhausted — new registration blocked',
        'unsecured_title' => 'Unsecured credit exhausted — registration blocked',
        'unsecured_subtitle' => 'This buyer has used up the credit granted, so new vehicle registration is blocked.',
        'base_limit' => 'Base limit (received × :pct%)',
        'unsecured_limit' => 'Unsecured credit',
        'total_limit' => 'Total limit',
        'used' => 'Used (confirmed purchase payments)',
        'reason' => 'Approval reason',
        'reason_ph' => 'e.g. L/C confirmed. Balance payment expected 5/20.',
        'need_manager' => 'Manager approval is required to proceed. Collect the outstanding amount and retry.',
        'approve_btn' => 'Approve & register',
    ],

    // Deposit apply (jin 2026-07-20)

    // Purchase funding via deposit (C2, jin 2026-07-21) — fund this car's KRW purchase cost with the buyer's deposit

    // Delete-reason modal (2026-07-08) — finance-linked vehicles require a reason + audit log
    'delete_gate' => [
        'title' => 'Delete finance-linked vehicle',
        'ctx_locked' => 'This vehicle has confirmed balance payments (accounting lock).',
        'ctx_settlement' => 'This vehicle has settlement records.',
        'target' => 'About to delete :number. Your reason will be recorded.',
        'reason' => 'Reason for deletion',
        'reason_ph' => 'e.g. Duplicate registration error; not a real purchase.',
        'confirm_btn' => 'Record reason & delete',
        'admin_only' => 'Only a super admin can delete a vehicle with confirmed balance payments.',
    ],

    // Per-vehicle cost statement bulk entry (towing etc., matched by vehicle no.)
    'ship_import' => [
        'btn' => 'EMS/DHL entry',
        'title' => 'Bulk entry — EMS / DHL shipments',
        'carrier' => 'Carrier',
        'tracking_no' => 'Tracking no.',
        'sent_date' => 'Sent date',
        'total' => 'Total fee (KRW)',
        'total_hint' => 'Split across the target vehicles (remainder goes to the first).',
        'targets' => 'Target vehicles — VIN or plate number',
        'targets_hint' => 'You can paste the VIN cell straight from the tracking sheet (space/newline separated).',
        'from_selection' => ':count checked vehicle(s) loaded',
        'preview' => 'Preview',
        'apply' => 'Apply',
        'per_unit' => 'per vehicle',
        'will_write' => ':count to write',
        'unchanged' => ':count unchanged',
        'removed' => ':count dropped from this number',
        'skipped' => ':count skipped',
        'unmatched' => ':count value(s) not found — these will not be written',
        'reason_not_found' => 'vehicle not found',
        'reason_no_scope' => 'no permission',
        'reason_settlement_closed' => 'secondary settlement closed',
        'done' => ':applied written · :unchanged unchanged · :removed dropped · :skipped skipped',
        'empty_plan' => 'Press Preview first to see the targets.',
    ],
    'cost_import' => [
        'btn' => 'Import Statement',
        'title' => 'Bulk Cost Entry from Statement',
        'subtitle' => 'Paste a vendor statement (e.g. Wika); vehicles are matched by number and the cost is filled at once. Management/admin · all vehicles · costs only.',
        'target_col' => 'Target cost',
        'col_hint' => 'Per-vehicle costs like towing. (License fee uses n/1 on the shipping bundle screen.)',
        'file_label' => 'Upload Excel file (vendor statement .xlsx)',
        'file_choose' => 'Choose Excel file',
        'file_btn' => 'Read file',
        'paste_label' => 'Or paste (copy from Excel)',
        'paste_ph' => "Paste lines containing a vehicle number and amount. e.g.\n393어3064  SM6  25000  10000  35000\n14주4848  AUDI  45000  20000  65000",
        'parse_btn' => 'Preview',
        'parse_empty' => 'No vehicle number found. Paste lines that include a vehicle number.',
        'file_error' => 'Could not read the Excel file. Make sure it is .xlsx.',
        'apply_failed' => 'Bulk entry failed: :msg',
        'matched' => ':count matched',
        'finalized' => ':count settled (excluded)',
        'finalized_badge' => 'settled',
        'protected' => 'unchanged',
        'finalized_suffix' => '· :count settled excluded',
        'unmatched' => ':count unmatched',
        'unmatched_title' => '⚠ Vehicle numbers not in ERP (typo/unregistered — check manually)',
        'current' => 'Current',
        'new_amount' => 'New',
        'no_matched' => 'No matched vehicles.',
        'applied' => 'Cost entered for :count vehicle(s)',
        'unchanged_suffix' => '(:count unchanged)',
        'apply_btn' => 'Apply',
        // License fee (customs license monthly statement — split total n/1 by export declaration number)
        'lic_col_hint' => 'Customs license-fee statement (monthly). Vehicles are grouped by export declaration number and the total is split n/1.',
        'lic_file_label' => 'Upload Excel file (customs license-fee monthly statement .xlsx)',
        'lic_parse_empty' => 'No export declaration number found. Make sure this is a license-fee statement (with declaration no., qty, total).',
        'lic_decl' => 'Declaration No.',
        'lic_carname' => 'Item',
        'lic_qty' => 'Qty (file)',
        'lic_matched' => 'ERP matched',
        'lic_total' => 'Total',
        'lic_per' => 'n/1 (each)',
        'lic_mismatch_hint' => '⚠ File qty differs from ERP matches. Per-vehicle amount is split by the file qty (applied only to matched vehicles). Check for missing declaration numbers.',
        'lic_unmatched_title' => '⚠ Declaration numbers not in ERP (number not entered on the vehicle — enter during customs stage)',
        // Vendor (form layout) — each company uses a different Excel layout, so parsers branch by coordinates.
        'company_label' => 'Vendor',
        'company' => [
            'wika' => 'Wika',
            'gucheonyuk' => 'Gucheonyuk',
            'hyundai_a1' => 'Hyundai A1',
            'mutual' => 'Mutual',
            'seongji' => 'Seongji',
        ],
        'seongji_notice' => 'Seongji license fees are infrequent (1-2/year), so instead of document mapping, enter the total in the shipping request "Secondary cost" tab to split n/1 across the bundle. (Handled from bundles pending secondary settlement.)',
        'seongji_goto' => 'Go to shipping requests',
    ],

    // Modals
    'modal' => [
        'cancel' => 'Cancel',
        'not_entered' => '(not entered)',
        // Close confirm
        'close_title' => 'You have unsaved changes',
        'close_body' => 'Closing without saving discards your changes.',
        'close_discard' => 'Close',
        // Doc-check mismatch
        'doc_title' => '⚠ Step entry check',
        'doc_desc' => 'For the items below, only one of the checkbox / document upload is done, so the step will not progress.',
        'doc_checked' => '✓ Checkbox: checked',
        'doc_unchecked' => '☐ Checkbox: unchecked',
        'doc_uploaded' => '✓ Document: uploaded',
        'doc_not_uploaded' => '☐ Document: not uploaded',
        'doc_checked_no_doc' => '→ Checked but no document, so ":label complete" step does not progress',
        'doc_doc_no_check' => '→ Document uploaded but not checked, so ":label complete" step does not progress',
        'doc_goto' => '→ Go to :label tab to fix',
        'doc_note' => 'On save, the above steps stay at the current step. Fill in the check/document and save again to progress.',
        'doc_cancel' => 'Cancel (go fix it)',
        'doc_save_anyway' => 'Save as is (aware steps not progressed)',
        // Future-date confirmation (jin 2026-07-21)
        'future_date_title' => 'Confirm future date',
        'future_date_today' => 'Today is :today. Are the dates below correct?',
        'future_date_note' => 'Purchase/sale dates change when amounts (receivable, payable, settlement) take effect. Please confirm the future date is correct (e.g. year typo).',
        'future_date_fix' => 'Fix date',
        'future_date_confirm' => 'Yes, save',
        // Ledger unlock
        'ledger_title' => '🔓 Ledger Unlock',
        'ledger_desc' => 'Buy/sell price, rate, license amount, 9 costs, buyer, salesman can be changed once. Auto re-locks right after save.',
        'ledger_warn' => '⚠️ The unlock is recorded in audit_logs with user, time and reason.',
        'ledger_reason_label' => 'Unlock reason',
        'ledger_reason_hint' => '(10+ chars)',
        'ledger_reason_ph' => 'e.g. Sales mis-entered buy price — entered 5,000,000 but actual contract 50,000,000. Confirmed with the salesman.',
        'ledger_submit' => '🔓 Unlock (allow 1 change)',
        // Overlap approval request
        'overlap_title' => 'New-deal Approval Request',
        'overlap_desc' => 'Notify an admin of the reason and get approval. Approval is locked per (buyer × vehicle number).',
        'overlap_vnum' => 'Vehicle No.:',
        'overlap_buyer_unpaid' => 'Buyer outstanding:',
        'overlap_unit' => '',
        'overlap_unpaid_vehicles' => 'Outstanding vehicles:',
        'overlap_reason_label' => 'Approval request reason',
        'overlap_reason_ph' => 'e.g. This buyer outstanding is due next month; the new vehicle has a separate 50% advance. Min 5 chars.',
        'overlap_send' => 'Send request',
        // Fund transfer request modal
        'transfer_req_title' => 'Inter-vehicle Fund Transfer Request',
        'transfer_req_desc' => 'After admin approval, a negative balance is created on the source vehicle and a positive balance on the target.',
        'sell_price_label' => 'sell price',
        'transfer_amount_label' => 'Transfer Amount',
        'transfer_amount_ph' => 'max :max :currency',
        'transfer_reason_ph' => 'e.g. Source vehicle 50% received. Same buyer contracted the target — transfer as down payment. Min 5 chars.',
        'memo_optional' => 'Memo',
        'optional' => '(optional)',
        'transfer_notes_ph' => 'Memo to attach to the transfer',
        // Transfer void modal
        'void_title' => 'Transfer Void Request',
        'void_desc' => 'After admin approval, opposite-sign balances are added to both vehicles so the original transfer is offset in accounting. Existing balances are not deleted (append-only).',
        'source' => 'Source',
        'target' => 'Target',
        'amount' => 'Amount',
        'void_reason_label' => 'Void reason',
        'void_reason_ph' => 'e.g. Buyer cancelled the contract on vehicle 2. Funds need to revert to vehicle 1. Min 5 chars.',
        'void_send' => 'Send void request',
        // quick-add (inline buyer/consignee)
        'qa_buyer_title' => 'New Buyer',
        'qa_consignee_title' => 'New Consignee',
        'qa_ctx_sale' => 'Sale',
        'qa_ctx_export' => 'Export Clearance',
        'qa_ctx_bl' => 'Shipping',
        'qa_consignee_note' => 'Belongs to buyer :buyer.',
        'qa_buyer_name' => 'Buyer Name',
        'qa_consignee_name' => 'Consignee Name',
        'qa_country' => 'Country',
        'qa_contact' => 'Contact',
        'qa_phone' => 'Phone',
        'qa_more' => 'Email, address, memo and other details can be added later in the Buyer/Consignee screens.',
        'qa_save' => 'Add + Select',
    ],

    // Save confirm modal (sales/management save preview)
    'save' => [
        'tab_default' => 'Save',
        'title' => 'Confirm :tab info',
        'desc' => 'Check the items below are correct, then click [Confirm & Save].',
        'note' => 'You can save even with missing items (— not entered), but please double-check.',
        'back' => 'Go back & edit',
        'confirm' => 'Confirm & Save',
        'won' => 'KRW',
        'val_empty' => '— not entered',
        'val_unselected' => '— not selected',
        'val_attached' => '✓ attached',
        'val_not_attached' => '— not attached',
        'val_checked' => '✓ done',
        'val_unchecked' => '— unchecked',
        'preview_purchase' => 'Purchase Info',
        'preview_sale' => 'Sale Info',
        'preview_bl' => 'Shipping (B/L) Info',
        'preview_clearance' => 'Export Clearance Info',
        'f' => [
            'purchase_date' => 'Purchase Date',
            'purchase_from' => 'Source',
            'purchase_price' => 'Buy Price',
            'selling_fee' => 'Dealer Fee',
            'bank' => 'Bank',
            'holder' => 'Account Holder',
            'account' => 'Account No.',
            'sale_date' => 'Sale Date',
            'currency' => 'Currency',
            'rate' => 'Rate',
            'sale_price' => 'Sell Price',
            'buyer' => 'Buyer',
            'bl_buyer' => 'B/L buyer',
            'consignee' => 'Consignee',
            'loading' => 'Loading Location',
            'bl_number' => 'B/L No.',
            'container' => 'Container No.',
            'vsl' => 'VSL',
            'bl_doc' => 'B/L Document',
            'exp_buyer' => 'Clearance Buyer',
            'ship_date' => 'Shipping Date',
            'forwarder' => 'Forwarder',
            'decl_amount' => 'Declaration Amount',
            'decl_doc' => 'Export Declaration',
            'cleared_check' => 'Clearance Done Check',
        ],
    ],

    // Documents tab
    'docs' => [
        'save_first' => 'Save the vehicle first to generate documents.',
        // PDF/print output modal (Excel stays as card downloads below)
        'output_pdf' => 'Get PDF',
        'output_print' => 'Print',
        'output_hint' => 'Excel = download from cards below · here = PDF/print',
        'output_modal_pdf' => 'Select a document for PDF',
        'output_modal_print' => 'Select a document to print',
        'output_generating' => 'Generating PDF… please wait (about 10-20s)',
        'sec_purchase' => 'Purchase Documents (4)',
        'sec_sale' => 'Sale Documents (export)',
        'sec_shipping' => 'Shipping Documents (4, export)',
        'sec_clearance' => 'Clearance Documents',
        'multi_hint' => 'To include multiple vehicles in one document, select them with checkboxes in the vehicle list, then use the "Shipping Docs" button at the top.',
        'note_pdf' => '※ PDFs open in a new tab and download automatically. Excel downloads immediately.',
        'note_empty' => '※ For blank items, fill in the vehicle info and regenerate.',
        'deregistration' => 'Vehicle Deregistration Form',
        'deregistration_sub' => 'Form No. 17 · .xlsx',
        'deregistration_set' => 'Deregistration Form + Contract',
        'deregistration_set_sub' => '1 file, 2 tabs (form · contract) · .xlsx',
        'derg_contract' => 'Deregistration Contract',
        'poa' => 'Power of Attorney',
        'deregistration_certificate' => 'Deregistration Certificate (KO·EN)',
        'deregistration_certificate_sub' => '1 file, 2 tabs (Korean · English) · .xlsx',
        'transfer_certificate' => 'Vehicle Transfer Certificate',
        'transfer_certificate_sub' => 'Form No. 16 · company as transferee · .xlsx',
        'invoice' => 'Proforma Invoice',
        'sales_contract' => 'Sales Contract',
        'container_invoice_packing' => 'Container Invoice & Packing',
        'container_contract' => 'Container Contract',
        'roro_invoice_packing' => 'RORO Invoice & Packing',
        'roro_contract' => 'RORO Contract',
        'clearance_set' => 'Clearance SET (reg./dereg. cert., invoice, packing)',
        'clearance_set_sub' => '1 purchase list → 8 sheets auto-linked · .xlsx',
        'sub_purchase' => 'Purchase · .xlsx',
        'sub_sale' => 'Sale · .xlsx',
        'sub_container' => 'Container shipping · .xlsx',
        'sub_roro' => 'RORO shipping · .xlsx',
    ],

    // Vehicle registry lookup (carmodoo)
    'wonbu' => [
        'unknown' => 'unknown',
        'unknown_note' => 'Could not read the lien counts from the lookup page. This is not zero — it means unverified. Check the association site directly.',
        'button' => 'Registry lookup',
        'loading' => 'Looking up…',
        'modal_title' => 'Vehicle Registration Record',
        'need_plate' => 'Enter a vehicle number first.',
        'failed' => 'Registry lookup failed.',
        'seizure' => 'Seizure',
        'mortgage' => 'Mortgage',
        'structure' => 'Structural',
        'liens' => 'Seizure · Mortgage · Structural details',
        'lien_type' => 'Type',
        'lien_date' => 'Registered',
        'lien_info' => 'Details (mgmt no. / creditor / claim amount)',
        'detail' => 'Registration record specs',
        'disclaimer' => 'Real-time lookup from the dealer association system. May differ from the registration authority; use as reference only. Not stored.',
    ],

    // Mail delivery (Documents tab → send uploaded documents to buyer)
    'mail' => [
        'section' => 'Send mail',
        'section_hint' => 'Email documents uploaded to this vehicle to the buyer. Configure the sender account in Feature Settings.',
        'open_btn' => 'Send mail',
        'modal_title' => 'Email documents to buyer',
        'to_label' => 'Recipient (buyer email)',
        'subject_label' => 'Subject',
        'body_label' => 'Message',
        'body_ph' => 'Enter the message to send to the buyer.',
        'docs_label' => 'Attachments (choose from uploaded documents)',
        'no_docs' => 'No documents uploaded for this vehicle. Upload in the Basic Info tab first.',
        'send_btn' => 'Send',
        'sending' => 'Sending…',
        'sent' => 'Email sent.',
        'send_failed' => 'Failed to send email. Check the sender settings (Feature Settings).',
        'to_invalid' => 'Enter a valid email address.',
        'not_configured' => 'Sender email is not configured. A super admin must set it up in Feature Settings → Mail delivery first.',
        'group_generated' => 'Auto-generated documents (Documents tab)',
        'group_file' => 'Stage-uploaded files',
        'group_upload' => 'Uploaded photos/attachments (Basic Info)',
        'group_ship' => 'Vessel photos/attachments (Shipping)',
        'file_deregistration' => 'Deregistration document',
        'file_export_declaration' => 'Export declaration',
        'file_bl' => 'B/L document',
        'gen_hint' => 'Auto-generated documents are freshly created from the latest vehicle data and attached as xlsx at send time.',
        'gen_failed' => 'An error occurred while generating a document. Check the vehicle data.',
        'body_default' => "Dear Customer,\n\nPlease find the attached document(s) for your vehicle.\nThank you for doing business with :company — it was a pleasure serving you, and we look forward to working with you again.\n\nBest regards,\n:company Team",
    ],

    // Deregistration certificate AlimTalk delivery to domestic buyer (purchase tab)
    'deregnotice' => [
        'hint_paid' => 'The [Notify] buttons next to the down payment and balance rows also use this number.',
        'label' => 'Deregistration cert. AlimTalk (domestic dealer)',
        'hint' => 'Sends a link to the deregistration certificate via KakaoTalk AlimTalk. (Link expires in 3 days · configure AlimTalk in Feature Settings)',
        'send_btn' => 'Send AlimTalk',
        'sent' => 'Deregistration AlimTalk sent.',
        'no_doc' => 'Upload and save the deregistration certificate first.',
        'no_phone' => 'Enter the dealer phone number.',
        'save_first' => 'Save the vehicle first, then you can send the AlimTalk.',
        'failed' => 'AlimTalk send failed: :reason (check Feature Settings → AlimTalk)',
    ],

    // Dealer purchase-payment notice (jin 2026-08-12) — shares the phone field above.
    'paidnotice' => [
        'btn' => '📨 Notify',
        'btn_title' => 'Notify the domestic dealer by KakaoTalk that this amount was paid',
        'kind_down' => 'Down payment',
        'kind_balance' => 'Purchase balance',
        'confirm' => 'Send the dealer a KakaoTalk notice that :kind of :amount KRW was paid. Press only after the transfer is actually made. Continue?',
        'sent' => 'Payment notice sent. (:amount KRW)',
        'no_phone' => 'Enter the dealer phone number in the deregistration notice field above first.',
        'not_confirmed' => 'No finance-confirmed payment yet. It can be sent once confirmed.',
        'save_first' => 'Save the vehicle before sending the notice.',
        'failed' => 'Notice failed: :reason (check Settings -> KakaoTalk)',
        'mortgage_unsaved' => 'The mortgage flag was changed but not saved yet. Save first, then send.',
        'disabled' => 'The KakaoTalk template is not configured yet. (Settings -> KakaoTalk)',
        'also_sent' => 'Also sent to :count internal recipient(s).',
        'sales_failed' => 'Sending to the internal recipients failed.',
        'sales_no_phone' => 'No salesperson phone number, so it was sent to the dealer only.',
        'mortgage_pending' => 'The mortgage notice was not included (template pending approval).',
        'confirm_mortgage_on' => ' * The mortgage release request will be included.',
        'confirm_mortgage_pending' => ' * Marked as mortgaged, but the notice is not sent yet (template pending approval).',
        'confirm_sales' => ' * The assigned salesperson and selected internal recipients receive the same message.',
    ],

    // Same-buyer outstanding banner (sale tab)
    'overlap' => [
        'title' => ':count vehicle(s) outstanding for this buyer (₩:amount)',
        'approved_match' => '✓ Admin approved (vehicle :number). Saving will register a new vehicle (this one time only).',
        'pending' => '⏳ Vehicle :number awaiting admin approval. You can save after approval.',
        'mismatch_approved' => 'The currently approved vehicle number is :number.',
        'mismatch_blocked' => 'Saving is blocked for the entered :number.',
        'mismatch_hint' => 'Match the vehicle number or send a new approval request.',
        'req_new_number' => 'Request approval for new number',
        'rejected' => '✗ Approval request for vehicle :number was rejected.',
        'req_reason' => 'Request reason:',
        'reject_note' => 'Rejection reason:',
        'resend' => 'Resend request',
        'need_approval' => 'New deals require admin approval. Approval is locked per (buyer × vehicle number).',
        'req_new' => 'Request new-deal approval',
    ],

    // Inter-vehicle fund transfer (sale tab)
    'transfer' => [
        'section' => 'Inter-vehicle Fund Transfer',
        'pending_title' => 'Transfer request awaiting approval',
        'target_vehicle' => 'Target vehicle',
        'target' => 'Target',
        'amount' => 'Amount',
        'req_date' => 'Requested',
        'req_no' => 'Request',
        'reason' => 'Reason',
        'transfer_no' => 'Transfer',
        'admin_fallback' => 'Admin',
        'void_reason_label' => 'Void request reason',
        'no_memo' => '(no memo)',
        'finance_confirmed_label' => 'Finance confirmed',
        'finance_rejected_label' => 'Finance rejected',
        'reject_reason_label' => 'Rejection reason',
        'void_finance_rejected_label' => 'Finance void rejection',
        'received' => 'Received',
        'limit' => 'Transfer limit',
        'limit_note' => '(received × 50%)',
        'candidates' => 'Transferable to :count vehicle(s) of the same buyer',
        'retry' => 'Request again',
        'request_btn' => 'Request transfer',
        'not_eligible' => 'Transfer eligibility conditions not met.',
        's' => [
            'approved_awaiting_finance' => 'Approved — awaiting finance',
            'executed' => 'Transfer complete (finance confirmed)',
            'voided_awaiting_finance' => 'Void approved — awaiting finance',
            'voided' => 'Transfer voided',
            'finance_rejected' => 'Finance rejected (cannot remit)',
            'approved' => 'Latest transfer request approved',
            'rejected' => 'Latest transfer request rejected',
            'cancelled' => 'Latest transfer request cancelled',
            'void_rejected' => 'Void request rejected',
            'void_cancelled' => 'Void request cancelled',
            'void_finance_rejected' => 'Finance rejected void (transfer kept)',
            'default' => 'Latest transfer request',
        ],
        'memo' => [
            'approval' => 'Approval memo',
            'reject' => 'Rejection reason',
            'note' => 'Memo',
        ],
    ],

    'ph' => [
        'brand' => 'Hyundai',
        'model_type' => 'Sonata',
        'color' => 'White',
        'fuel_type' => 'Gasoline',
        'use_type' => 'Private',
        'transmission' => 'Auto',
        'drive_type' => '2WD',
        'purchase_from' => 'Auction / Dealer / Private',
        'bank_name' => 'KB / Shinhan / Woori, etc.',
        'account_holder' => 'Individual or corporate name',
        'account_memo' => 'Notes for remittance (optional)',
        'amount_won' => 'Amount (KRW)',
        'note' => 'Note',
        'amount' => 'Amount',
        'rate' => 'Rate',
        'export_decl_amount' => 'leave empty to auto-apply sell price',
        'memo' => 'Internal memo',
    ],

    // Custom validation messages (approval / transfer / ledger unlock reasons)
    'valmsg' => [
        'unlock_reason_required' => 'Enter an unlock reason of at least 10 characters.',
        'unlock_reason_min' => 'The unlock reason must be at least 10 characters.',
        'approval_reason_required' => 'Enter a reason for the approval request.',
        'reason_min5' => 'The reason must be at least 5 characters.',
        'transfer_target_required' => 'Select a target vehicle for the transfer.',
        'transfer_amount_required' => 'Enter the transfer amount.',
        'void_reason_required' => 'Enter a reason for voiding the transfer.',
        // ① new registration required — which tab (salesman = Basic / buyer = Sale)
        'party_required' => 'New registration requires a salesman (Basic tab) and a buyer (Sale tab).',
        'salesman_required' => 'Select a salesman — Basic tab.',
        'buyer_required' => 'Select a buyer — Sale tab.',
        // Foreign-currency vehicles require an exchange rate — otherwise settlement miscalculates.
        'exchange_rate_required' => 'Foreign-currency vehicles require an exchange rate — Sale tab. (Settlement miscalculates without it.)',
        // Sale required trio (pre-empts chk_sale_required DB CHECK)
        'sale_date_required' => 'A sale date is required when a sale price is entered — Sale tab.',
        'sale_buyer_required' => 'A buyer is required when a sale price is entered — Sale tab. To revert to "buyer undecided" after a cancelled purchase, clear the sale price, sale date and freight first; the buyer field then becomes clearable and the undecided checkbox appears.',
    ],

    'tooltip' => [
        'ship_photos_drop' => 'Drag shipping photos here or choose them (multiple)',
        'approval_no' => 'Approval #:no',
        'transfer_target' => 'Target vehicle',
        'sailing' => 'Sailed :ship - ETA :eta',
        'arrived' => 'ETA :eta has passed (estimate)',
        'photos_drop' => 'Drag photos here or choose them (multiple)',
    ],
    'domestic' => [
        'badge' => 'Domestic',
        'badge_pending' => 'Domestic (KRW pending)',
        'awaiting_krw' => 'This is a domestic buyer but the currency is :currency. Saving is allowed, but a settlement created now uses the export formula, not the domestic one — switch the currency to KRW before settlement to get domestic treatment.',
        'buyer_has_foreign' => 'This buyer has :count non-KRW vehicle(s). Those are settled with the export formula, not the domestic one, until their currency is changed to KRW.',
    ],
    'breakdown' => [
        'settlement_closed' => 'The secondary settlement for this vehicle is closed, so the down payment, interim, advance-1 and fee cannot be changed. Unlock it for accounting revision (manager approval) from the settlement screen.',
    ],
    'taxinvoice' => [
        'not_paid' => 'The purchase amount is not fully paid. ₩:amount is still outstanding, so a tax invoice cannot be requested.',
        'send_btn' => '🧾 Request tax invoice',
        'confirm' => 'This sends a tax-invoice request for this purchase, along with a link to download the business registration certificate. Send it?',
        'save_first' => 'Save the vehicle first.',
        'no_phone' => 'Enter the domestic dealer phone number first.',
        'no_cert' => 'Register the business registration certificate first.
Contact the system administrator.',
        'sent' => 'Tax-invoice request sent.',
        'failed' => 'Send failed (:reason)',
    ],
];
