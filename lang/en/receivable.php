<?php

// i18n — Receivables (erp/receivables). progress=domain.progress, role=domain.role reused.
return [
    'title' => 'Receivables',
    'subtitle' => 'Outstanding balance · collection history · risk monitoring',
    'admin_only' => 'Access: managers only',
    'forbidden' => 'You do not have permission to access Receivables.',

    'tab' => [
        'all' => 'All receivables',
        'grace' => 'Grace period',
        'before_shipping' => 'Pre-shipment unpaid',
        'after_shipping' => 'Post-shipment unpaid',
        'deposit' => 'Deposit (savings used)',
        'paid_up' => 'Paid up',
        'import_cleared' => 'Import write-down',
        'import_cleared_hint' => 'Vehicles whose unpaid balance was zeroed out with an "other" collection entry during the historical data import. Money is still owed.',
    ],
    'import_cleared_note' => 'These vehicles look paid up but money was never actually received. During the historical data import the shortfall was recorded as an "other" collection entry so the balance would read zero. The unpaid figure shows 0, so bill from the amount on the "other" line in the collection history below. Once payment arrives, delete that line and record the real payment.',
    'import_cleared_badge' => 'Import write-down',
    'import_cleared_cars' => ':n vehicles',
    'import_cleared_empty' => 'No matching entries',
    'import_cleared_settle' => 'Received',
    'import_cleared_settle_hint' => 'Enter the amount actually received. It is recorded as a cash collection and this line is reduced by the same amount. If paid in full the line disappears and the vehicle drops off the list. No accounting figure moves — revenue was recognised at the point of sale and the outstanding balance stays at zero.',
    'import_cleared_settled' => 'Receipt recorded.',
    'import_cleared_settled_note' => 'Import write-down actually received',
    'tab_math' => 'All receivables :all = grace :grace + pre-shipment :before + post-shipment :after (paid-up shown separately)',
    'period_note' => 'Outstanding balances are shown in full, regardless of date filter',
    'overpaid_badge' => 'Overpaid',
    'overpaid_note' => ':count vehicles · :amount KRW received in excess (refund due) — see the Paid up tab',

    'kpi' => [
        'total_sale' => 'Total sales (KRW)',
        'total_paid' => 'Total received',
        'total_unpaid' => 'Total outstanding',
        'grace' => 'Payment grace (excl. from receivables)',
        'grace_hint' => ':count vehicle(s) · becomes receivable at sale date +10d',
        'unpaid_ratio' => 'Outstanding :pct%',
        'basis_note' => 'Based on :sold sold (:unpaid unpaid, paid-up included)',
        'default_ratio' => 'Default rate :pct% (among :count unpaid)',
        'share_split' => 'Pre-shipment :before% · post-shipment :after% (same as the daily alert)',
        'paid_ratio' => 'Collected :pct%',
        'ratio_basis' => '(:cur basis)',
        'risk_count' => 'At-risk count (danger + critical)',
    ],
    'unit_won' => 'KRW',
    'unit_count' => '',
    'currency_label' => 'Currency',
    'currency_all' => 'All (₩)',

    'search_ph' => 'Plate · VIN · brand · vessel (VSL) · container',
    'all_salesman' => 'All salesmen',
    'all_buyer' => 'All buyers',
    'all_progress' => 'All statuses',
    'all_risk' => 'All risk levels',
    'cancel' => [
        'all' => 'All',
        'only' => 'Cancelled only',
        'normal' => 'Normal only',
        'overpaid' => 'Overpaid only',
        'import_cleared' => 'Import write-down only',
    ],
    'ratio_all' => 'Unpaid % ALL',
    'ratio_min' => ':percent%+',

    'risk' => [
        'grace' => 'Payment pending',
        'safe' => 'Safe',
        'caution' => 'Caution',
        'danger' => 'Danger',
        'critical' => 'Critical',
    ],

    'col' => [
        'no' => 'No.',
        'vehicle_no' => 'Plate',
        'brand_type' => 'Brand/Model',
        'vin' => 'VIN',
        'salesman' => 'Salesman',
        'buyer' => 'Buyer',
        'sale_total' => 'Sale total',
        'unpaid' => 'Unpaid',
        'unpaid_ratio' => 'Unpaid %',
        'progress' => 'Status',
        'bl' => 'BL',
        'age' => 'Age',
        'risk' => 'Risk',
        'manager' => 'AR manager',
    ],
    'age_days' => ':days d',
    'sort' => [
        'unpaid' => 'Largest unpaid',
        'age' => 'Oldest first',
    ],
    'empty' => 'No vehicles found.',
    'buyer_none' => 'Buyer unassigned',
    'mobile_unpaid' => 'Unpaid',

    // Slide panel (collection history)
    'history' => 'Collection History',
    'manager_section' => 'AR Manager',
    'manager_unassigned' => 'Unassigned',
    'assign' => 'Assign',
    'manager_assigned' => 'AR manager assigned.',
    'form_title_edit' => 'Edit Collection',
    'form_title_add' => 'Add Collection',

    'field' => [
        'date' => 'Collection date',
        'collector' => 'Collector',
        'method' => 'Method',
        'amount' => 'Amount',
        'amount_attr' => 'Collection amount',
        'rate' => 'Rate',
        'memo' => 'Memo',
        'select' => 'Select',
    ],
    'pending_confirm' => 'Awaiting finance confirmation',
    'pending_confirm_title' => 'The receivable moves only after finance confirms it on the Finance screen.',
    'deposit_two_step' => 'Deposit creates a sale balance; the receivable drops only after finance confirms it. Right after saving the receivable is unchanged - use Cash/Offset/Other for an immediate effect.',
    'method' => [
        'deposit' => 'Deposit',
        'cash' => 'Cash',
        'offset' => 'Offset',
        'other' => 'Other',
        'savings' => 'Savings credit',
        'write_off' => 'Write-off (seller absorbed)',
        'misc_loss' => 'Misc loss',
    ],
    'method_deposit_full' => 'Deposit (auto-creates final_payment)',
    'savings' => [
        'balance_hint' => 'Available savings credit for this buyer: :balance :currency',
        'insufficient' => 'Exceeds the savings balance. Available: :balance :currency',
        'no_buyer' => 'Savings credit can only be used on vehicles with a buyer assigned.',
        'auto_note' => 'Savings usage entered on the vehicle sales tab',
    ],
    'memo_ph' => 'Collection notes · contact log, etc.',
    'btn_edit_save' => 'Save changes',
    'btn_add' => 'Add',
    'saved_edit' => 'Collection updated.',
    'saved_add' => 'Collection added.',
    'deleted' => 'Collection deleted.',
    'delete_failed' => 'This collection record cannot be deleted — it is linked to a finance-confirmed payment.',
    'save_failed' => 'Failed to save the collection record. Please check the amount and method.',
    'err_closed_no_deposit' => "This vehicle's secondary settlement is closed; the 'Deposit' method can't be used. Choose cash, offset, other, or write-off.",
    'err_closed_no_amount_edit' => 'The secondary settlement is closed; the deposit amount cannot be changed retroactively. Use the vehicle ledger unlock (management approval).',
    'err_closed_no_date_edit' => 'The secondary settlement is closed; the collection date cannot be changed retroactively. Use the vehicle ledger unlock (management approval).',
    'err_closed_no_rate_edit' => 'The exchange rate cannot be edited for a vehicle whose secondary settlement is closed (prevents retroactive FX-difference changes).',

    // Overpayment → savings credit conversion
    'overpay' => [
        'closed_note' => 'The secondary settlement for this vehicle is closed. Leave a correction reason to convert it.',
        'closed_denied' => 'Only managers and above can convert a vehicle whose secondary settlement is closed.',
        'reason_ph' => 'Correction reason (:n characters or more) - e.g. buyer sent in error, converting to credit',
        'reason_required' => 'The secondary settlement for this vehicle is closed. Enter a correction reason of at least :n characters.',
        'title' => 'Overpaid',
        'hint' => 'Move the excess to the buyer savings credit and set this vehicle receivable to 0.',
        'btn' => 'Convert to savings',
        'confirm' => 'Convert overpayment :amount to buyer savings credit and set this vehicle receivable to 0. Proceed?',
        'done' => 'Overpayment :amount converted to buyer savings credit.',
        'not_overpaid' => 'Not overpaid (receivable ≥ 0).',
        'no_buyer' => 'No sale buyer assigned; cannot convert to savings.',
        'exceeds_confirmed' => 'Excess exceeds total confirmed balance payments. Check the overpayment source and correct manually.',
        'failed' => 'Overpayment-to-savings conversion failed.',
        'hint_choice' => 'Savings = credit the buyer can spend later / Misc loss = closed as company money (not refunded).',
        'btn_misc_loss' => 'Convert to misc loss',
        'confirm_misc_loss' => 'Treat the overpayment of :amount as a miscellaneous loss (company money) and zero this vehicle\'s receivable. It will not be refunded to the buyer. Proceed?',
        'done_misc_loss' => 'Overpayment of :amount was recorded as a miscellaneous loss.',
        'misc_loss_note' => 'Overpayment recorded as misc loss',
        'ledger_note' => 'Overpayment clean-up (:plate)',
    ],
    'list_title' => 'Collection history (latest first)',
    'list_empty' => 'No collection history.',
    'mirror_title' => 'Mirrored with final_payments',
    'delete_confirm' => 'Delete this collection record? The mirrored payment record will also be deleted.',
];
