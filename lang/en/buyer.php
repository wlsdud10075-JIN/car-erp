<?php

// i18n — Buyer management (erp/buyers/index). Shared in common.php, ID types reuse consignee.id_type.
return [
    'title' => 'Buyers',
    'create_btn' => 'Add Buyer',
    'edit_title' => 'Edit Buyer',
    'create_title' => 'Add Buyer',
    'search_ph' => 'Name · email · contact',
    'no_salesman_filter' => 'No salesman',
    'all_salesmen' => 'All Salesmen',
    'unassigned' => 'Unassigned',
    'empty' => 'No buyers.',
    'delete_confirm' => 'Delete buyer :name?',
    'delete_confirm_simple' => 'Delete this buyer? This cannot be undone.',
    'saved' => 'Buyer saved.',
    'deleted' => 'Buyer deleted.',
    'col_name' => 'Buyer Name',

    'tab' => [
        'basic' => 'Basic Info',
        'consignees' => 'Consignees',
        'savings' => 'Savings',
        'cash' => 'Cash',
    ],

    'receivable' => [
        'title' => 'Buyer Receivables',
        'summary' => ':count vehicles · ₩:amount total',
        'fully_paid' => '✓ Paid',
        'paid_pct' => 'Paid :pct%',
        'unpaid' => 'Outstanding ₩:amount',
        'unpaid_ratio' => 'Unpaid :pct%',
        'completed_label' => 'Completed (separate)',
        'completed_value' => ':count vehicles · ₩:amount',
        'shipped_label' => 'Shipped out (separate)',
        'shipped_value' => ':count vehicles · ₩:amount',
        'deposit_title' => 'Purchasing room',
        'room_received' => 'Can buy up to :pct% of what was received',
        'room_spent' => 'Already spent on purchases (deposit·balance)',
        'room_basis' => 'Based on ₩:paid received across :count pre-shipping vehicles',
        'deposit_note' => 'You can buy up to this amount for this buyer. It grows as sale balances come in.',
        'deposit_note_full' => '⚠ Purchasing room used up — more sale payments are needed to buy for this buyer.',
    ],
    'fees' => [
        'title' => '💸 Cumulative Seller-Absorbed Amount (fees · write-offs)',
        'desc' => 'Remittance fees and losses we have absorbed for this buyer cumulatively. Use as a negotiation point.',
        'count' => ':count items',
        'breakdown' => ':fee fee + :wo write-off',
    ],

    'field' => [
        'domestic' => 'Domestic-sale buyer',
        'domestic_hint' => 'Vehicles sold to this buyer settle with the domestic formula (total sale − purchase − deregistration − towing) instead of the export margin. Only KRW vehicles get that treatment — non-KRW vehicles can still be assigned to this buyer, but they settle with the export formula until their currency is changed to KRW.',
        'name' => 'Buyer Name',
        'name_ph' => 'TOKYO AUTO TRADING',
        'salesman' => 'Salesman',
        'salesman_unassigned' => '-- Unassigned --',
        'salesman_note' => '[Management] accounts see only the buyers of the salesmen they manage.',
        'inherited' => 'Inherited buyer',
        'inherited_hint' => 'Buyer taken over from a departed salesman. Employee settlements for this buyer are fixed at 50,000 KRW per unit (overrides tiered settlement; 0 for loss-making vehicles). Freelancers keep the ratio-based settlement.',
        'credit_score' => 'Credit score :score',
        'credit_unavailable' => 'No trade history yet, so no score. The global thresholds apply as-is — grading a new buyer at the bottom would block their first deal outright.',
        'credit_loss_capped' => 'grade capped by write-off history',
        'credit_apply' => 'Fill recommended (ship :entry% / purchase :purchase%)',
        'credit_mismatch' => 'Current settings differ from the recommendation — :list. You can ignore this; a value once entered never follows the score automatically.',
        'lock_applied' => 'Recommended values filled in. Press Save to apply.',
        'lock_section' => 'Lock thresholds (super admin only)',
        'lock_hint' => 'Required paid ratio for this buyer only. Leave blank to follow the company-wide setting. This value takes precedence over the global one; a per-vehicle unpaid override skips both.',
        'lock_shipping_entry' => 'Clearance / shipping entry',
        'lock_purchase_registration' => 'Purchase registration',
        'lock_global' => 'global :pct%',
        'lock_zero_note' => 'Note: 0 means "no lock" and is different from blank. Blank uses the global setting. Changes are recorded in the audit log.',
        'lock_readonly' => 'Only a system administrator can change these values. Ask them if an adjustment is needed.',
        'unsecured_limit' => 'Unsecured credit',
        'unsecured_limit_ph' => 'Leave blank for none (e.g. 5,000,000)',
        'unsecured_limit_hint' => 'Allows purchases up to this amount even with no vehicles in Korea to serve as collateral. Intended for long-standing buyers. Leave blank to keep the current behaviour (unpaid-ratio gate only). If set, this buyer is blocked once base limit + unsecured credit is used up.',
        'unsecured_available' => 'Unsecured remaining',
        'unsecured_used' => 'used :amount',
        'unsecured_untouched' => 'Not used yet. It stays intact while the deposit still has room.',
        'base_limit' => 'Deposit (received x :pct%)',
        'total_limit' => 'Total limit',
        'limit_used' => 'Used (confirmed purchase payments)',
        'limit_available' => 'Remaining',
        'shipped_unpaid_note' => 'Note: KRW :amount outstanding on shipped vehicles is excluded from this limit calculation.',
        'inherited_from' => 'Previous salesman (optional)',
        'inherited_at' => 'Inherited on (optional)',
        'inherited_optional_note' => 'The two fields below are for record keeping. Leave them blank if the departed salesman is unknown or not listed — the checkbox alone applies the 50,000 KRW per-unit rate.',
        'contact_name' => 'Contact Name',
        'passport_id' => 'Passport/ID No.',
        'passport_id_ph' => 'e.g. M12345678',
        'passport_id_hint' => 'Filled into the Passport/ID number field of the buyer block on the Sales Contract.',
        'eori_ph' => 'e.g. DE7078617',
        'tax_ph' => 'e.g. LT792855314',
    ],

    'cons' => [
        'saved' => 'Consignee saved.',
        'deleted' => 'Consignee deleted.',
        'save_first' => 'Save the basic info first to add consignees.',
        'empty' => 'No consignees.',
        'add' => '+ Add Consignee',
        'delete_confirm' => 'Delete this consignee?',
        'form_edit' => 'Edit Consignee',
        'form_add' => 'Add Consignee',
        'name' => 'Consignee Name',
        'id_type' => 'ID Type',
        'id_value' => 'ID Number',
    ],

    'savings' => [
        'save_first' => 'Save the basic info first to manage savings.',
        'balances_title' => 'Savings Balance by Currency',
        'no_balance' => 'No savings transactions.',
        'add_txn' => 'Add Transaction',
        'currency' => 'Currency',
        'type' => 'Type',
        'amount' => 'Amount',
        'add_btn' => 'Add',
        'rate' => 'Rate at earning',
        'rate_note' => '(optional)',
        'won' => ' KRW',
        'unrated_note' => '⚠ :amount :currency excluded from the KRW total (no rate recorded at earning).',
        'next_out_line' => 'Next out → :date batch, :amount left (:rate)',
        'rate_label' => 'rate :rate',
        'rate_none' => 'no rate',
        'col_fifo' => 'FIFO',
        'lot_left' => ':amount left',
        'lot_spent' => 'spent',
        'take_line' => ':amount from the :date batch (:left left)',
        'back_line' => ':amount restored to the :date batch',
        'type_earned' => 'Earned',
        'type_used' => 'Used',
        'type_refund' => 'Refund',
        'type_adjustment' => 'Adjustment',
        'col_date' => 'Date',
        'col_amount' => 'Amount',
        'col_balance' => 'Balance',
        'added' => 'Savings transaction added.',
        'cancelled' => 'Savings transaction cancelled.',
        'cancel_confirm' => 'Cancel this transaction?',
        'cancel_title' => 'Cancel',
        'no_history' => 'No history.',
        't_earned' => 'Earned',
        't_used' => 'Used',
        't_refund' => 'Refund',
        't_adjustment' => 'Adjustment',
        't_cancelled' => 'Cancelled',
        'neg_balance' => 'Cancelling would make the balance negative. Cannot cancel.',
    ],

    // Buyer cash ledger (2026-09-04) — see docs/design/buyer-cash-ledger.md
    'cash' => [
        'save_first' => 'Save the buyer first to manage cash receipts.',
        'disabled' => 'This company does not use the buyer cash ledger. Turn it on in feature settings.',
        'no_permission' => 'Only finance and management can record cash receipts.',
        'balances_title' => 'Cash by currency',
        'received' => 'Received',
        'allocated' => 'Applied to vehicles',
        'remaining' => 'Remaining',
        'no_receipt' => 'No receipts recorded.',
        'add_title' => 'Record a receipt',
        'received_date' => 'Received on',
        'currency' => 'Currency',
        'amount' => 'Amount (FX)',
        'add_btn' => 'Record',
        'added' => 'Receipt recorded.',
        'deleted' => 'Receipt removed.',
        'delete_used_confirm' => 'This receipt is already applied to vehicles. Deleting it will redraw those amounts from the remaining receipts. Continue?',
        'delete_short' => 'Cannot delete — the remaining receipts cannot cover what was already applied. :reason',
        'deleted_reallocated' => 'Receipt removed; the applied amounts were redrawn from the remaining receipts.',
        'delete_confirm' => 'Remove this receipt?',
        'col_date' => 'Received',
        'col_amount' => 'Amount',
        'col_remaining' => 'Remaining',
        'col_used' => 'Applied to',
        'used_date_title' => 'The date is the payment date of that sale balance (for a ledger fee, the fee date). When one balance spans two receipts the same vehicle appears on both rows - the oldest cash is drawn first.',
        'ledger_fee' => 'Ledger fee',
        // 🔁 FIFO 재배분 (jin 2026-09-10 제보) — 잔금 수금일 < 입금 수령일 인 줄.
        //    실측: 09-10 입금이 368머4746 의 09-09 잔금 0.49 를 메웠다. 표시가 없으면
        //    「09-10 에 받은 돈이 09-09 에 쓰였다」로 읽혀 시간이 거꾸로 간 것처럼 보인다.
        'backfill_badge' => 'Backfilled',
        'backfill_hint' => 'This balance was recorded before this receipt arrived. The earlier cash went to another vehicle, so this receipt covered the shortfall - the oldest cash is always drawn first (the amounts are correct).',
        'col_by' => 'By',
        'spent' => 'Fully applied',
        'not_used' => 'Not applied yet',
        'next_out' => 'Next to be drawn from',
        'rate_note' => 'The exchange rate is entered on the sale balance row — record only the FX amount here.',
        'gate_blocked' => 'Not enough cash on hand for :buyer. Needed :need, available :available (short :short). Record the incoming payment on the buyer Cash tab first.',
        'fee_section' => 'Write off as fee',
        'overpay_section' => 'Overpayment clean-up',
        'fee_hint' => 'When a wire fee made the actual receipt smaller than the recorded amount, clear the difference here. It is taken from the remaining cash, oldest receipt first.',
        'fee_hint_vehicle' => 'A wire fee that belongs to one vehicle goes in Sale tab -> Wire fee; that already reduces this cash. Use this box only for a fee you cannot attach to a vehicle - entering it in both places deducts it twice.',
        'fee_badge' => 'Wire fee',
        'where_to_delete' => 'This table is read-only. Remove a vehicle wire fee from that vehicle’s sales tab, and a balance from the receivables screen.',
        'fee_badge_hint' => 'Remove this line from the Wire fee field on that vehicle’s sales tab.',
        'fee_date' => 'Fee date',
        'fee_amount' => 'Fee (foreign currency)',
        'fee_note' => 'Reason (optional)',
        'fee_note_ph' => 'e.g. intermediary bank fee',
        'fee_add_btn' => 'Write off',
        'fee_open' => '+ Fee',
        'fee_close' => 'Close',
        'fee_added' => 'Fee recorded.',
        'fee_deleted' => 'Fee cancelled — the cash is back.',
        'fee_delete_confirm' => 'Cancel this fee? The cash that was deducted comes back.',
        // Savings use (jin 2026-09-22) — inside the cash tab, separate from the ledger
        'savings_title' => 'Savings credit',
        'savings_balance_none' => 'No savings balance',
        'savings_hint' => 'Savings are company-granted credit, kept apart from the cash ledger above. Using them here updates the vehicle sale tab, receivables and the savings balance together.',
        'savings_vehicle' => 'Vehicle (unpaid)',
        'savings_vehicle_ph' => 'Select vehicle',
        'savings_amount' => 'Amount',
        'savings_date' => 'Date',
        'savings_use_btn' => 'Apply savings to balance',
        'savings_col_vehicle' => 'Vehicle',
        'savings_no_use' => 'No savings usage yet.',
        'savings_no_vehicle' => 'Not this buyer\'s vehicle.',
        'savings_no_vehicle_label' => '(no vehicle)',
        'savings_insufficient' => 'Insufficient savings — remaining :balance :currency',
        'savings_over_unpaid' => 'Exceeds the unpaid amount — this vehicle owes :unpaid :currency',
        'savings_note_default' => 'Savings used from the buyer cash tab',
        'savings_used_ok' => 'Savings applied to the balance of :vehicle.',
        'fee_none' => 'No fees recorded.',
        'fee_list_title' => 'Fees',
        'fee_from' => 'Taken from',
        'fee_zero' => 'Fee amount is zero.',
        'fee_short' => 'Remaining cash is short by :short :currency. You can only write off what is left.',
        'fee_ceiling' => 'Max you can write off now: :amount',
        'gate_race' => 'That buyer cash was just used elsewhere. Reload the screen and save again.',
    ],
];
