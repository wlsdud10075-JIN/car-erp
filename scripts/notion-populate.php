...
$items = [
    ...
    // ✅ 2026-07-01 배포분
    $done('판매계약서 서류 자동생성 (수출 다중차량·30슬롯)', ['기능']),
    $done('2차 정산 비용 일괄 기입 (면허비 n/1·탁송비 명세서 매칭)', ['기능']),
    $done('선적요청 탭 재구성 (할 일 뷰·검색·페이지네이션)', ['기능']),
    $done('미입금 우회 통합 (통관·선적 50% 게이트 1건 승인)', ['기능']),
    $done('판매탭 UX 개선 (총판매가·남은잔금·저장하고 계속)', ['기능']),
    $done('차량 목록 검색에 차대번호(VIN) 추가', ['기능']),
