...
function buildWorkflow(?string $finId, ?string $cleId, ?string $mgrId, ?string $catalogId): array
{
    ...
    $b[] = bul('② 말소  [관리] — 말소신청 · 등록증 · 위임장 · 말소증(한글/영문)');
    $b[] = bul('③ 판매 · 입금  [재무] — 판매가 · 통화 · 환율 · 바이어, 입금 확정(계약금/중도금/잔금)');
    $b[] = bul('④ 반입(선적) · 통관  [수출통관] — 입금률이 「선적 진입」 기준 이상 필요 · 통관 바이어 · 포워딩사 · 면장 · 수출신고서');
    $b[] = bul('⑤ B/L 발급  [수출통관] — 「B/L 발행」 기준 충족(기본 완납) 필요 · B/L 문서 업로드 → 거래완료');
    $b[] = bul('⑥ 서류 발송(EMS · DHL) · 거래완료  [수출통관] — 등기번호 · 운송장번호 · 발송비');
    $b[] = bul('⑦ 정산 · 2차 정산  [재무 확정 → 관리 지급] — 마진 자동계산 · confirmed · paid · closed');
    $b[] = divider();
    $b[] = h2('판매채널 3종');
    $b[] = bul('export(수출) — 다중통화, 통관 · B/L · 서류 발송(EMS·DHL) 풀 흐름');
    $b[] = bul('heyman(헤이맨) / carpul(카풀) — 국내 바이어, 원화 정산, 통관 · B/L · DHL 흐름 없음');
    $b[] = divider();
    $b[] = h2('🚦 진행 게이트 (막히면 아래 「에러 & 잠금」으로)');
    $b[] = callout('🟡', '입금률이 「선적 진입」 기준 이상 → 반입(선적) · 통관 진입 가능. (미달 시 관리/관리자 미입금 우회 승인 — 진입 1건이면 통관·선적 둘 다 통과)', 'yellow_background');
    $b[] = callout('🔴', '「B/L 발행」 기준 충족(기본 완납) → B/L 발급 가능. (미완납은 관리/관리자 우회 승인 — B/L 발행 단계 별도. 진입 우회로는 안 뚫림)', 'red_background');
    if ($catalogId) {
...
function buildLockCatalog(?string $workflowId): array
{
    ...
    $b[] = table([
        ['증상 (화면 문구)', '원인', '해결 · 우회'],
        ['"판매 입금률 < N% (미수율 XX%) 차량은 통관·선적 진입 불가"', 'C5 게이트 — 「선적 진입」 기준 입금률 미만 (N은 기능설정 값, 바이어별 설정이 있으면 그쪽 우선)', '기준 이상 입금, 또는 관리/관리자 [미입금 우회] 승인. 진입 우회 1건이면 통관·선적 둘 다 통과'],
        ['"환율 미입력 외화 차량은 통관 진입 불가"', '외화차 환율 0 → 미수율 평가 불가', '환율 입력(송금받을때 기준), 또는 관리자 미입금 우회 승인'],
        ['"B/L 발행 차단 — 미수율 XX% (잔금 100% 미완납)"', 'G1 게이트 — 잔금 100% 미완납 (화물 인도권)', '100% 완납, 또는 관리/관리자 우회 승인 \'B/L 발행\' 단계. ※진입 우회로는 안 뚫림 — 별도 승인'],
