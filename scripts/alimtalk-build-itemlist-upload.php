...
function rowValues(string $code, string $profile): array
{
    ...
    $v = [
        ...
        'B' => $code,
        'C' => $t['name'],
        // 🚨 D·H 를 「샘플 값 그대로」 두면 안 된다 (2026-08-26). 기본형 스크립트는 6행만 쓰니 괜찮았지만
        //    여긴 6~21행에 걸쳐 쓰는데 **샘플은 행마다 예시가 다르다** — 실측 13행 MI · 14행 AD · 15행 EX,
        //    13·14행엔 H=0 까지 박혀 있다. 12종을 한 번에 넣고서야 드러났다(그전엔 1~2종이라 6·7행만 썼다).
        //    verify 가 잡아줬지만, 잔재는 「업로드는 되는데 심사에서 반려」로 나타나므로 여기서 못박는다.
        'D' => 'BA',
        'E' => $t['body'],
        // 🚫 H(보안 템플릿 여부)는 **비우지 말 것** — 샘플은 6~21행 전부 '0' 이고, 업로드가 통과한
        //    승인본도 '0' 이다. 한 번 비웠다가 BizM 업로더가 「6행이 틀렸다」로 거부했다(2026-08-26).
        //    D 와 달리 H 는 행마다 같으므로 샘플 값을 그대로 두는 게 맞다.
